...
build:
    cargo build --release -p moqtap-cli
...
publish-dry-all:
    ...
    cargo publish -p moqtap-cli --dry-run
...
release crate version:
    ...
    if [ "{{crate}}" = "moqtap-cli" ]; then
        echo "CLI binaries will be built and attached to a GitHub release."
    fi
