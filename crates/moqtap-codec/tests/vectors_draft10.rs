...
fn run_message_vectors(relative_path: &str) {
    ...
    for vector in &file.vectors {
        if let Some(expected_decoded) = &vector.decoded {
            ...
            let actual_json = test_vectors::draft10_json::message_to_json(&msg);
            assert_eq!(
                actual_json,
                *expected_decoded,
...
macro_rules! d10_test {
    ($name:ident, $file:expr) => {
        #[test]
        fn $name() {
            run_message_vectors(concat!("transport/draft10/codec/messages/", $file));
        }
    };
}

d10_test!(d10_client_setup, "client-setup.json");
d10_test!(d10_server_setup, "server-setup.json");
d10_test!(d10_goaway, "goaway.json");
d10_test!(d10_max_subscribe_id, "max-subscribe-id.json");
d10_test!(d10_subscribes_blocked, "subscribes-blocked.json");
d10_test!(d10_subscribe, "subscribe.json");
d10_test!(d10_subscribe_ok, "subscribe-ok.json");
d10_test!(d10_subscribe_error, "subscribe-error.json");
d10_test!(d10_subscribe_update, "subscribe-update.json");
d10_test!(d10_subscribe_done, "subscribe-done.json");
d10_test!(d10_unsubscribe, "unsubscribe.json");
d10_test!(d10_announce, "announce.json");
d10_test!(d10_announce_ok, "announce-ok.json");
d10_test!(d10_announce_error, "announce-error.json");
d10_test!(d10_announce_cancel, "announce-cancel.json");
d10_test!(d10_unannounce, "unannounce.json");
d10_test!(d10_subscribe_announces, "subscribe-announces.json");
d10_test!(d10_subscribe_announces_ok, "subscribe-announces-ok.json");
d10_test!(d10_subscribe_announces_error, "subscribe-announces-error.json");
d10_test!(d10_unsubscribe_announces, "unsubscribe-announces.json");
d10_test!(d10_track_status_request, "track-status-request.json");
d10_test!(d10_track_status, "track-status.json");
d10_test!(d10_fetch, "fetch.json");
d10_test!(d10_fetch_ok, "fetch-ok.json");
d10_test!(d10_fetch_error, "fetch-error.json");
d10_test!(d10_fetch_cancel, "fetch-cancel.json");
d10_test!(d10_unknown_type, "unknown-type.json");
...
fn run_subgroup_vectors(relative_path: &str) {
    ...
    for vector in &file.vectors {
        if let Some(expected) = &vector.decoded {
            ...
            let any_header = AnySubgroupHeader::Draft10(header.clone());
        ...
        if vector.error.is_some() {
            ...
            let failure = match SubgroupHeader::decode_stream(&mut cursor) {
                ...
                Ok(header) => dispatch_check::drain_subgroup_objects(
                    &AnySubgroupHeader::Draft10(header),
                    &mut cursor,
                ),
            };
            match failure {
...
fn run_datagram_vectors(relative_path: &str) {
    ...
    for vector in &file.vectors {
        if let Some(expected) = &vector.decoded {
            ...
            // Draft-10 (same as draft-09): datagram payload is everything remaining after header.
...
#[test]
fn d10_data_stream_subgroup() {
    run_subgroup_vectors("transport/draft10/codec/data-streams/subgroup.json");
}

#[test]
fn d10_data_stream_datagram() {
    run_datagram_vectors("transport/draft10/codec/data-streams/datagram.json");
}

#[test]
fn d10_data_stream_datagram_status() {
    run_datagram_status_vectors("transport/draft10/codec/data-streams/datagram-status.json");
}

#[test]
fn d10_data_stream_fetch() {
    run_fetch_vectors("transport/draft10/codec/data-streams/fetch-header.json");
}
