#![cfg(feature = "draft20")]
//! Draft-20 error and status code registries, checked against the draft.
//!
//! The code points asserted here were read out of the `Name`/`Code`/
//! `Specification` tables in the IANA Considerations of draft-20: tables 18
...
//!
//! Draft-19 assigned three code points that draft-20 does not, one in each of
...
//! transcription slip. Section 14 is what makes the removal a receiver's
...
macro_rules! check_registry {
    ($name:literal, $ty:ty, [$($code:expr => $variant:expr),+ $(,)?]) => {{
        ...
        // highest code point any draft-20 registry uses (REQUEST_ERROR 0x36).
        for c in 0u64..=0x40 {
            ...
            assert!(
                ...
                "{}: {:#x} is unassigned in draft-20 but decoded to {:?}",
                $name,
                c,
                <$ty>::from_u64(c)
            );
...
/// Draft-20 section 15.11.1, table 18 (Session Termination Error Codes).
/// 20 assignments; definitions in section 3.5. Draft-19 had 21: `0x15`
/// `VERSION_NEGOTIATION_FAILED` is gone.
#[test]
fn session_error_codes_match_draft20_section_15_11_1() {
    ...
    // Draft-14 assigned TOO_MANY_REQUESTS at 0x7. Draft-20 does not assign 0x7
...
/// Draft-20 section 15.11.2, table 19 (REQUEST_ERROR Codes).
/// 18 assignments; definitions in section 10.6.2. Draft-19 had 19: `0x32`
/// `INVALID_JOINING_REQUEST_ID` went with the Joining Fetch.
#[test]
fn request_error_codes_match_draft20_section_15_11_2() {
...
/// Draft-20 section 15.11.3, table 20 (PUBLISH_DONE Codes).
/// 9 assignments; definitions in section 10.12 — draft-19's 10.11, renumbered
/// because PUBLISH_STATE_NOTIFY took 10.10. Draft-19 had 10: `0x3`
/// `SUBSCRIPTION_ENDED` is gone, and the behaviour went with it (Section 5.1.2:
...
#[test]
fn publish_done_status_codes_match_draft20_section_15_11_3() {
    ...
    // Section 10.12 prints MALFORMED_TRACK (0x12) between EXPIRED (0x6) and
...
/// Draft-20 section 15.11.4, table 21 (Stream Reset Error Codes).
/// 10 assignments; definitions in section 3.3.4. The one registry of the four
/// draft-20 left alone.
#[test]
fn stream_reset_error_codes_match_draft20_section_15_11_4() {
...
/// The three code points draft-19 assigned and draft-20 does not.
///
...
///
/// Nothing here refuses a frame carrying one. Draft-20 Section 14: "Receipt of
...
///
/// ```text
/// PUBLISH_DONE 0x3 (draft-19 SUBSCRIPTION_ENDED) is unassigned in draft-20
...
fn the_three_codes_draft20_removed_are_unassigned() {
    assert!(
        ...
        "session 0x15 (draft-19 VERSION_NEGOTIATION_FAILED) is unassigned in draft-20 and \
         decoded to {:?}",
        SessionErrorCode::from_u64(0x15)
    );
    assert!(
        RequestErrorCode::from_u64(0x32).is_none(),
        "REQUEST_ERROR 0x32 (draft-19 INVALID_JOINING_REQUEST_ID) is unassigned in draft-20 \
         and decoded to {:?}",
        RequestErrorCode::from_u64(0x32)
    );
    assert!(
        PublishDoneStatusCode::from_u64(0x3).is_none(),
        "PUBLISH_DONE 0x3 (draft-19 SUBSCRIPTION_ENDED) is unassigned in draft-20 and \
         decoded to {:?}",
