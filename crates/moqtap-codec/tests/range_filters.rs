//! Draft-19's Range Filters, and the two things about them that are easy to
//! get wrong in opposite directions.
//!
//! Section 5.1.3 gives five parameter types, 0x25 through 0x29, one value shape,
//! and three rules. Every one of the three is answered with "MUST be rejected
...
///
/// Section 5.1.3: "For example, to express ranges 3-5 and 10-15: the first Start
...
/// asserted rather than assumed. Draft-19 answers a bad Range Filter with a
...
/// # What it catches
///
/// Calling the reader from draft-19's `decode_parameters` — the shape the
...
fn a_filter_this_module_cannot_read_does_not_refuse_the_frame() {
    let message = ControlMessage::Subscribe(Subscribe {
        ...
        parameters: vec![KeyValuePair {
            ...
            // three things Section 5.1.3 answers with a REQUEST_ERROR.
...
///
/// Section 10.2.12: "If a decoded value exceeds 255, the endpoint MUST reject
...
///
/// Sections 10.2.13 and 10.2.14: the Property Type "MUST be even, i.e. a single
