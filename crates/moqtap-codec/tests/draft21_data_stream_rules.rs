//! Draft-21 data-stream rules that are about the frame rather than its
...
//!
//! The set of valid values did not move — computing Section 11.3.1's three
//! conditions and Section 11.2.1's three gives back draft-19's enumerations
...
//!   falls back on Section 6.4.1's unknown-type rule;
...
use moqtap_codec::draft21::types::{ObjectStatus, PayloadPermission};
...
/// Every subgroup header Type draft-21 Section 11.3.1 names as invalid
...
/// Whether draft-21 Section 11.3.1 permits `header_type`.
///
...
///
/// Note the shape of the middle one. Section 11.2.1 reserves the same bit for a
...
/// The decoder accepts exactly the subgroup header Types draft-21 assigns.
///
...
///
/// ```text
/// type 0x00 was accepted; draft-21 Section 11.3.1 lists it as invalid
/// ```
///
...
///
/// ```text
/// type 0x16 was accepted; draft-21 Section 11.3.1 lists it as invalid
/// ```
#[test]
    ...
    for header_type in 0u8..=0x7F {
        ...
        if subgroup_type_is_valid(header_type) {
            let header = decoded.unwrap_or_else(|e| {
                panic!("type {header_type:#04x} is valid under draft-21 but was refused: {e:?}")
            });
            assert!(
            ...
            match decoded {
                Ok(_) => panic!(
                    "type {header_type:#04x} was accepted; \
                     draft-21 Section 11.3.1 lists it as invalid"
                ),
...
/// Every datagram Type draft-21 Section 11.2.1 names as invalid because it
...
/// bit. Non-empty on purpose: draft-21 Section 11.2.1 makes a datagram that
...
/// Whether draft-21 Section 11.2.1 permits `datagram_type`.
///
...
/// specified, and Section 11.3.1 states an explicit "128 or greater" condition
/// in its place.
...
enum Refusal {
    /// A Type no table assigns. Section 6.4.1 ends the session.
    Unknown,
    /// A Type inside a form the draft defines but on a list it calls invalid.
    /// Sections 11.3.1 and 11.2.1 end the session and name the code.
    NamedInvalid,
...
///
/// Every invalid value inside the one-byte space is one Section 11.3.1 names —
...
/// rest to Section 6.4.1's unknown-stream-type rule; the value set is the same
/// and the complaint is not. FETCH_HEADER is the one exception: Table 2 assigns
...
///
/// As above: Section 11.2.1's three conditions cover every invalid byte, so
...
/// The decoder accepts exactly the datagram Types draft-21 assigns.
///
...
///
/// ```text
/// type 0x10 was accepted; draft-21 Section 11.2.1 lists it as invalid
/// ```
///
/// Keeping the form check but dropping the STATUS-with-END_OF_GROUP arm:
///
/// ```text
/// type 0x22 was accepted; draft-21 Section 11.2.1 lists it as invalid
/// ```
#[test]
fn datagram_types_outside_the_drafts_lists_are_refused() {
    for datagram_type in 0u8..=0x7F {
        ...
        if datagram_type_is_valid(datagram_type) {
            let header = decoded.unwrap_or_else(|e| {
                panic!("type {datagram_type:#04x} is valid under draft-21 but was refused: {e:?}")
            });
            assert!(
                !cursor.has_remaining(),
            ...
            match decoded {
                Ok(_) => panic!(
                    "type {datagram_type:#04x} was accepted; \
                     draft-21 Section 11.2.1 lists it as invalid"
                ),
                Err(error) => assert_refusal(
...
/// half of what Table 2 assigns lives.
///
/// Table 2 gives four Types and two are several bytes wide: SETUP at 0x2F00 and
...
/// case left for Section 6.4.1, and it is what keeps
...
///
/// ```text
/// SETUP is a Type Table 2 assigns, so a subgroup reader must refuse it without
...
fn a_type_wider_than_one_byte_is_judged_by_its_value() {
    ...
    for (name, ty) in [("SETUP", 0x2F00u64), ("PADDING", PADDING_STREAM_TYPE)] {
        ...
        match SubgroupHeader::decode(&mut &wire[..]) {
            ...
            Err(e) => assert!(
                ...
                "{name} is a Type Table 2 assigns, so a subgroup reader must refuse it without \
    ...
    match SubgroupHeader::decode(&mut &unassigned[..]) {
        Ok(h) => panic!("0x0100 is not a Type draft-21 assigns, but decode accepted {h:?}"),
        Err(e) => {
            // The one place [`Refusal::Unknown`] is still reachable on
            // draft-21: inside the byte space every invalid value is one the
            // draft names, so Section 6.4.1's rule has only this left to answer.
    ...
    // A value inside the byte space but at 128 or above is Section 11.3.1's
    // third condition, not Section 6.4.1's rule, even though it arrives in two
    ...
    match SubgroupHeader::decode(&mut &over_127[..]) {
        ...
        Err(e) => assert!(
            ...
            "Type Flags 144 is Section 11.3.1's third condition, got {e:?}"
        ),
    }
}
...
///
/// **This is a decision, and draft-21 does not settle it.** Section 8.1
/// permits non-minimal encodings; Section 11.3.1 words its third condition as
...
fn a_non_minimal_type_flags_value_is_accepted_on_receive() {
    ...
    // The same, on a datagram. Section 11.3.1's "more than a one-byte
    // variable-length integer encoding" clause has no counterpart in Section
    // 11.2.1 at all, so the reading there is if anything plainer.
...
/// Draft-21 Section 16.9, Table 16, "Payload" column, transcribed. A second
...
fn a_framing_only_read_reports_the_registrys_payload_column() {
    ...
    for &(code, permission) in PAYLOAD_COLUMN {
        let status = ObjectStatus::from_u64(code)
            .unwrap_or_else(|| panic!("draft-21 assigns status {code:#x}"));
...
/// and draft-21 has nothing to say about it.
///
/// # What this catches, observed by making the change and running it
///
...
/// Serialization Flags values draft-21 Section 11.4.1 leaves undefined: at or
...
/// A Serialization Flags value draft-21 does not define is refused, on both
/// sides.
///
...
/// claims to be one.
///
/// Draft-21 Section 11.4.1.2 gives them different meanings a subscriber must
...
fn the_three_end_of_range_indicators_are_distinguished() {
    ...
    assert_eq!(END_OF_RANGE_FLAGS.len(), 3, "draft-21 Table 7 assigns three End of Range values");
...
///
/// **This is a decision, and draft-21 does not settle it.** Section 11.4.1.2
...
/// 11.4.1.1 unchanged, because a marker's flags are literally the ordinary
...
fn an_end_of_range_marker_resolves_against_the_object_before_it() {
    use moqtap_codec::draft21::data_stream::{FetchObjectReader, GroupOrder};
    ...
    for &(flags, marker) in END_OF_RANGE_FLAGS {
        ...
        assert_eq!(seen.subgroup_id, None, "Section 11.4.1.2: a marker carries no Subgroup ID");
        assert_eq!(
    ...
    // Section 11.4.1.1's first-Object rule makes both fields absolute — which
    // is where the two readings of Section 11.4.1.2 agree.
    let mut wire = Vec::new();
    FetchObjectHeader {
