...
fn run_message_vectors(relative_path: &str) {
    ...
    for vector in &file.vectors {
        if let Some(expected_decoded) = &vector.decoded {
            ...
            let actual_json = test_vectors::draft08_json::message_to_json(&msg);
            assert_eq!(
                actual_json,
                *expected_decoded,
...
macro_rules! d08_test {
    ($name:ident, $file:expr) => {
        #[test]
        fn $name() {
            run_message_vectors(concat!("transport/draft08/codec/messages/", $file));
        }
    };
}

d08_test!(d08_client_setup, "client-setup.json");
d08_test!(d08_server_setup, "server-setup.json");
d08_test!(d08_goaway, "goaway.json");
d08_test!(d08_max_subscribe_id, "max-subscribe-id.json");
d08_test!(d08_subscribes_blocked, "subscribes-blocked.json");
d08_test!(d08_subscribe, "subscribe.json");
d08_test!(d08_subscribe_ok, "subscribe-ok.json");
d08_test!(d08_subscribe_error, "subscribe-error.json");
d08_test!(d08_subscribe_update, "subscribe-update.json");
d08_test!(d08_subscribe_done, "subscribe-done.json");
d08_test!(d08_unsubscribe, "unsubscribe.json");
d08_test!(d08_announce, "announce.json");
d08_test!(d08_announce_ok, "announce-ok.json");
d08_test!(d08_announce_error, "announce-error.json");
d08_test!(d08_announce_cancel, "announce-cancel.json");
d08_test!(d08_unannounce, "unannounce.json");
d08_test!(d08_subscribe_announces, "subscribe-announces.json");
d08_test!(d08_subscribe_announces_ok, "subscribe-announces-ok.json");
d08_test!(d08_subscribe_announces_error, "subscribe-announces-error.json");
d08_test!(d08_unsubscribe_announces, "unsubscribe-announces.json");
d08_test!(d08_track_status_request, "track-status-request.json");
d08_test!(d08_track_status, "track-status.json");
d08_test!(d08_fetch, "fetch.json");
d08_test!(d08_fetch_ok, "fetch-ok.json");
d08_test!(d08_fetch_error, "fetch-error.json");
d08_test!(d08_fetch_cancel, "fetch-cancel.json");
d08_test!(d08_unknown_type, "unknown-type.json");
...
fn run_datagram_vectors(relative_path: &str) {
    ...
    for vector in &file.vectors {
        if let Some(expected) = &vector.decoded {
            ...
            assert_eq!(
                header.extension_count.into_inner().to_string(),
                js_str(expected, "extension_count"),
                "[{}] extension_count mismatch",
                vector.id
            );
            ...
            assert_eq!(
                header.payload_length.into_inner().to_string(),
                js_str(expected, "payload_length"),
                "[{}] payload_length mismatch",
                vector.id
            );

            let plen = header.payload_length.into_inner() as usize;
            assert!(cursor.remaining() >= plen, "[{}] datagram payload underrun", vector.id);
            let mut payload = vec![0u8; plen];
            cursor.copy_to_slice(&mut payload);
            assert_eq!(
                hex::encode(&payload),
...
#[test]
fn d08_data_stream_subgroup() {
    run_subgroup_vectors("transport/draft08/codec/data-streams/subgroup.json");
}

#[test]
fn d08_data_stream_datagram() {
    run_datagram_vectors("transport/draft08/codec/data-streams/datagram.json");
}

#[test]
fn d08_data_stream_datagram_status() {
    run_datagram_status_vectors("transport/draft08/codec/data-streams/datagram-status.json");
}

#[test]
fn d08_data_stream_fetch() {
    run_fetch_vectors("transport/draft08/codec/data-streams/fetch-header.json");
}
