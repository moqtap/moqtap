...
//! drafts 15 through 20 qualify with "as long as the combination of Token Type
//! and Token Value are unique after resolving any aliases" — and it is exempt
//! on every draft from 11 through 20. It is numbered 0x01 on draft-11 and 0x03
//! on drafts 12 through 20.
//!
...
//!
//! Only the draft-17, draft-18, draft-19 and draft-20 modules exercise the
//! exemption directly. The drafts 11 through 16 modules build their repeats out of
...
// The four items in this file that every per-draft module below shares. Each
// module is behind a `#[cfg(feature = "draftNN")]`, so the zero-draft row -
// `--no-default-features --all-targets` - compiles this file with none of them
// and none of these is reached. That row exists to prove the crate still builds
// with no draft at all; the `allow`s reach exactly as far as it does, because
// under any real feature set every one of these has a caller.
#[allow(dead_code)]
