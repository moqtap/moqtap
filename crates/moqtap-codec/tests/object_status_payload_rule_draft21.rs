#![cfg(feature = "draft21")]
//! Draft-21's Object Status registry decides, per status, whether an Object may
//! carry a payload — and this crate's draft-21 encoders answer from that
//! registry rather than from a payload length.
//!
//! Draft-21 Section 11.1.2 states the rule as a deferral: an Object has an
//! empty payload unless its Object Status is registered as permitting one. The
//! registry it defers to is Section 16.9, Table 16, whose rows carry a
...
//!
//! The three rows assigned today make draft-21's registry rule and draft-18's
...
//! exercised draft-21 would pass just as happily against an implementation that
...
//! Draft-21 refuses it, because the registry rules on the pair. Draft-18 writes
...
/// genuine extraction failure says. Draft-21's Object Status registry has no
...
/// distinction is the whole difference between draft-18 and draft-21 for these
/// three rows.
...
mod d20 {
    pub use moqtap_codec::draft21::data_stream::{
        DatagramHeader, SubgroupHeader, SubgroupObject, SubgroupObjectReader,
    };
    pub use moqtap_codec::draft21::types::ObjectStatus;
}
...
/// The registry, not a payload length, decides which draft-21 statuses may
...
///
/// ```text
/// thread 'the_registry_decides_which_draft21_statuses_may_carry_a_payload'
/// panicked at crates\moqtap-codec\tests\object_status_payload_rule.rs:
/// draft-21 status 0x3 is registered "Payload: No", so write_object must refuse
...
/// thread 'the_registry_decides_which_draft21_statuses_may_carry_a_payload'
/// panicked at crates\moqtap-codec\tests\object_status_payload_rule.rs:
/// assertion `left == right` failed: draft-21 Table 16 marks 0x3 "Payload: no",
/// ObjectStatus::permits_payload disagrees
///   left: true
///  right: false
/// ```
#[test]
fn the_registry_decides_which_draft21_statuses_may_carry_a_payload() {
    let reg = object_status_registry(19);
    let column = payload_column(&reg, 19);

    // Draft-21 states this rule per row, in a column of its own. The rest of
    // this test reads that column, so a re-extraction that lost it would leave
    // the sweep asserting draft-18's rule under draft-21's name.
    assert_eq!(
        reg["payload_column"],
        Value::Bool(true),
        "draft-21 Object Status was extracted without a Payload column"
    );
    for (code, source) in payload_sources(&reg) {
        assert_eq!(
            source, "payload-column",
            "draft-21 status {code:#x} takes its payload rule from {source}, not the registry"
        );
    }
    ...
    assert_eq!(
        ...
        "draft-21 assigns {spec_codes:?}, this crate {codec_codes:?}"
    );

    for (&code, &permits) in &column {
        let status = d20::ObjectStatus::from_u64(code)
            .unwrap_or_else(|| panic!("draft-21 assigns {code:#x}, this crate does not decode it"));

        // The crate's own answer for this row.
        assert_eq!(
            status.permits_payload(),
            permits,
            "draft-21 Table 16 marks {code:#x} \"Payload: {}\", \
             ObjectStatus::permits_payload disagrees",
        ...
        if permits {
            let bytes = carrying.unwrap_or_else(|(e, _)| {
                panic!("draft-21 status {code:#x} is registered \"Payload: Yes\", {e} anyway")
            });
            ...
            let (err, written) = match carrying {
                Ok(bytes) => panic!(
                    "draft-21 status {code:#x} is registered \"Payload: No\", so write_object \
            ...
            assert!(
                ...
                "draft-21 status {code:#x} with a payload was refused as {err}, \
...
/// header needs both:
///
/// - The framing. Draft-21 Section 11.2.1 puts the Object Status field exactly
...
/// - The registry. Section 16.9, Table 16 says whether an Object at this
...
#[test]
fn a_draft21_datagram_keeps_the_framing_and_the_registry_apart() {
    let column = payload_column(&object_status_registry(19), 19);
    let mut framing_and_registry_disagreed = false;

    for (&code, &permits) in &column {
        let status = d20::ObjectStatus::from_u64(code).expect("draft-21 decodes its own status");
...
/// non-zero codes, and its encoder lets the payload length decide; draft-21
...
/// difference in behaviour: draft-21 refuses the Object that states a forbidden
...
/// draft-21's registry backwards over a draft that has none, and it has to be
/// argued here rather than arrived at by sharing one conversion across every
/// draft.
///
/// # Ablation
///
/// Giving draft-18's `write_object` draft-21's refusal — the shared-helper
/// mistake this exists to catch — erases the difference:
///
/// ```text
/// thread 'draft18_reads_the_same_rule_off_the_payload_length_and_draft21_off_the_registry'
/// panicked at crates\moqtap-codec\tests\object_status_payload_rule.rs:
...
#[cfg(feature = "draft18")]
#[test]
fn draft18_reads_the_same_rule_off_the_payload_length_and_draft21_off_the_registry() {
    ...
    for (&code, &permits) in &column18 {
        ...
        let status19 = d20::ObjectStatus::from_u64(code).expect("draft-21 decodes the same status");

        // Draft-21: the registry rules on the pair, and refuses it.
        let refused = write_19(&object_19(status19, b"abc"));
        assert!(
            refused.is_err(),
            "draft-21 accepted a payload under {status19:?}, which its registry forbids one"
        );
...
// compiled draft in one test, draft-21 among them, because the dispatch layer
