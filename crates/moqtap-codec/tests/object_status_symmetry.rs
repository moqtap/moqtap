...
//! # The shapes covered
//!
//! Drafts 15-21 carry an Object Status in exactly three places, and all three
//! are here:
//!
...
//! their own (`FetchObject`, `FetchObjectHeader`), but drafts 15-21 have no
...
//! Does Not Exist) and drafts 16-21 do not. So the same sweep body requires
...
//! swept over the wide values too; the drafts 17-21 datagram status is a single
...
//! draft-17 with the MoQT varint of draft-17 Section 1.4.1, and drafts 18-21
...
//! the datagram status as a varint, drafts 17-21 as a single byte — so both are
//! ablated below.
//!
...
/// Frame `code` as the single octet the drafts 17-21 datagram status is, or
...
subgroup_row!(draft21_subgroup_status, "draft21", draft21, put_moqt18, take_moqt18);
...
/// drafts 17-21 dropped the property field from the type and hold the status in
...
datagram_row!(byte_status draft21_datagram_status, "draft21", draft21);
