...
//! The committed draft-21 corpus, run through this codec in both directions.
//!
//! Every file under `test-vectors/transport/draft21/codec/` is read by one of
//! the four runners below, and each runner counts the vectors it asserted on.
//! [`d21_every_committed_vector_is_run`] adds those counts up and compares the
...
//! draft-21 — `SUBSCRIPTION_ENDED` and `INVALID_JOINING_REQUEST_ID` were removed
//! along with the behaviour behind them — and both vectors expect a successful
//! decode anyway.
//!
//! That is what Section 13 requires: "Receipt of an unknown error code in any
...
//! `error_codes_draft21.rs`: `PublishDoneStatusCode::from_u64` and
    ...
    for vector in &file.vectors {
        if let Some(expected_decoded) = &vector.decoded {
            ...
            let actual_json = test_vectors::fields_json::to_json(
                &moqtap_codec::draft21::fields::message_fields(&msg),
            );
            assert_eq!(
                actual_json,
                *expected_decoded,
...
macro_rules! d21_test {
    ($name:ident, $file:expr) => {
        #[test]
        fn $name() {
            run_message_vectors(concat!("transport/draft21/codec/messages/", $file));
        }
    };
}

/// Every message file the draft-21 corpus carries, in the order the message
...
d21_test!(d21_setup, "setup.json");
d21_test!(d21_goaway, "goaway.json");
d21_test!(d21_subscribe, "subscribe.json");
d21_test!(d21_subscribe_ok, "subscribe-ok.json");
d21_test!(d21_request_update, "request-update.json");
d21_test!(d21_publish, "publish.json");
d21_test!(d21_publish_ok, "publish-ok.json");
d21_test!(d21_publish_state_notify, "publish-state-notify.json");
d21_test!(d21_publish_done, "publish-done.json");
d21_test!(d21_publish_namespace, "publish-namespace.json");
d21_test!(d21_publish_skipped, "publish-skipped.json");
d21_test!(d21_namespace, "namespace.json");
d21_test!(d21_namespace_done, "namespace-done.json");
d21_test!(d21_subscribe_namespace, "subscribe-namespace.json");
d21_test!(d21_subscribe_tracks, "subscribe-tracks.json");
d21_test!(d21_track_status, "track-status.json");
d21_test!(d21_request_ok, "request-ok.json");
d21_test!(d21_request_error, "request-error.json");
d21_test!(d21_fetch, "fetch.json");
d21_test!(d21_fetch_ok, "fetch-ok.json");
d21_test!(d21_unknown_type, "unknown-type.json");
...
fn run_subgroup_vectors(relative_path: &str) -> usize {
    ...
    for vector in &file.vectors {
        if let Some(expected) = &vector.decoded {
            ...
            let any_header = AnySubgroupHeader::Draft21(header.clone());
        ...
        if vector.error.is_some() {
            ...
            let failure = match SubgroupHeader::decode(&mut cursor) {
                ...
                Ok(header) => dispatch_check::drain_subgroup_objects(
                    &AnySubgroupHeader::Draft21(header),
                    &mut cursor,
                ),
            };
            match failure {
...
/// Decode a draft-21 fetch stream and check every object against the vectors.
///
...
/// each object resolves to are computed here, because draft-21 Section 11.4.1
...
/// as absolute; draft-21 Section 11.4.1.2 says only that "the Group ID and
...
fn run_fetch_vectors(relative_path: &str) -> usize {
    ...
    for vector in &file.vectors {
        if let Some(expected) = &vector.decoded {
            ...
            // test file and never executes the codec's own Section 11.4.1.1
...
#[test]
fn d21_data_stream_subgroup() {
    run_subgroup_vectors("transport/draft21/codec/data-streams/subgroup.json");
}

#[test]
fn d21_data_stream_datagram() {
    run_datagram_vectors("transport/draft21/codec/data-streams/datagram.json");
}
...
#[test]
fn d21_data_stream_fetch() {
    run_fetch_vectors("transport/draft21/codec/data-streams/fetch-header.json");
}
...
/// Every vector in the committed draft-21 corpus is asserted on by one of the
/// runners above.
///
...
///
/// ```text
/// transport/draft21/codec/messages/publish-state-notify.json is in the corpus
/// and no runner above opens it
/// ```
#[test]
fn d21_every_committed_vector_is_run() {
    use std::collections::BTreeSet;

    let root = vectors_dir().join("transport/draft21/codec");

    // What the corpus holds.
    let mut on_disk: BTreeSet<String> = BTreeSet::new();
    fn walk(dir: &std::path::Path, root: &std::path::Path, out: &mut BTreeSet<String>) {
        for entry in std::fs::read_dir(dir).expect("the draft-21 corpus directory") {
    ...
    for file in MESSAGE_FILES {
        run += run_message_vectors(&format!("transport/draft21/codec/messages/{file}"));
    }
    run += run_subgroup_vectors("transport/draft21/codec/data-streams/subgroup.json");
    run += run_datagram_vectors("transport/draft21/codec/data-streams/datagram.json");
    run += run_fetch_vectors("transport/draft21/codec/data-streams/fetch-header.json");
    ...
    let varint_path = vectors_dir().join("transport/draft21/codec/varint.json");
    ...
    assert_eq!(
        on_disk_count,
        324,
        "the committed draft-21 corpus is 324 vectors across 25 files; it now has \
         {on_disk_count} across {}",
        on_disk.len()
    );
    assert_eq!(on_disk.len(), 25, "the committed draft-21 corpus is 25 files");
}
