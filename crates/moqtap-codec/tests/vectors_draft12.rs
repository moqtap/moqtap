...
use moqtap_codec::draft12::data_stream::{
    DatagramHeader, DatagramStatusHeader, DatagramType, FetchHeader, FetchObjectHeader, StreamType,
    SubgroupHeader,
};
...
fn run_message_vectors(relative_path: &str) {
    ...
    for vector in &file.vectors {
        if let Some(expected_decoded) = &vector.decoded {
            ...
            let actual_json = test_vectors::draft12_json::message_to_json(&msg);
            assert_eq!(
                actual_json,
                *expected_decoded,
...
macro_rules! d12_test {
    ($name:ident, $file:expr) => {
        #[test]
        fn $name() {
            run_message_vectors(concat!("transport/draft12/codec/messages/", $file));
        }
    };
}

d12_test!(d12_client_setup, "client-setup.json");
d12_test!(d12_server_setup, "server-setup.json");
d12_test!(d12_goaway, "goaway.json");
d12_test!(d12_max_request_id, "max-request-id.json");
d12_test!(d12_requests_blocked, "requests-blocked.json");
d12_test!(d12_subscribe, "subscribe.json");
d12_test!(d12_subscribe_ok, "subscribe-ok.json");
d12_test!(d12_subscribe_error, "subscribe-error.json");
d12_test!(d12_subscribe_update, "subscribe-update.json");
d12_test!(d12_subscribe_done, "subscribe-done.json");
d12_test!(d12_unsubscribe, "unsubscribe.json");
d12_test!(d12_announce, "announce.json");
d12_test!(d12_announce_ok, "announce-ok.json");
d12_test!(d12_announce_error, "announce-error.json");
d12_test!(d12_announce_cancel, "announce-cancel.json");
d12_test!(d12_unannounce, "unannounce.json");
d12_test!(d12_subscribe_announces, "subscribe-announces.json");
d12_test!(d12_subscribe_announces_ok, "subscribe-announces-ok.json");
d12_test!(d12_subscribe_announces_error, "subscribe-announces-error.json");
d12_test!(d12_unsubscribe_announces, "unsubscribe-announces.json");
d12_test!(d12_track_status_request, "track-status-request.json");
d12_test!(d12_track_status, "track-status.json");
d12_test!(d12_fetch, "fetch.json");
d12_test!(d12_fetch_ok, "fetch-ok.json");
d12_test!(d12_fetch_error, "fetch-error.json");
d12_test!(d12_fetch_cancel, "fetch-cancel.json");
d12_test!(d12_publish, "publish.json");
d12_test!(d12_publish_ok, "publish-ok.json");
d12_test!(d12_publish_error, "publish-error.json");
d12_test!(d12_unknown_type, "unknown-type.json");
...
fn run_subgroup_vectors(relative_path: &str) {
    ...
    for vector in &file.vectors {
        if let Some(expected) = &vector.decoded {
            ...
            let header = SubgroupHeader::decode_with_type(st, &mut cursor)
            ...
            let any_header = AnySubgroupHeader::Draft12(header.clone());
            ...
            // Only some of these stream types put a Subgroup ID on the wire.
            // The first-object types put none there and take it from the first
            // Object instead, so the header field holds the zero the decoder
            // left; asserting on that would compare the runner's own input.
            // Each type is checked against whatever actually states its
            // Subgroup ID.
            //
            // Ablation: assigning the first Object's ID into the header and
            // asserting the header field, as this did before. Emptying this
            // draft's `subgroup-first-obj-id` of objects then passes, because
            // nothing is assigned and the decoder's zero is compared against a
            // Subgroup ID no Object states — `[subgroup-first-obj-id] a
            // first-object stream carrying no objects` is what it costs.
            if matches!(
                st,
                ...
                let first = objects.first().unwrap_or_else(|| {
                    panic!("[{}] a first-object stream carrying no objects", vector.id)
                });
                assert_eq!(
                    first.object_id.to_string(),
                    js_str(expected, "subgroup_id"),
                    "[{}] subgroup_id (from the first object) mismatch",
                    vector.id
                );
            } else {
                assert_eq!(
        ...
        if vector.error.is_some() {
            ...
            let failure = match SubgroupHeader::decode_stream(&mut cursor) {
                ...
                Ok(header) => dispatch_check::drain_subgroup_objects(
                    &AnySubgroupHeader::Draft12(header),
                    &mut cursor,
                ),
            };
            match failure {
...
#[test]
fn d12_data_stream_subgroup() {
    run_subgroup_vectors("transport/draft12/codec/data-streams/subgroup.json");
}

#[test]
fn d12_data_stream_datagram() {
    run_datagram_vectors("transport/draft12/codec/data-streams/datagram.json");
}

#[test]
fn d12_data_stream_fetch() {
    run_fetch_vectors("transport/draft12/codec/data-streams/fetch-header.json");
}
