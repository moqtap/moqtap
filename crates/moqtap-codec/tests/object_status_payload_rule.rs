#![cfg(feature = "draft19")]
//! Draft-19's Object Status registry decides, per status, whether an Object may
//! carry a payload — and this crate's draft-19 encoders answer from that
//! registry rather than from a payload length.
//!
//! Draft-19 Section 11.2.1.1 states the rule as a deferral: an Object has an
...
//!
//! The three rows assigned today make draft-19's registry rule and draft-18's
...
//! exercised draft-19 would pass just as happily against an implementation that
...
//! Draft-19 refuses it, because the registry rules on the pair. Draft-18 writes
...
/// genuine extraction failure says. Draft-19's Object Status registry has no
...
/// distinction is the whole difference between draft-18 and draft-19 for these
/// three rows.
...
mod d19 {
    pub use moqtap_codec::draft19::data_stream::{
        DatagramHeader, SubgroupHeader, SubgroupObject, SubgroupObjectReader,
    };
    pub use moqtap_codec::draft19::types::ObjectStatus;
}
...
fn subgroup_header_19() -> d19::SubgroupHeader {
    d19::SubgroupHeader {
        header_type: 0x10,
...
fn object_19(status: d19::ObjectStatus, payload: &[u8]) -> d19::SubgroupObject {
    d19::SubgroupObject {
...
fn write_19(object: &d19::SubgroupObject) -> Result<Vec<u8>, (CodecError, Vec<u8>)> {
    let header = subgroup_header_19();
    let mut writer = d19::SubgroupObjectReader::new(&header);
    let mut buf = Vec::new();
...
/// consumed exactly.
fn read_19(bytes: &[u8]) -> d19::SubgroupObject {
    let header = subgroup_header_19();
    let mut reader = d19::SubgroupObjectReader::new(&header);
...
/// The registry, not a payload length, decides which draft-19 statuses may
...
/// ```
#[test]
fn the_registry_decides_which_draft19_statuses_may_carry_a_payload() {
    let reg = object_status_registry(19);
    let column = payload_column(&reg, 19);

    // Draft-19 states this rule per row, in a column of its own. The rest of
    // this test reads that column, so a re-extraction that lost it would leave
    // the sweep asserting draft-18's rule under draft-19's name.
    assert_eq!(
        reg["payload_column"],
        Value::Bool(true),
        "draft-19 Object Status was extracted without a Payload column"
    );
    for (code, source) in payload_sources(&reg) {
        assert_eq!(
            source, "payload-column",
            "draft-19 status {code:#x} takes its payload rule from {source}, not the registry"
        );
    }
    ...
    // file.
    let codec_codes: Vec<u64> = d19::ObjectStatus::ALL.iter().map(|s| s.as_u64()).collect();
    let spec_codes: Vec<u64> = column.keys().copied().collect();
    assert_eq!(
        codec_codes, spec_codes,
        "draft-19 assigns {spec_codes:?}, this crate {codec_codes:?}"
    );

    for (&code, &permits) in &column {
        let status = d19::ObjectStatus::from_u64(code)
            .unwrap_or_else(|| panic!("draft-19 assigns {code:#x}, this crate does not decode it"));

        // The crate's own answer for this row.
        assert_eq!(
            status.permits_payload(),
            permits,
            "draft-19 Table 16 marks {code:#x} \"Payload: {}\", \
             ObjectStatus::permits_payload disagrees",
        ...
        if permits {
            let bytes = carrying.unwrap_or_else(|(e, _)| {
                panic!("draft-19 status {code:#x} is registered \"Payload: Yes\", {e} anyway")
            });
            ...
            let (err, written) = match carrying {
                Ok(bytes) => panic!(
                    "draft-19 status {code:#x} is registered \"Payload: No\", so write_object \
            ...
            assert!(
                ...
                "draft-19 status {code:#x} with a payload was refused as {err}, \
...
/// header needs both:
///
/// - The framing. Draft-19 Section 11.3.1 puts the Object Status field exactly
...
#[test]
fn a_draft19_datagram_keeps_the_framing_and_the_registry_apart() {
    let column = payload_column(&object_status_registry(19), 19);
    let mut framing_and_registry_disagreed = false;

    for (&code, &permits) in &column {
        let status = d19::ObjectStatus::from_u64(code).expect("draft-19 decodes its own status");
        ...
        let decoded = d19::DatagramHeader::decode(&mut &bytes[..])
    ...
    header.encode(&mut bytes);
    let decoded =
        d19::DatagramHeader::decode(&mut &bytes[..]).expect("a payload datagram did not decode");
...
/// non-zero codes, and its encoder lets the payload length decide; draft-19
...
/// difference in behaviour: draft-19 refuses the Object that states a forbidden
...
/// draft-19's registry backwards over a draft that has none, and it has to be
/// argued here rather than arrived at by sharing one conversion across every
/// draft.
///
/// # Ablation
///
/// Giving draft-18's `write_object` draft-19's refusal — the shared-helper
...
#[cfg(feature = "draft18")]
#[test]
fn draft18_reads_the_same_rule_off_the_payload_length_and_draft19_off_the_registry() {
    ...
    for (&code, &permits) in &column18 {
        ...
        let status19 = d19::ObjectStatus::from_u64(code).expect("draft-19 decodes the same status");

        // Draft-19: the registry rules on the pair, and refuses it.
        let refused = write_19(&object_19(status19, b"abc"));
        assert!(
            refused.is_err(),
            "draft-19 accepted a payload under {status19:?}, which its registry forbids one"
        );
...
/// The shared dispatch layer must not contradict the per-draft one about
...
/// that they do.
///
/// Restoring the blanket rule (`(Some(_), false) => Err(InvalidField)`) fails
/// this test with:
///
/// ```text
/// ---- the_shared_writer_lets_a_normal_object_carry_a_payload stdout ----
/// draft Draft17 refused a payload on Normal, which every draft 07-19 permits:
/// InvalidField
/// ```
#[test]
fn the_shared_writer_lets_a_normal_object_carry_a_payload() {
    ...
    // `UnsupportedDraft` for one left out by a feature flag, which would fail
    // this test for a reason that has nothing to do with the rule it gates.
    ...
    for &draft in drafts {
        let write = |status: Option<u64>| -> Result<Vec<u8>, CodecError> {
            let mut cursor = &header_bytes[..];
            let header = AnySubgroupHeader::decode(draft, &mut cursor)
                .unwrap_or_else(|e| panic!("draft {draft:?} header: {e}"));
            let mut writer = AnySubgroupObjectWriter::new(&header)
                .unwrap_or_else(|e| panic!("draft {draft:?} writer: {e}"));
        ...
        let stated = write(Some(0)).unwrap_or_else(|e| {
            panic!("draft {draft:?} refused a payload on Normal, which every draft 07-19 permits: {e:?}")
        });
        let elided = write(None).unwrap_or_else(|e| {
            panic!("draft {draft:?} refused a payload with no status stated: {e:?}")
        });

        assert_eq!(
            stated, elided,
            "draft {draft:?}: naming Normal changed the bytes, so it was not the status the \
             encoding elides"
        );

        // The contrast that keeps this from being "the writer accepts
        // anything": a status that is not Normal beside a payload is still
        // refused, because that pair has no encoding.
        assert!(
            write(Some(3)).is_err(),
            "draft {draft:?} accepted End of Group beside a payload; the wire cannot state both"
        );
    }
}
