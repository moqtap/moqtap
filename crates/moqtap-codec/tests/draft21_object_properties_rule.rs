#![cfg(feature = "draft21")]
//! Draft-21 forbids Object Properties on an Object whose status is not Normal,
//! and this crate reports that rather than refusing to parse it.
//!
//! Section 11.1.3: "Any Object with status Normal can have properties
//! (Section 8.4). If an endpoint receives properties on an Object with status
//! that is not Normal, it MUST close the session with a PROTOCOL_VIOLATION."
//! Section 11.2.1 builds the datagram's Properties field out of that same
...
fn properties_on_a_non_normal_status_decode_and_are_reported_as_forbidden() {
    ...
    assert!(
        ...
        "properties on End of Group are forbidden by draft-21 Section 11.1.3 and must be \
         reported as such"
    );
}
