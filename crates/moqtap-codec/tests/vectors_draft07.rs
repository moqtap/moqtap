...
use moqtap_codec::draft07::data_stream::{
    DatagramHeader, FetchHeader, FetchObjectHeader, StreamType, SubgroupHeader,
};
    ...
    for vector in &file.vectors {
        if let Some(expected_decoded) = &vector.decoded {
            ...
            let actual_json = test_vectors::fields_json::to_json(
                &moqtap_codec::draft07::fields::message_fields(&msg),
            );
            assert_eq!(
                actual_json,
                *expected_decoded,
...
fn run_subgroup_vectors(relative_path: &str) {
    ...
    for vector in &file.vectors {
        if let Some(expected) = &vector.decoded {
            ...
            let any_header = AnySubgroupHeader::Draft07(header.clone());
        ...
        if vector.error.is_some() {
            ...
            let failure = match SubgroupHeader::decode_stream(&mut cursor) {
                ...
                Ok(header) => dispatch_check::drain_subgroup_objects(
                    &AnySubgroupHeader::Draft07(header),
                    &mut cursor,
                ),
            };
            match failure {
...
#[test]
fn d07_data_stream_subgroup() {
    run_subgroup_vectors("transport/draft07/codec/data-streams/subgroup.json");
}

#[test]
fn d07_data_stream_datagram() {
    run_datagram_vectors("transport/draft07/codec/data-streams/datagram.json");
}

#[test]
fn d07_data_stream_fetch() {
    run_fetch_vectors("transport/draft07/codec/data-streams/fetch-header.json");
}
