#![cfg(feature = "draft21")]
//! What draft-21's encoders emit, its decoders accept — and what they cannot
...
//!
//! Three of draft-21's four uint8-valued Message Parameters restrict their
//! range, and the draft answers a value outside it with a session close:
//! GROUP_ORDER (0x22) allows only Ascending and Descending (Section 9.20.9),
//! FORWARD (0x10) only 0 and 1 (Section 9.20.19), and INCLUDE_PROPERTIES
//! (0x35), new in draft-20, only 0 and 1 (Section 9.20.22). Checking that on decode alone
...
use moqtap_codec::draft21::data_stream::DatagramHeader;
use moqtap_codec::draft21::message::{ControlMessage, Subscribe};
use moqtap_codec::draft21::types::ObjectStatus;
...
/// (Section 9.20.8) and so must survive every value a byte can hold.
const SUBSCRIBER_PRIORITY: u64 = 0x20;
/// INCLUDE_PROPERTIES, new in draft-20 and the third restricted uint8
/// (Section 9.20.22): 0 or 1, and nothing else.
...
/// thread '...' panicked at crates\moqtap-codec\tests\draft21_encode_decode_symmetry.rs:
...
/// `Type Delta` of 0.
///
/// **Two rules that interact, and draft-21 does not say how.** Section 3.3.2
...
/// REQUEST_UPDATE" — while Section 9.20 requires parameters to be "serialized in
...
///
/// ```text
/// a second SUBGROUP_FILTER must encode; Section 3.3.2 permits it:
...
fn a_range_filter_may_repeat_and_an_ordinary_parameter_may_not() {
    ...
    // Section 9.20.5 grants it nothing.
