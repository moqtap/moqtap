...
fn d15_param_name(key: u64) -> Option<&'static str> {
    // Union of SETUP and non-SETUP parameter names. Ambiguous keys (e.g. 0x02)
    // are disambiguated by the caller (see kvp_to_json_d15_setup / kvp_to_json_d15_msg).
    match key {
        0x01 => Some("path"),
    ...
    let obj = match msg {
        ...
        ControlMessage::Fetch(m) => {
            ...
            match &m.fetch_payload {
                moqtap_codec::draft15::message::FetchPayload::Standalone {
                    track_namespace,
                    track_name,
                    start_group,
                    start_object,
                ...
                moqtap_codec::draft15::message::FetchPayload::Joining {
                    joining_request_id,
                    joining_start,
                } => {
