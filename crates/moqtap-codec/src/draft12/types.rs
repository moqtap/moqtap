...
/// Object status values, from MoQ Transport draft-12 Section 9.2.1.1
/// "Object Status".
///
/// Draft-12 assigns 0x0, 0x1, 0x3 and 0x4, the same four as draft-11. Of every
/// other value the section says: "Any other value SHOULD be treated as a
/// protocol error and terminate the session with a Protocol Violation".
/// [`ObjectStatus::from_u64`] answers `None` for 0x2, for 0x5, and for
/// everything else the draft leaves unassigned.
...
impl ObjectStatus {
    /// Every status draft-12 assigns, in ascending wire order.
    ///
    ...
    /// Convert a raw u64 to an `ObjectStatus`, or `None` if draft-12 does not
    /// assign that value.
