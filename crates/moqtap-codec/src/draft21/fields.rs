use crate::draft21::message::ControlMessage;
...
// Draft-21 known parameter types and their encodings. Two rows are new:
...
// Draft-21 setup option names — unchanged from draft-19.
...
/// Render a draft-21 Range Filter parameter value: SetID (u8), an optional
...
/// Section 5.1.3 with proper figures in Section 8.6 and changed no codepoint,
/// field order or delta arithmetic.
///
/// # The parse belongs to [`crate::range_filter`], not here
///
/// Draft-21's parameter table gives 0x25-0x29 `LengthPrefixed`, which stores
...
/// Draft-21 answers both with REQUEST_ERROR rather than a session close, so
...
/// Decode a draft-21 LOCATION_FILTER value.
///
...
/// comes from how many `vi64` fields the value holds instead (Section 9.20.10).
...
fn decode_location_filter(bytes: &[u8]) -> Value {
    ...
    let fields = crate::draft21::message::decode_location_filter(bytes)
...
/// Decode a draft-21 FILL_PARAMETERS value: a nested parameter block in its own
...
fn decode_fill_parameters(bytes: &[u8]) -> Value {
    let nested = crate::draft21::message::decode_fill_parameters(bytes)
        .expect("the decoder accepted this parameter, so its value parses");
...
fn params_to_json(params: &[KeyValuePair]) -> Value {
    crate::fields::kvp_entries(params, |key, value| {
        let Some(name) = d20_param_name(key) else {
            return (None, None);
        };
        let rendered = match (value, key) {
            (KvpValue::Bytes(b), 0x21) => decode_location_filter(b),
            (KvpValue::Bytes(b), 0x23) => decode_fill_parameters(b),
            ...
            (KvpValue::Bytes(b), _) if name == "authorization_token" => auth_token_to_json_d20(b),
...
pub(crate) fn options_to_json(options: &[KeyValuePair]) -> Value {
    crate::fields::kvp_entries(options, |key, value| {
        ...
        let rendered = match value {
            ...
            KvpValue::Bytes(b) if name == "authorization_token" => auth_token_to_json_d20(b),
...
pub fn message_fields(msg: &ControlMessage) -> Map {
    let obj = match msg {
        ...
        // Draft-21's FETCH is byte-identical to SUBSCRIBE apart from its type
