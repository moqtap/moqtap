...
//!
//! One enum per registry in the IANA Considerations section (Section 16.11). The
...
//! (Section 13). That is a range rather than an assignment, so it gets no
...
//!   Subscription." Joining fetches went with the FETCH rewrite (Section 9.11).
//! * `SUBSCRIPTION_ENDED` (PUBLISH_DONE `0x3`), draft-19: "The publisher reached
//!   the end of an associated location filter." Draft-21 Section 3.3.1 removed
...
/// Session termination error codes (draft-21 Section 16.11.1).
///
...
/// definitions are in Section 12.2 of the draft. Note that this draft assigns no code point 0x7,
...
///
/// The registry also reserves the code points `0x7f * N + 0x9D` for greasing (draft-21 Section
/// 13). That is a range rather than an assignment, so it has no variant here and `from_u64`
...
pub enum SessionErrorCode {
    ...
    /// significant bit for the sender, or a duplicate Request ID. See Section 6.4.2.1.
    ...
    /// `MALFORMED_PATH` — The PATH parameter does not conform to the rules in Section 9.1.2.
    MalformedPath = 0x9,

    /// `GOAWAY_TIMEOUT` — The session was closed because the peer took too long to close the
    /// session in response to a GOAWAY (Section 9.2) message. See session migration (Section
    /// 6.6.1).
    GoawayTimeout = 0x10,
    ...
    /// `AUTH_TOKEN_CACHE_OVERFLOW` — The Session limit Section 9.1.3 of the size of all
    ...
    /// `EXPIRED_AUTH_TOKEN` — Authorization token has expired (Section 8.9).
    ...
    /// per-stream limit communicated via the MAX_REQUEST_UPDATES Setup Option (Section 9.1.7).
    TooManyRequestUpdates = 0x1B,
}

/// REQUEST_ERROR codes (draft-21 Section 16.11.2).
///
...
///
/// Section 9.4 states that REQUEST_ERROR is sent in response to any request: SUBSCRIBE, FETCH,
/// PUBLISH, SUBSCRIBE_NAMESPACE, SUBSCRIBE_TRACKS, PUBLISH_NAMESPACE, TRACK_STATUS and
/// REQUEST_UPDATE. The per-code definitions are in Section 12.3, which the registry table cites
/// as Section 9.4.
///
...
pub enum RequestErrorCode {
    ...
    /// 3.6) that the endpoint does not understand.
...
/// PUBLISH_DONE codes (draft-21 Section 16.11.3).
///
/// Carried in the Status Code field of PUBLISH_DONE. The per-code definitions are in Section
/// 12.4, beside the other three registries' — draft-21 collected all four under Error Handling,
/// where draft-20 kept each one beside the message or the session rule it belonged to.
///
...
pub enum PublishDoneStatusCode {
    ...
    /// `UPDATE_FAILED` — REQUEST_UPDATE failed on this subscription (see Section 9.5).
    UpdateFailed = 0x8,
...
/// Stream reset error codes (draft-21 Section 16.11.4).
///
/// Section 12.5 says the application SHOULD use a relevant code from this registry when
...
/// data streams; draft-21 keeps both the name and the wider scope, so this enum
/// takes that title.
///
/// The registry also reserves the code points `0x7f * N + 0x9D` for greasing (draft-21 Section
/// 13). That is a range rather than an assignment, so it has no variant here and `from_u64`
...
pub enum StreamResetErrorCode {
    ...
    /// `DELIVERY_TIMEOUT` — A delivery timeout (Section 5.2) was exceeded for this stream.
    DeliveryTimeout = 0x2,
    ...
    /// limits and is being terminated (see Section 5.2).
    TooFarBehind = 0x5,
...
impl SessionErrorCode {
    /// Every Session Error Code draft-21 assigns, in ascending wire order.
    ///
...
impl RequestErrorCode {
    /// Every Request Error Code draft-21 assigns, in ascending wire order.
    ///
    /// This is the set [`Self::from_u64`] accepts, written out so that it can
    /// be enumerated: nothing can iterate an enum's variants, so a caller that
    /// wants the registry has to be handed it. Writing it down is also what
...
impl PublishDoneStatusCode {
    /// Every PUBLISH_DONE Status Code draft-21 assigns, in ascending wire order.
    ///
    /// This is the set [`Self::from_u64`] accepts, written out so that it can
    /// be enumerated: nothing can iterate an enum's variants, so a caller that
    /// wants the registry has to be handed it. Writing it down is also what
...
impl StreamResetErrorCode {
    /// Every Stream Reset Error Code draft-21 assigns, in ascending wire order.
    ///
    /// This is the set [`Self::from_u64`] accepts, written out so that it can
    /// be enumerated: nothing can iterate an enum's variants, so a caller that
    /// wants the registry has to be handed it. Writing it down is also what
