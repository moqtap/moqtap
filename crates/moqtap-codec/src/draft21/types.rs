...
//!
//! Object Status is present only on subscription-delivered Objects. Draft-21
//! Section 11.1.2 is unchanged on this: a fetch stream's objects carry no
//! status field at all.

/// Whether an Object carrying a given status is permitted a non-empty payload:
/// the "Payload" column of the Object Status registry, MoQ Transport draft-21
/// Section 16.9, Table 16.
///
/// Draft-21 Section 11.1.2 phrases the rule as "An Object MUST have an empty
/// payload unless its Object Status value is registered as permitting a
/// payload", and Section 16.9 adds that each new registration "MUST indicate
...
/// Object status values, from MoQ Transport draft-21 Section 11.1.2
/// "Object Status", with the same three code points listed in the IANA Object
/// Status registry at Section 16.9, Table 16.
///
...
impl ObjectStatus {
    /// Every status draft-21 assigns, in ascending wire order.
    ///
    ...
    /// Convert a raw u64 to an `ObjectStatus`, or `None` if draft-21 does not
    /// assign that value.
    ...
    /// The registry's "Payload" column for this status, from draft-21
    /// Section 16.9, Table 16: Normal is "Yes", End of Group and End of Track
    /// are "No".
    ///
    /// This is the whole of the rule draft-21 Section 11.1.2 states — an
    ...
    /// formed, and draft-21's encodings have a way to spell it.
    ...
    ///
    /// A draft-21 status datagram carries its status as one bare byte rather
