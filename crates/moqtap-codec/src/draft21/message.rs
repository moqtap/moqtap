//! Draft-21 control message encoding and decoding.
//!
...
//!   a nested parameter block in its own scope (Section 9.20.16).
...
//!
//! Draft-21 restructures draft-20 without changing any of this: it moves the
//! text and renumbers it, and every reference above and below is draft-21's.
...
// ============================================================
// Parameter encoding helpers for draft-21
// ============================================================
...
enum ParamEncoding {
    ...
    LengthPrefixed,
    /// A Track Namespace as defined in draft-21 Section 8.7: a varint field
    /// count followed by that many length-prefixed fields.
    ///
    /// Not one of the four value encodings draft-21 Section 9.20 lists. A
...
fn param_encoding(key: u64) -> Option<ParamEncoding> {
    match key {
        // 0x02 = OBJECT_DELIVERY_TIMEOUT (Section 9.20.5)
        // 0x04 = RENDEZVOUS_TIMEOUT (Section 9.20.7). Not MAX_CACHE_DURATION:
        //        that is Property Type 0x04 in the separate Properties
        //        registry (Section 16.8), a different namespace that happens
        //        to reuse the number.
        // 0x06 = SUBGROUP_DELIVERY_TIMEOUT (Section 9.20.4)
        // 0x08 = EXPIRES (Section 9.20.17, draft-19's 10.2.15)
        // 0x0A = FILL_TIMEOUT (Section 9.20.6)
        // 0x32 = NEW_GROUP_REQUEST (Section 9.20.20, draft-19's 10.2.18)
        0x02 | 0x04 | 0x06 | 0x08 | 0x0A | 0x32 => Some(ParamEncoding::Varint),
        // 0x10 = FORWARD (Section 9.20.19), 0x20 = SUBSCRIBER_PRIORITY,
        // 0x22 = GROUP_ORDER, and 0x35 = INCLUDE_PROPERTIES, new in draft-20.
        // Section 9.20.22: "The INCLUDE_PROPERTIES parameter (Parameter Type
        // 0x35) is a uint8."
        0x10 | 0x20 | 0x22 | 0x35 => Some(ParamEncoding::Uint8),
        // 0x09 = LARGEST_OBJECT. Draft-21 Section 9.20.18: "The LARGEST_OBJECT
        //        parameter (Parameter Type 0x9) is a Location." A Location is
        //        two consecutive varints (Section 9.20), with no length ahead
        ...
        // 0x34 = TRACK_NAMESPACE_PREFIX. Section 9.20.21: it "uses the Track
        //        Namespace encoding described in Section 8.7".
        0x34 => Some(ParamEncoding::TrackNamespaceValue),
        // 0x03 = AUTHORIZATION_TOKEN
        // 0x21 = LOCATION_FILTER, whose value draft-20 rebuilt (Section 9.20.10)
        // 0x23 = FILL_PARAMETERS, new in draft-20. Section 9.20.16: it "uses
        ...
        //        (Range Filters, Section 8.6 — draft-19's Section 5.1.3)
...
/// 9.20.9), FORWARD allows only 0 and 1 (Section 9.20.19), and
/// INCLUDE_PROPERTIES — new in draft-20 — allows only 0 and 1 (Section
/// 9.20.22). SUBSCRIBER_PRIORITY (Section 9.20.8) uses the whole 0-255 range,
...
fn uint8_value_in_range(key: u64, value: u8) -> bool {
    match key {
        ...
        // INCLUDE_PROPERTIES (0x35), Section 9.20.22: "The allowed values are
...
/// Whether draft-21 lets a message carry parameter type `key` more than once.
///
/// Section 9.20 states the default: "Senders MUST NOT repeat the same Parameter
...
///
/// * `AUTHORIZATION_TOKEN` (0x03), Section 8.9: "An Authorization Token MAY
///   be repeated within a message as long as the combination of Token Type and
///   Token Value are unique after resolving any aliases." Draft-20 wrote the
///   same rule as "The AUTHORIZATION TOKEN parameter MAY be repeated"; the
///   restructure reworded the subject and changed nothing it requires.
/// * The five Range Filters (0x25 through 0x29), Section 3.3.2 — draft-19's
...
///
/// The two rules interact, and **the draft does not say how**. Section 9.20 also
...
/// Section 3.3.2's permission unusable, because there is no other encoding for
/// a second filter of the same type.
///
/// What the filters may **not** do is repeat the same (Parameter Type, SetID,
/// Property Type) triple, and Section 3.3.2 answers that with a REQUEST_ERROR
...
///
/// Draft-21 Section 8.3: "The previous Type value plus the Delta Type MUST NOT
...
///
/// Draft-21 Section 8.7: "The maximum total length of a Full Track Name is
...
///
/// Section 8.9: "If the Token structure cannot be decoded, the receiver
/// MUST close the Session with KEY_VALUE_FORMATTING_ERROR." That is the answer
/// Section 8.3 gives for any Type whose value does not match the
...
/// The LOCATION_FILTER parameter type, draft-21 Section 9.20.10.
pub const LOCATION_FILTER: u64 = 0x21;

/// The FILL_PARAMETERS parameter type, draft-21 Section 9.20.16. New in
/// draft-20.
pub const FILL_PARAMETERS: u64 = 0x23;

/// The most `vi64` fields a `LOCATION_FILTER` value can hold: `StartGroup`,
/// `StartObject`, `EndGroupDelta`, `EndObject` (draft-21 Section 9.20.10).
...
/// Decode the `vi64` fields of a draft-21 `LOCATION_FILTER` value.
///
...
/// filter is comes from how many came back; draft-21 Section 9.20.10 gives the
/// table.
///
...
/// to nine bytes wide and Section 8.1 permits non-minimal encodings, so a
...
/// * more than four decoded values makes it malformed, because Section 9.20.10
    ...
    while cursor.has_remaining() {
        if fields.len() == LOCATION_FILTER_MAX_FIELDS {
            return Err(CodecError::SubscriptionFilterMalformed {
                detail: "it holds more than the four vi64 fields Section 3.3.1 defines",
            });
        }
...
/// Hold a decoded `LOCATION_FILTER` to the one arithmetic rule draft-21 states
/// about it.
///
/// Section 3.3.1: "EndGroupDelta is delta encoded from StartGroup, but both the
...
/// never precede the start group; and within one group draft-21 states no rule
/// about `EndObject` being below `StartObject`. Section 3.3.1 says the opposite
...
/// producing a recognisable error, so it is worth naming: Section 9.20 defines a
...
/// in Section 9. The outer length prefix is the generic length-prefixed value
...
/// both in the message and inside FILL_PARAMETERS." Section 9.20 defines
...
/// the parameter appeared in. A type draft-21 does not define at all is
...
fn decode_parameters(buf: &mut impl Buf) -> Result<Vec<KeyValuePair>, CodecError> {
    ...
    for i in 0..count {
        ...
        // parameter before it. Draft-21 Section 9.20: "Receivers SHOULD check
        ...
        // Section 9.20: "All Message Parameters MUST be defined in the
...
fn decode_kvp_delta(buf: &mut impl Buf) -> Result<Vec<KeyValuePair>, CodecError> {
    ...
    while buf.has_remaining() {
        ...
        let value = if abs_key.is_multiple_of(2) {
            ...
            let len = VarInt::decode_moqt::<Wire>(buf)?.into_inner() as usize;
            // Draft-21 Section 8.3: "The maximum length of a value is 2^16-1
...
///
/// Section 10.7: Immutable Properties are "a Track or Object Property that
...
/// Whether `value` is inside the range draft-21 allows for a Track Property
/// type that restricts one.
///
/// Two types do, and each answers anything outside its range with a session
/// close. DEFAULT_PUBLISHER_GROUP_ORDER (0x22), Section 10.5: "The allowed
...
/// PROTOCOL_VIOLATION." DYNAMIC_GROUPS (0x30), Section 10.6: "The allowed
...
///
/// DEFAULT_PUBLISHER_PRIORITY (0x0E) is not here. Section 10.4 says
...
/// contents Section 10.7 defines as properties themselves. The draft asks for
...
/// rather than refused. Section 10.7 says relays "MAY decode and view the
...
///
/// Section 9.1 assigns PATH, AUTHORIZATION TOKEN, MAX_AUTH_TOKEN_CACHE_SIZE, AUTHORITY,
/// MAX_FILTER_RANGES, MOQT_IMPLEMENTATION and MAX_REQUEST_UPDATES.
///
/// The list exists for one rule and one direction. Section 9.1: "Receivers
...
///
/// Section 9.1.4: "The AUTHORIZATION TOKEN Setup Option (Option Type 0x03)
...
///
/// Section 9.1: "Senders MUST NOT repeat the same Option Type in a message
...
pub enum MessageType {
    ...
    /// REQUEST_OK (0x07). Draft-21 Section 9.3 uses PUBLISH_OK as a shorthand
...
/// GOAWAY (0x10). Draft-21's GOAWAY has no Request ID field, so the
...
/// codes; drafts 19 and 20 do not, because Section 3.1 lets an endpoint hold
...
pub struct PublishDone {
    ...
    /// the draft's instruction rather than an omission. Draft-21 Section 13:
    ...
    /// it together (Section 3.3.1: "A publisher does not end a subscription
    /// solely because the Largest Object advances past the end of the current
    /// Location Filter"). A draft-21 receiver reads a 0x3 as INTERNAL_ERROR.
    pub status_code: VarInt,
    /// The number of streams the publisher opened for this subscription.
    ///
    /// Draft-21 Section 9.9 widens what is counted: the total now includes
    /// "streams that contained no Objects (e.g., an empty Subgroup) and
    /// including any fill fetch streams (see Section 3.4)". Draft-19 counted
...
pub mod publish_done_codes {
    ...
    ///
    /// Draft-21 Section 9.9: "If the publisher is unable to set Stream Count
    ...
    /// QUIC varint tops out; MoQT's own varint (Section 8.1) is a
    ...
    ///
    /// **In draft-21 the sentinel is indistinguishable from a well-formed
...
/// and 17 carry a `subscribe_options` varint here; on draft-21 a namespace
...
/// parameter (Section 9.20.19), which on drafts 15 through 17 may appear on
...
/// FETCH (0x16), rebuilt in draft-20 (Section 9.11, Figure 15).
///
/// ```text
/// FETCH Message {
///   Type (vi64) = 0x16,
...
///
/// The range now travels in the `LOCATION_FILTER` parameter (Section 3.3.1). A
/// FETCH with none covers `{0,0}` through Largest Object, inclusive.
///
/// # `INVALID_RANGE` and a relative start
///
/// Section 9.11 keeps draft-19's rule: "If no Objects have been published for
...
/// it. That is why draft-21 has a FETCH decoder of its own rather than sharing
/// draft-19's.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Fetch {
    pub request_id: VarInt,
...
pub struct FetchOk {
    ...
    /// 0 to indicate the entire Group)". Draft-21 Section 9.12 drops that
    /// parenthesis entirely, and Sections 3.3.1 and 9.11 both say the Location
    ...
    /// filter meant "all objects in the End Group", Section 9.12 does not say
...
/// PUBLISH_STATE_NOTIFY (0x22), new in draft-20 (Section 9.10, Figure 14).
///
/// ```text
...
/// than the frame, and which the caller therefore owns (Section 9.10):
///
...
// Draft-21 has no range check on a control message, and the absence is the
...
// (Section 9.11), so there is no pair left in any message to compare.
//
// What replaced them cannot express the malformation either. A `LOCATION_FILTER`
// (Section 3.3.1) states its end as an unsigned `EndGroupDelta` added to
// `StartGroup`, so the end group can never precede the start group; and within
// one group draft-21 states no rule about an `EndObject` below `StartObject`.
// Section 3.3.1 says the opposite for subscriptions — "A Location Filter on a
...
// is not there.
//
// The one backwards-range rule draft-21 does keep is in Section 9.12: "If End
...
///
/// One draft-21 message carries a field that says which of the following fields
...
/// both bodies (Section 9.11), so FETCH has nothing left to disagree with
/// itself about.
///
...
/// Whether draft-21 lets Message Parameter `key` appear in `message`.
///
...
///
/// * Six of the names are one wire type. Section 9.3: "This document uses the
...
/// * The five Range Filters state their scope in Section 3.3.2 — draft-19's
...
///   instead; the Range Filters dropped it via Section 3.3.2. `EXPIRES` (0x08)
...
///   sending any of the six on a PUBLISH_OK is a Section 9.20.1 violation.
...
/// than an omission here: Section 9.12 gives it a Parameters field and no
...
/// that list as closed is **a decision this codec makes**. Section 9.10 says
...
/// anything else there is refused under Section 9.20.1 on that reading.
///
...
pub fn parameter_in_scope(key: u64, message: MessageType) -> bool {
    ...
    match key {
        ...
        // Section 9.20.18 LARGEST OBJECT: "It MAY appear in SUBSCRIBE_OK,
        ...
        // Section 9.20.6 FILL TIMEOUT: it "MAY appear in a FETCH message, or
        // inside a FILL_PARAMETERS parameter (see Section 9.20.16) in a
        ...
        // per Section 9.20.16 — so it is enforced by
        // [`decode_fill_parameters`] against Table 6 instead.
        0x0A => matches!(message, M::Fetch),
        // Section 9.20.19 FORWARD: "It MAY appear in SUBSCRIBE, REQUEST_UPDATE
        ...
        // message." Draft-20 opened the same definition with a sentence of its
        // own, "uses length-prefixed encoding", which draft-21 dropped; the
        ...
        // Section 9.20.9 GROUP ORDER: "It MAY appear in a SUBSCRIBE, PUBLISH,
        ...
        // Section 3.3.2, of TRACK_PROPERTY_FILTER (Section 9.20.15) alone: it
        ...
        // Section 9.20.22 INCLUDE_PROPERTIES, new in draft-20: "It MAY appear
...
/// place there.
///
/// Section 9.20.1 answers this with a close, which the drafts below do not.
...
fn check_parameter_scope(message: &ControlMessage) -> Result<(), CodecError> {
    let parameters = match message {
        ...
        // 9.20.1 notes that "since Setup Options use a separate namespace, it
...
impl ControlMessage {
    pub fn encode(&self, buf: &mut impl BufMut) -> Result<(), CodecError> {
        ...
        // Draft-21: 16-bit length (big-endian)
    ...
    pub fn decode(buf: &mut impl Buf) -> Result<Self, CodecError> {
        ...
        // Draft-21 Section 9: "If the length does not match the length of the
    ...
    fn encode_body(&self, buf: &mut impl BufMut) -> Result<(), CodecError> {
        match self {
            ...
            // Section 9.10, Figure 14: parameter count, then parameters,
            ...
            // Section 9.11, Figure 15. Byte-identical to SUBSCRIBE above:
    ...
    fn decode_body(msg_type: MessageType, buf: &mut impl Buf) -> Result<Self, CodecError> {
        match msg_type {
            ...
            MessageType::GoAway => {
                ...
                // Draft-21 Section 9.2: an endpoint that receives a New
            ...
            MessageType::RequestError => {
                ...
                // Draft-21 Section 8.5: a received reason phrase length above
            ...
            MessageType::PublishDone => {
                // The Status Code is carried up whatever it is. Section 13
                ...
                let reason_len = VarInt::decode_moqt::<Wire>(buf)?.into_inner() as usize;
                // Draft-21 Section 8.5, same bound as REQUEST_ERROR above.
                if reason_len > MAX_REASON_PHRASE_LENGTH {
                    return Err(CodecError::ReasonPhraseTooLong);
                }
                let reason_phrase = read_bytes(buf, reason_len)?;
...
mod tests {
    ...
    /// Frame `body` as a draft-21 control message of `type_id`, declaring
    ...
    /// Draft-21 Section 8.5: "The reason phrase length has a maximum value of
    ...
    /// Draft-21 Section 9.2: "The maximum length of the New Session URI is
    ...
    /// Draft-21 Section 9.20.9 (GROUP_ORDER): "The allowed values are Ascending
    ...
    /// 9.20.18 says the same of FORWARD with the values 0 and 1.
    ///
    ...
    ///
    /// ```text
    /// panicked at crates\moqtap-codec\src\draft21\message.rs:
    ...
    /// Draft-21 Section 9.20.21: the TRACK_NAMESPACE_PREFIX parameter
    /// (Parameter Type 0x34) "uses the Track Namespace encoding described in
    /// Section 8.7" — a varint field count followed by that many
    /// length-prefixed fields, and nothing in front of it. That encoding is not
    /// one of the four Section 9.20 lists, so it cannot be assumed to be
    /// Length-prefixed by default.
    ///
    /// The frame below is built from Section 8.7: REQUEST_UPDATE for request
    ...
    /// unread. Draft-21's Section 9 body-length check turns that into a
