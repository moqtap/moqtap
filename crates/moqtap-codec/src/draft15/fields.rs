use crate::draft15::message::ControlMessage;
...
fn d15_setup_param_name(key: u64) -> Option<&'static str> {
    match key {
        0x01 => Some("path"),
...
fn d15_msg_param_name(key: u64) -> Option<&'static str> {
    match key {
...
fn decode_subscription_filter(bytes: &[u8]) -> Value {
    ...
    match filter_type {
        ...
        _ => {
            // LatestGroup (1), LatestObject (2), or unknown — no extra fields
        }
    }
    Value::Map(obj)
}

/// Render a draft-15 LARGEST_OBJECT (0x09) parameter value: a Group and an
...
/// their extractor is two varints by construction. Draft-15 has no such table.
/// 0x09 is an odd Type, so `KeyValuePair::decode` keeps whatever
/// length-prefixed bytes arrived, and none of `decode_parameters`' four checks
/// — duplicates, authorization tokens, varint value ranges, subscription
/// filters — looks at 0x09. `KNOWN_VERSION_SPECIFIC_PARAMETERS` admits it and
/// `check_parameter_scope` permits it on SUBSCRIBE_OK, so an eight-byte
/// SUBSCRIBE_OK carrying `0x09` with an empty value reaches here.
///
...
/// `auth_token_to_json_d15` do. Field extraction runs on a message that has
...
/// Parse an authorization_token byte value into structured JSON.
fn auth_token_to_json_d15(bytes: &[u8]) -> Value {
    let mut buf = bytes;
...
fn kvp_to_json_d15_inner(
    ...
    crate::fields::kvp_entries(params, |key, value| {
        ...
        let rendered = match (value, key) {
            ...
            (KvpValue::Bytes(b), _) if name == "authorization_token" => auth_token_to_json_d15(b),
...
fn kvp_to_json_d15(params: &[KeyValuePair]) -> Value {
...
pub fn message_fields(msg: &ControlMessage) -> Map {
    let obj = match msg {
        ControlMessage::ClientSetup(m) => {
            let mut o = Map::new();
            o.insert("parameters".into(), kvp_to_json_d15_setup(&m.parameters));
            o
        }
        ...
        ControlMessage::RequestOk(m) => {
            ...
            o.insert("parameters".into(), kvp_to_json_d15(&m.parameters));
            o
        }
        ...
        ControlMessage::Fetch(m) => {
            ...
            match &m.fetch_payload {
                crate::draft15::message::FetchPayload::Standalone {
                    track_namespace,
                    track_name,
                    start_group,
                    start_object,
                ...
                crate::draft15::message::FetchPayload::Joining {
                    joining_request_id,
                    joining_start,
                } => {
