//! Draft-08 control message encoding and decoding.
...
///
/// Draft-08: filter_type is a VarInt. AbsoluteRange has no end_object.
#[derive(Debug, Clone, PartialEq, Eq)]
...
///
/// Draft-08: no end_object field (only end_group).
#[derive(Debug, Clone, PartialEq, Eq)]
...
///
/// Draft-08: has stream_count instead of content_exists/final_group/final_object.
#[derive(Debug, Clone, PartialEq, Eq)]
...
///
/// Draft-08: largest_group_id and largest_object_id are always present.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FetchOk {
    /// The subscribe ID this response corresponds to.
    pub subscribe_id: VarInt,
...
fn check_track_status(
    ...
    let code = crate::draft08::error_codes::TrackStatusCode::from_u64(status_code.into_inner())
    ...
    fn encode_payload(&self, buf: &mut impl BufMut) -> Result<(), CodecError> {
        match self {
            ...
            ControlMessage::Subscribe(m) => {
                ...
                m.track_namespace.validate(TrackNamespaceRules::for_draft(8))?;
            ...
            ControlMessage::SubscribeAnnounces(m) => {
                m.track_namespace_prefix.validate(TrackNamespaceRules::for_draft(8))?;
