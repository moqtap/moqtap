...
/// Session termination error codes (draft-19 Section 15.11.1).
///
...
/// and none in 0xA-0xF. 0x7 was last assigned by draft-17, as `INVALID_REQUIRED_REQUEST_ID`;
/// drafts 14 through 16 used it for `TOO_MANY_REQUESTS`. Draft-18 vacated it and draft-19 leaves
/// it vacant.
///
/// The registry also reserves the code points `0x7f * N + 0x9D` for greasing (draft-19 Section
...
/// REQUEST_ERROR codes (draft-19 Section 15.11.2).
///
...
pub enum RequestErrorCode {
    ...
    /// `INVALID_JOINING_REQUEST_ID` — In response to a Joining FETCH, the referenced Request ID
...
/// PUBLISH_DONE codes (draft-19 Section 15.11.3).
///
...
pub enum PublishDoneStatusCode {
    ...
    /// `SUBSCRIPTION_ENDED` — The publisher reached the end of an associated location filter.
...
/// Stream reset error codes (draft-19 Section 15.11.4).
///
...
/// data streams; draft-19 keeps both the name and the wider scope, so this enum
/// takes that title.
///
/// The registry also reserves the code points `0x7f * N + 0x9D` for greasing (draft-19 Section
/// 14). That is a range rather than an assignment, so it has no variant here and `from_u64`
...
pub enum StreamResetErrorCode {
    ...
    /// (Section 10.11) may have a more detailed status code.
    Cancelled = 0x1,
...
impl SessionErrorCode {
    /// Every Session Error Code draft-19 assigns, in ascending wire order.
    ///
...
impl RequestErrorCode {
    /// Every Request Error Code draft-19 assigns, in ascending wire order.
    ///
    /// This is the set [`Self::from_u64`] accepts, written out so that it can
    /// be enumerated: nothing can iterate an enum's variants, so a caller that
    /// wants the registry has to be handed it. Writing it down is also what
...
impl PublishDoneStatusCode {
    /// Every PUBLISH_DONE Status Code draft-19 assigns, in ascending wire order.
    ///
    /// This is the set [`Self::from_u64`] accepts, written out so that it can
    /// be enumerated: nothing can iterate an enum's variants, so a caller that
    /// wants the registry has to be handed it. Writing it down is also what
...
impl StreamResetErrorCode {
    /// Every Stream Reset Error Code draft-19 assigns, in ascending wire order.
    ///
    /// This is the set [`Self::from_u64`] accepts, written out so that it can
    /// be enumerated: nothing can iterate an enum's variants, so a caller that
    /// wants the registry has to be handed it. Writing it down is also what
