...
///
/// "The TRACK_STATUS message format is identical to the SUBSCRIBE message", so
/// the fields below are the SUBSCRIBE fields and nothing else — including the
/// two that only some filters put on the wire. Draft-12 had no such message and
/// draft-13's changelog records the change, which is why this shape does not
/// look like the draft-12 TRACK_STATUS_REQUEST it replaced.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct TrackStatus {
    ...
    pub forward: Forward,
    pub filter_type: FilterType,
    /// Present only for the AbsoluteStart and AbsoluteRange filter types.
    pub start_group: Option<VarInt>,
    /// Present only for the AbsoluteStart and AbsoluteRange filter types.
    pub start_object: Option<VarInt>,
    /// Present only for the AbsoluteRange filter type.
    pub end_group: Option<VarInt>,
    pub parameters: Vec<KeyValuePair>,
}
...
pub struct FetchOk {
    pub request_id: VarInt,
    pub group_order: GroupOrder,
    /// End-of-track flag, a single byte.
    ///
...
/// SHOULD be used". "Values larger than 0x2 are a protocol error."
///
...
///
/// SUBSCRIBE_OK, PUBLISH, PUBLISH_OK and FETCH_OK, and TRACK_STATUS_OK,
/// whose format "is identical to the SUBSCRIBE_OK message" each say
...
/// answer it can give. SUBSCRIBE, TRACK_STATUS and FETCH are the requests, and there
...
/// Read a Filter Type.
///
/// Unlike the fields above this one really is `(i)`, so the varint reader is
/// right here. "A filter type other than the above MUST be treated as error."
///
/// An error and not a close: this draft names no code and no session
/// consequence, where draft-14 Section 9.7 answers the same value with "MUST be
/// close the session with PROTOCOL_VIOLATION". The message is refused either
/// way; only draft-14's refusal reaches the session.
...
///
/// Several draft-13 messages carry a field that says which of the fields after
/// it are on the wire — SUBSCRIBE's and PUBLISH_OK's Filter Type, SUBSCRIBE_OK's
/// and PUBLISH's Content Exists, FETCH's Fetch Type. This codec holds the
/// optional halves in `Option`s and an enum, so a value can say one thing in its
/// discriminator and another in its body, and the two sides resolve that
/// differently: the encoder writes whatever the body holds, the decoder reads
/// whatever the discriminator announces.
///
/// The result is a message that does not survive its own round trip. A
/// SUBSCRIBE_OK saying content exists with no location encodes to a frame whose
/// own reader takes the parameter count for a Group ID. Refusing at the encoder
/// keeps the two readings from diverging on the wire.
///
/// TRACK_STATUS is here because its format is the SUBSCRIBE format, so its
/// Filter Type governs the same two optional fields.
fn check_discriminators(message: &ControlMessage) -> Result<(), CodecError> {
    /// Both halves of a Start Location are present together or not at all, and
    /// only the two absolute filters put one on the wire.
    fn check_filter(
        filter_type: FilterType,
...
///
/// The number is this draft's own. Draft-13 assigns AUTHORIZATION TOKEN
