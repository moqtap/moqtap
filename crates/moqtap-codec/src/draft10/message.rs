...
//!
//! The wire format is draft-09's, unchanged: the same 26 message types with the
//! same field lists. Only the section numbering moves, which is why the
//! citations here are one chapter later than draft-09's.
//!
...
/// Control message type IDs (draft-10).
...
///
/// Filter Type `0x1` is rejected on decode. It was withdrawn at draft-09 and
/// draft-10 does not bring it back.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Fetch type for FETCH message (draft-10).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u64)]
pub enum FetchType {
...
/// Hold a TRACK_STATUS Status Code and the fields after it to Section 8.17.
///
...
fn check_track_status(
    ...
    let code = crate::draft10::error_codes::TrackStatusCode::from_u64(status_code.into_inner())
...
/// A decoded MoQT control message (draft-10).
#[derive(Debug, Clone, PartialEq, Eq)]
...
///
/// SUBSCRIBE's AbsoluteRange filter (Section 8.6), SUBSCRIBE_UPDATE
/// (Section 8.9) and FETCH (Section 8.12) each state it, and the fields
...
///
/// Section 8.1: "Senders MUST NOT repeat the same parameter type in a
...
///
/// Section 8.2.2.2 gives MAX_SUBSCRIBE_ID as "an initial value for the Maximum
/// Subscribe ID". PATH (Section 8.2.2.1) is a URI string and carries no implied
...
/// integer.
///
/// Section 8.1.1.2 gives DELIVERY TIMEOUT as "the duration in milliseconds"
/// and Section 8.1.1.3 gives MAX CACHE DURATION as "An integer expressing a
...
///
/// Section 8.1: "If a receiver understands a parameter type, and the parameter
    ...
    fn encode_payload(&self, buf: &mut impl BufMut) -> Result<(), CodecError> {
        match self {
            ...
            ControlMessage::Subscribe(m) => {
                ...
                m.track_namespace.validate(TrackNamespaceRules::for_draft(10))?;
            ...
            ControlMessage::SubscribeAnnounces(m) => {
                m.track_namespace_prefix.validate(TrackNamespaceRules::for_draft(10))?;
    ...
    fn decode_payload(msg_type: MessageType, buf: &mut impl Buf) -> Result<Self, CodecError> {
        match msg_type {
            ...
            MessageType::Subscribe => {
                ...
                // Filter Type 0x1 (NextGroupStart/LatestGroup) is not a value
                // this draft assigns.
                if filter_val == 1 {
