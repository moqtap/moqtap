...
/// Session termination codes (draft-10, Section 3.4 "Termination").
///
/// Sent in the QUIC CONNECTION_CLOSE frame, or in the
/// CLOSE_WEBTRANSPORT_SESSION capsule when WebTransport is used. The draft
/// states the application MAY use any error message and SHOULD use a relevant
/// code from this table.
///
/// The table assigns 0x0 through 0x6 and then jumps to 0x10. Draft-10 declares
/// no reserved range over 0x7 to 0xF, so those values are simply unassigned and
/// `from_u64` rejects them.
...
pub enum SessionErrorCode {
    ...
    /// `Parameter Length Mismatch` — Section 3.4 lists this code but does not
    /// describe it; Section 8.1 requires a receiver that understands a
    /// parameter type to terminate the session with this code when the length
    /// implied by that type does not match the Parameter Length field.
    ...
    /// to close the session in response to a GOAWAY message (Section 8.3).
    /// Section 3.5 adds that a server SHOULD terminate the session with this
    /// code after a sufficient timeout if there are still open subscriptions or
    /// fetches on the connection.
    GoawayTimeout = 0x10,
    ...
    /// stream MAY send a STOP_SENDING on that stream, terminate the
    /// subscription, or close the session with an error.
...
///
/// The draft calls these status codes rather than error codes: a subscription
/// that ends normally is also reported through this registry.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u64)]
...
/// ANNOUNCE_ERROR codes (draft-10, Section 8.20 "ANNOUNCE_ERROR").
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u64)]
...
/// SUBSCRIBE_ANNOUNCES_ERROR codes (draft-10, Section 8.25
...
impl SessionErrorCode {
    /// Every session termination code draft-10 assigns, in ascending wire order.
    ///
    ...
    /// Convert a raw u64 to a `SessionErrorCode`, if draft-10 defines it.
...
impl SubscribeErrorCode {
    ...
    /// Convert a raw u64 to a `SubscribeErrorCode`, if draft-10 defines it.
    pub fn from_u64(v: u64) -> Option<Self> {
        match v {
...
impl SubscribeDoneStatusCode {
    ...
    /// Convert a raw u64 to a `SubscribeDoneStatusCode`, if draft-10 defines it.
    pub fn from_u64(v: u64) -> Option<Self> {
        match v {
...
impl FetchErrorCode {
    ...
    /// Convert a raw u64 to a `FetchErrorCode`, if draft-10 defines it.
    pub fn from_u64(v: u64) -> Option<Self> {
        match v {
...
impl AnnounceErrorCode {
    /// Every ANNOUNCE_ERROR code draft-10 assigns, in ascending wire order.
    ///
    /// This is the set [`Self::from_u64`] accepts, written out so that it can
    /// be enumerated: nothing can iterate an enum's variants, so a caller that
    /// wants the registry has to be handed it. Writing it down is also what
    ...
    /// Convert a raw u64 to an `AnnounceErrorCode`, if draft-10 defines it.
    pub fn from_u64(v: u64) -> Option<Self> {
        match v {
...
impl SubscribeAnnouncesErrorCode {
    ...
    /// Convert a raw u64 to a `SubscribeAnnouncesErrorCode`, if draft-10
    /// defines it.
    pub fn from_u64(v: u64) -> Option<Self> {
        match v {
...
/// TRACK_STATUS Status Code values (draft-10, Section 8.17).
///
...
impl TrackStatusCode {
    ...
    ///
    /// Section 8.17 says of 0x01 "Subsequent fields MUST be zero, and any other
