...
//! the other.** Section 11.4.2 gives SUBGROUP_HEADER three conditions and
//! Section 11.3.1 gives OBJECT_DATAGRAM three of its own, and they disagree at
//! two points:
//!
...
/// draft-20 does not assign.
///
/// Draft-20 Section 11.2.1.1 lists the codes an object may carry — the three
...
/// Unidirectional stream type for padding, draft-20 Section 11.5.1: "An
...
/// draft-20 variable-length integer encoding the value takes five bytes, the
...
/// Datagram type for padding, draft-20 Section 11.5.2: "An endpoint MAY send a
...
/// The unidirectional stream Type draft-20 Section 10.3 gives the control
/// stream.
...
/// The SUBGROUP_ID_MODE value draft-20 reserves, once the mask is applied and
/// the field shifted down.
const SUBGROUP_ID_MODE_RESERVED: u8 = 0b11;

/// Refuse a subgroup header `Type Flags` value draft-20 Section 11.4.2 lists
/// as invalid.
///
...
///
/// **These are not the datagram's rules.** Section 11.3.1 reserves bit 4 and
...
/// Whether `raw` is a subgroup `Type Flags` value draft-20 admits: below 128,
/// bit 4 set, and not the reserved SUBGROUP_ID_MODE.
fn subgroup_type_is_valid(raw: u64) -> bool {
    // `raw < 0x80` is Section 11.4.2's third condition, and it subsumes the
...
/// reader wants is.
///
/// Draft-20 states two rules about such a Type and answers both with a close,
...
///
/// Section 11.4.2 is about the subgroup form specifically, and gives three
...
///
/// Section 11.4.2's conditions are about a one-byte flags field, so they are
/// applied to values that fit one — `raw <= 0xFF`. Above that the value is out
/// of the space the section describes and the question is Table 3's instead:
...
/// Section 11.4.2's bit-4 condition calls it — while a stray 0x5000 stays an
/// unknown stream type.
...
impl SubgroupHeader {
    ...
    /// and the value it decodes to is what the three Section 11.4.2 conditions
    ...
    ///
    /// Infallible, and so willing to write a Type draft-20 Section 11.4.2
    ...
    /// Serialize the header, refusing a Type value draft-20 forbids.
    ///
    /// Errors with [`CodecError::InvalidField`] for exactly the Types draft-20
...
/// One object within a draft-20 subgroup stream. Object IDs are
...
pub struct SubgroupObject {
    ...
    /// the Object Status registry (draft-20 Section 15.9) that permits the
    ...
    /// any value, and draft-20 assigns three of them; the decoder refuses the
    ...
    /// way to express. That combination is what draft-20's registry rules on:
...
impl SubgroupObject {
    /// The object's status, with the one draft-20's encoding elides filled in.
    ///
    ...
    /// Status registry (draft-20 Section 15.9) permitting a payload, so the
    ...
    /// payload, per draft-20 Section 15.9.
    ///
...
impl SubgroupObjectMeta {
    ...
    ///
    /// Draft-20 Section 15.9, Table 16 gives the registry a "Payload" column,
    ...
    /// column in for the three statuses draft-20 assigns — 0x0 Normal is
...
impl SubgroupObjectReader {
    ...
    /// that can reach this method is one draft-20 assigns and one
    ...
    /// (draft-20 Section 15.9) does not permit the object's status to carry a
    /// payload and a payload was handed over anyway. Draft-20 Section 11.2.1.1
    ...
    pub fn write_object(
        ...
        // draft-20 Section 11.2.1.2 forbids them. The two rules differ in kind.
...
/// Bit 4. Draft-20 Section 11.3.1: "This bit is reserved and MUST be zero."
/// The opposite of Section 11.4.2, where the same bit MUST be 1.
const DATAGRAM_RESERVED_BIT: u8 = 0x10;

/// Refuse a datagram `Type Flags` value draft-20 Section 11.3.1 lists as
/// invalid.
///
...
/// unchanged.
///
/// **These are not the subgroup header's rules.** Section 11.4.2 requires bit 4
...
/// Every bit draft-20 Section 11.3.1 gives a datagram `Type Flags` value a
...
/// Whether `raw` is a datagram `Type Flags` value draft-20 admits.
...
/// Section 11.3.1's three conditions each get an arm, so a log names which one
/// a value fell into rather than reporting all three as one complaint; a value
/// too wide to be a one-byte flags field at all is Section 3.4's
...
pub struct DatagramHeader {
    ...
    /// registry (draft-20 Section 15.9) permitting one. [`Self::status`]
    /// resolves the field either way.
    ///
    /// Typed rather than a bare byte. The wire field is one octet with 256
    /// values, and draft-20 Section 11.2.1.1 assigns three of them; the
...
impl DatagramHeader {
    ...
    pub fn decode(buf: &mut impl Buf) -> Result<Self, CodecError> {
        ...
        // Two rules of draft-20 Section 11.3.1 reach the properties block just
    ...
    /// that boundary is the only thing that delimits the payload — draft-20
    ...
    ///   opposite in Section 11.4.2 — there the PROPERTIES bit is fixed for the
    ...
    ///
    /// Also errors with [`CodecError::InvalidField`] for a Type value draft-20
    ...
    /// clear is discarded here without a word, and a Type value draft-20
    ...
    ///
    /// - The framing. Draft-20 Section 11.3.1: "The STATUS bit (0x20)
    ...
    /// has.
    ///
    /// The same rule the subgroup form obeys. Draft-20 Section 11.3.1 builds
    ...
    ///
    /// Draft-20 Section 11.3.1: "If an endpoint receives a datagram with the
...
///   Section 5.1.3 says the fill is "delivered as a FETCH response" and
///   Section 5.1.3.1 says there is no `REQUEST_ERROR` for it, so there is no OK
...
///   and does not synthesize a `FETCH_OK`. Section 10.13's rule that gaps
...
/// * **Failure is a stream reset, not an error message.** Section 5.1.3.1: if
...
/// (draft-20 Section 11.4.4.1, Table 8).
...
/// The largest Serialization Flags value whose bits are flags. Draft-20
...
/// cannot be, because Section 11.4.4 reserves values below 128 for the bit-flag
...
/// Locations it covers, from draft-20 Section 11.4.4.2.
///
...
pub enum FetchEndOfRange {
    ...
    ///
    /// Section 11.4.4.2: "A publisher SHOULD NOT use End of Non-Existent Range
...
///
/// Draft-20 Section 11.4.4 rebuilt the fetch object. Earlier drafts wrote a
/// fixed set of fields on every Object; draft-20 writes a Serialization Flags
...
///
/// There is no Object Status field. Draft-20 Section 11.2.1.1 states that
...
pub struct FetchObjectHeader {
    ...
    /// property blocks on the subgroup and datagram forms — draft-20
...
impl FetchObjectHeader {
    ...
    ///
    /// Draft-20 Section 11.4.4, Table 7 gives the three indicators their own
    ...
    /// uses for its deltas, and Section 11.4.4.2 says only that "the Group ID
    ...
    /// say so: Section 11.4.4.2 states what is *absent* from a marker
    ...
    /// This codec applies Section 11.4.4.1 unchanged, because the marker's
    ...
    /// `0b11` = an explicit field is present. Draft-20 Section 11.4.4.1
    ...
    /// [`CodecError::InvalidField`] if draft-20 does not define that
    ...
    ///
    /// The three indicators get their layout from Section 11.4.4.2 rather than
    ...
    /// Section 11.4.4.2 lists what is absent and does not name it, and Figure
    ...
    /// value draft-20 Section 11.4.4 does not define, and with
    ...
    /// draft-20 does not define, and when any optional field is present while
...
/// Delta is applied.
///
/// Draft-20 Section 11.4.4.1: "If the Group Order is Ascending, the Group ID is
...
pub struct FetchObject {
    ...
    ///
    /// The draft-20 fallback for a subscription that never stated a priority is
...
///
/// Draft-20 Section 11.4.4.1 defines nearly every field of a fetch frame
...
impl FetchObjectReader {
    ...
    pub fn read_object_header(&mut self, buf: &mut impl Buf) -> Result<FetchObject, CodecError> {
        ...
        // 11.4.4.2 says the prior Subgroup ID and prior Priority for whatever
...
/// *against*, and draft-20 Section 11.4.4.1 defines nearly every field against
...
/// handed that no draft-20 stream could carry, and inventing a value for it
...
mod tests {
    ...
    /// `test-vectors/transport/draft20/codec/data-streams/subgroup.json`.
    ...
    /// Every status draft-20 assigns can be written and read back as the same
    ...
    ///                found enum `Option<draft20::types::ObjectStatus>`
    /// ```
    #[test]
    ...
    /// Draft-20 Section 15.9, Table 16, "Payload" column: Normal "Yes", End of
