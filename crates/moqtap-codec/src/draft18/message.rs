...
//! - New TRACK_NAMESPACE_PREFIX parameter (0x34) for REQUEST_UPDATE, carrying
//!   a bare Track Namespace (Section 2.4.1) with no length ahead of it.
...
enum ParamEncoding {
    ...
    LengthPrefixed,
    /// A Track Namespace as defined in draft-18 Section 2.4.1: a varint field
    /// count followed by that many length-prefixed fields.
    ///
    /// Not one of the four value encodings draft-18 Section 10.2 lists. A
...
fn param_encoding(key: u64) -> Option<ParamEncoding> {
    match key {
        ...
        // 0x09 = LARGEST_OBJECT. Draft-18 Section 10.2.11: "The LARGEST_OBJECT
        ...
        // 0x34 = TRACK_NAMESPACE_PREFIX (new in draft-18). Section 10.2.14:
        //        it "uses the Track Namespace encoding described in
        //        Section 2.4.1".
...
/// Whether `value` is inside the range draft-18 allows for a uint8-valued
/// parameter.
///
...
/// 10.2.8), and FORWARD allows only 0 and 1 (Section 10.2.12).
/// SUBSCRIBER_PRIORITY (Section 10.2.7) uses the whole 0-255 range, so it has no
/// entry here.
///
...
///
/// Draft-18 Section 1.4.3: "The previous Type value plus the Delta Type MUST NOT
/// be greater than 2^64 - 1. If a Delta Type is received that would be too
/// large, the Session MUST be closed with a PROTOCOL_VIOLATION." Section 10.2
/// repeats it for Message Parameters. MoQT varints span the whole 64-bit range,
/// so a peer can drive the sum past the end: a debug build panicked on the
/// addition and a release build wrapped the key and reported the parameter under
/// a type its sender never wrote.
...
///
/// Draft-18 Section 2.4.1: "The maximum total length of a Full Track Name is
...
/// MUST close the session with PROTOCOL_VIOLATION." Section 10.2.9: "The
...
///
/// The End Group is a delta, and this draft is the first to say what happens
/// when resolving it leaves the number space: "the last Group ID to be delivered
...
fn decode_parameters(buf: &mut impl Buf) -> Result<Vec<KeyValuePair>, CodecError> {
    ...
    for i in 0..count {
        ...
        // parameter before it. Draft-18 Section 10.2: "Receivers SHOULD check
...
/// * A repeated type, except AUTHORIZATION_TOKEN (Section 10.2.2).
...
fn decode_kvp_delta(buf: &mut impl Buf) -> Result<Vec<KeyValuePair>, CodecError> {
    ...
    while buf.has_remaining() {
        ...
        let value = if abs_key.is_multiple_of(2) {
            ...
            let len = VarInt::decode_moqt::<Wire>(buf)?.into_inner() as usize;
            // Draft-18 Section 1.4.3: "The maximum length of a value is 2^16-1
...
///
/// Section 10.3.1 assigns PATH, AUTHORIZATION TOKEN, MAX_AUTH_TOKEN_CACHE_SIZE, AUTHORITY and
/// MOQT_IMPLEMENTATION.
///
...
///
/// Two draft-18 messages carry a field that says which of the following fields
...
///
/// Five of the names are one wire type. Section 10.5: "This document uses the
...
/// sent in response to the corresponding request type." Which one a given
/// REQUEST_OK is depends on the request its Request ID answers, which is
/// session state and not in the frame, so each of those names widens the same
/// arm and a REQUEST_OK is held to their union. Draft-17 needed none of this:
/// there PUBLISH_OK is its own message type, and the two drafts' tables differ
/// accordingly.
///
/// Where a name is qualified — "REQUEST_UPDATE (for a subscription)",
/// "REQUEST_UPDATE for a SUBSCRIBE_NAMESPACE or SUBSCRIBE_TRACKS request" — the
/// qualifier says which instance of the destination is meant rather than naming
/// another, and settling it needs the same session state, so the message type
/// alone decides.
///
...
fn parameter_in_scope(key: u64, message: MessageType) -> bool {
    ...
    match key {
        ...
        0x02 => matches!(message, M::RequestOk | M::Subscribe | M::RequestUpdate),
        ...
        // PUBLISH_OK, SUBSCRIBE, or REQUEST_UPDATE message."
        0x06 => matches!(message, M::RequestOk | M::Subscribe | M::RequestUpdate),
        // Section 10.2.10 EXPIRES: "It MAY appear in SUBSCRIBE_OK, PUBLISH,
        // PUBLISH_OK, or REQUEST_UPDATE_OK."
        0x08 => matches!(message, M::SubscribeOk | M::Publish | M::RequestOk),
        // Section 10.2.11 LARGEST OBJECT: "It MAY appear in SUBSCRIBE_OK,
        ...
        // Section 10.2.12 FORWARD: "It MAY appear in SUBSCRIBE, REQUEST_UPDATE
        ...
        // message."
        0x20 => matches!(message, M::Subscribe | M::Fetch | M::RequestUpdate | M::RequestOk),
        // Section 10.2.9 SUBSCRIPTION FILTER: "It MAY appear in a SUBSCRIBE,
        // PUBLISH_OK or REQUEST_UPDATE (for a subscription) message."
        0x21 => matches!(message, M::Subscribe | M::RequestOk | M::RequestUpdate),
        // Section 10.2.8 GROUP ORDER: "It MAY appear in a SUBSCRIBE,
        // PUBLISH_OK, or FETCH."
        0x22 => matches!(message, M::Subscribe | M::RequestOk | M::Fetch),
        // Section 10.2.13 NEW GROUP REQUEST: "It MAY appear in PUBLISH_OK,
        // SUBSCRIBE or REQUEST_UPDATE for a subscription."
        0x32 => matches!(message, M::RequestOk | M::Subscribe | M::RequestUpdate),
        // Section 10.2.14 TRACK_NAMESPACE_PREFIX: "It MAY appear in
    ...
    fn decode_payload(msg_type: MessageType, buf: &mut impl Buf) -> Result<Self, CodecError> {
        match msg_type {
            ...
            MessageType::GoAway => {
                ...
                // Draft-18 Section 10.4: "The maximum length of the New Session
                ...
                let request_id = if buf.has_remaining() {
            ...
            MessageType::RequestError => {
                ...
                // Draft-18 Section 1.4.4: "The reason phrase length has a
            ...
            MessageType::PublishDone => {
                ...
                let reason_len = VarInt::decode_moqt::<Wire>(buf)?.into_inner() as usize;
                // Draft-18 Section 1.4.4, the same bound as REQUEST_ERROR above.
                if reason_len > MAX_REASON_PHRASE_LENGTH {
                    return Err(CodecError::ReasonPhraseTooLong);
                }
                let reason_phrase = read_bytes(buf, reason_len)?;
...
mod tests {
    ...
    /// Frame `body` as a draft-18 control message of `type_id`.
    ...
    /// Draft-18 Section 10.2.11: "The LARGEST_OBJECT parameter (Parameter Type
    ...
    /// (OBJECT_DELIVERY_TIMEOUT, type `0x02`, 5000ms as the four-byte varint
    ...
    /// Draft-18 Section 10.2.14: the TRACK_NAMESPACE_PREFIX parameter
    ...
    ///
    /// This one does not fail loudly, which is the reason it exists. With
    /// `0x34` back in the Length-prefixed arm the field count `0x02` is read as
    /// an outer length of two bytes, so the parameter comes out holding the
    /// first field's length byte and the letter `l` of "live" — a namespace
    /// silently replaced by two bytes of its own framing:
    ///
    /// ```text
    /// assertion `left == right` failed
    ///   left: [KeyValuePair { key: VarInt(52), value: Bytes([4, 108]) }]
    ///  right: [KeyValuePair { key: VarInt(52), value: Bytes([2, 4, 108, 105, 118,
    ///          101, 6, 115, 112, 111, 114, 116, 115]) }]
    /// ```
    ///
    /// [`an_empty_track_namespace_prefix_is_one_zero_byte`] fails the same way,
    /// with `Bytes([])` where the one-byte namespace should be.
    #[test]
