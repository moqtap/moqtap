...
use crate::draft09::endpoint::{Endpoint, EndpointError};
...
impl FramedSendStream {
    ...
    /// Draft-09 subgroup objects are stateless — no header-dependent bookkeeping.
...
impl FramedRecvStream {
    ...
    pub async fn read_subgroup_header(&mut self) -> Result<AnySubgroupHeader, ConnectionError> {
        ...
        loop {
            ...
            match AnySubgroupHeader::decode(DraftVersion::Draft09, &mut cursor) {
                Ok(header) => {
    ...
    pub async fn read_fetch_header(&mut self) -> Result<AnyFetchHeader, ConnectionError> {
        ...
        loop {
            let mut cursor = &self.buf[..];
            match AnyFetchHeader::decode(DraftVersion::Draft09, &mut cursor) {
                Ok(header) => {
                    let consumed = self.buf.len() - cursor.remaining();
                    self.buf.advance(consumed);
                    return Ok(header);
