...
use crate::draft11::endpoint::{Endpoint, EndpointError};
use crate::draft11::event::{ClientEvent, Direction, FetchObject, StreamKind, SubgroupObject};
use crate::draft11::observer::ConnectionObserver;
use crate::draft11::session::request_id::Role;
use crate::draft11::session::setup;
...
use moqtap_codec::draft11::message::ControlMessage;
...
pub enum ConnectionError {
    ...
    /// A control message this build decoded for draft-11 and then could not
    /// narrow to draft-11's own message type.
    ///
    ...
    #[error(
        "a control message decoded for draft-11 did not narrow to draft-11: a defect in this          build, and evidence about nothing the peer did"
    )]
...
pub struct ClientConfig {
    /// Additional draft versions to offer in CLIENT_SETUP (draft-11 is always
    /// offered first).
...
impl ClientConfig {
    ...
    /// Draft-11 first, then any additional versions.
    pub fn supported_versions(&self) -> Vec<VarInt> {
        let mut versions = vec![DraftVersion::Draft11.version_varint()];
    ...
    pub fn alpn(&self) -> Vec<Vec<u8>> {
        match &self.transport {
            TransportType::Quic => vec![DraftVersion::Draft11.quic_alpn().to_vec()],
...
/// extension block on each object. The second has to be remembered rather than
/// guessed: a header type whose Extensions Present column reads Yes puts an
/// Extension Headers Length on every object of the subgroup, so assuming
/// "absent" would write a stream a reader could not follow.
...
impl FramedSendStream {
    ...
    /// Append a draft-11 subgroup object (header + payload) to the stream.
    ///
    ...
    /// Append a draft-11 fetch object (header + payload) to the stream.
...
/// What an Object Status makes of an object, for Section 9.1.1.1's table.
///
...
pub struct FramedRecvStream {
    ...
    /// outside [`Connection::accept_subgroup_stream`] — and on such a stream
    /// the objects are read without being measured against the track at all.
...
impl FramedRecvStream {
    ...
    ///
    /// The whole of Section 9.1.1.1's rule that a single header cannot settle: the
    ...
    pub async fn read_control(
        ...
        // Draft-11 uses a fixed 16-bit length after the type id.
        ...
        // Now decode the whole message using the draft-11 dispatcher
        let mut frame = &self.buf[..total];
        let msg = AnyControlMessage::decode(DraftVersion::Draft11, &mut frame)?;
    ...
    pub async fn read_subgroup_header(&mut self) -> Result<AnySubgroupHeader, ConnectionError> {
        ...
        loop {
            ...
            match AnySubgroupHeader::decode_stream(DraftVersion::Draft11, &mut cursor) {
                Ok(header) => {
                    ...
                    match header {
                        AnySubgroupHeader::Draft11(ref d) => {
                            self.subgroup_has_extensions = d.stream_type.has_extensions();
                        }
                        // Only this draft's header sets the flag. With draft 11 the only enabled
    ...
    pub async fn read_fetch_header(&mut self) -> Result<AnyFetchHeader, ConnectionError> {
        ...
        loop {
            let mut cursor = &self.buf[..];
            match AnyFetchHeader::decode_stream(DraftVersion::Draft11, &mut cursor) {
                Ok(header) => {
                    let consumed = self.buf.len() - cursor.remaining();
                    self.buf.advance(consumed);
                    return Ok(header);
    ...
    /// Read the next draft-11 subgroup object (header + payload).
    ///
    ...
    /// Read the next draft-11 fetch object (header + payload).
...
impl Connection {
    ...
    ///
    /// `None` offers what `config` implies, which is what [`Self::adopt`]
    /// passes. `Some` replaces the list outright, and takes raw varints rather
    /// than [`DraftVersion`]s because the reason to reach for this is to offer
    /// a version no draft assigns — which an enum of drafts cannot name.
    ///
    /// A server MUST answer with a version the client offered and MUST
    /// otherwise close the session; from draft-11 the code for that is
    /// `VERSION_NEGOTIATION_FAILED` (0x15). How a relay spells the refusal is
    /// a conformance measurement, and offering a version deliberately outside
    /// the negotiable set is the only way to ask for it.
    pub async fn adopt_offering(
        transport: Transport,
        config: ClientConfig,
        versions: Option<Vec<VarInt>>,
    ) -> Result<Self, ConnectionError> {
        // PATH is for native QUIC only, and the transport is known here and
        // nowhere further in. Refusing before dialling means a session that
        // the server would close on sight is never opened.
        ...
        endpoint.connect()?;
        let setup_msg = endpoint.send_client_setup(
            versions.unwrap_or_else(|| config.supported_versions()),
            config.setup_parameters.clone(),
        )?;
        let any_setup = AnyControlMessage::Draft11(setup_msg);
        ...
        match &server_setup {
            AnyControlMessage::Draft11(ControlMessage::ServerSetup(ref ss)) => {
    ...
    /// only deduplication: both dials hand the same `QuicDialOptions` to the
    /// same config constructor in `transport::quic`, so the bundled roots and
    /// `config.ca_certs` are what each of them trusts and one relay gets one
    /// verdict whichever transport carries it. Settling trust at this call site
    /// instead — from `wtransport`'s own builder settings, or from a second
    /// config of this draft's own — puts the decision in two places, and a
    /// caller's private CA then reaches only the dials whose call site
    /// installed it.
    ...
    pub async fn send_control(&mut self, msg: &ControlMessage) -> Result<(), ConnectionError> {
        let any = AnyControlMessage::Draft11(msg.clone());
    ...
    pub async fn recv_control(&mut self) -> Result<ControlMessage, ConnectionError> {
        ...
        match any {
            AnyControlMessage::Draft11(msg) => Ok(msg),
            // `AnyControlMessage` carries one variant per enabled draft feature. With draft 11 the
    ...
    // ── Subscribe flow ──────────────────────────────────────
    /// The lowest Track Alias no live subscription on this connection holds.
    ///
    /// This draft makes the Track Alias the subscriber's to choose, so
    /// [`Self::subscribe`] takes one and every later-era caller has nothing to
    /// pass. See
    /// [`Endpoint::next_free_track_alias`](crate::draft11::endpoint::Endpoint::next_free_track_alias)
    /// for why the value is read off the endpoint rather than asked of the
    /// caller, and why picking one here stays correct alongside a caller that
    /// picks its own.
    pub fn next_free_track_alias(&self) -> VarInt {
        self.endpoint.next_free_track_alias()
    }
    ...
    pub async fn subscribe_ok(
        ...
        let msg = self.endpoint.send_subscribe_ok(request_id, expires, group_order, parameters)?;
        self.send_control(&msg).await
    }
    ...
    pub async fn subscribe_done(
        ...
        let msg = self.endpoint.send_subscribe_done(request_id, status_code, reason_phrase)?;
        self.send_control(&msg).await
    }
    ...
    /// subscription it names. Section 8.13.1 computes the range from "the
    /// Preceding Group Offset", which is this field under the name it carried
    /// one draft ago; the field list calls it Joining Start and says that for
    ...
    ///
    /// The subscription is named by `joining_subscribe_id`, which is the
    /// field's name in this draft even though the draft describes it as "The
    /// Request ID of the existing subscription to be joined".
    ...
    ///
    /// `joining_start` is the group to begin at. Section 8.13 has an Absolute
    ...
    /// group it wants can ask for it without first being told a Largest Group
    /// to count back from.
    ///
    ...
    pub async fn subscribe_announces(
        ...
        let (req_id, msg) = self.endpoint.subscribe_announces(track_namespace_prefix)?;
        self.send_control(&msg).await?;
        Ok(req_id)
    }
    ...
    pub async fn subscribe_announces_ok(
        ...
        let msg = self.endpoint.send_subscribe_announces_ok(request_id)?;
        self.send_control(&msg).await
    }
    ...
    pub async fn subscribe_announces_error(
        ...
        let msg =
            self.endpoint.send_subscribe_announces_error(request_id, error_code, reason_phrase)?;
        self.send_control(&msg).await
    }
    ...
    pub async fn announce(
        ...
        let (req_id, msg) = self.endpoint.announce(track_namespace)?;
        self.send_control(&msg).await?;
        Ok(req_id)
    }
    ...
    pub async fn track_status_request(
        ...
        let (req_id, msg) = self.endpoint.track_status_request(track_namespace, track_name)?;
        self.send_control(&msg).await?;
        Ok(req_id)
    }
    ...
    pub async fn recv_datagram(&self) -> Result<(AnyDatagramHeader, Bytes), ConnectionError> {
        ...
        let mut cursor = &data[..];
        let header = AnyDatagramHeader::decode(DraftVersion::Draft11, &mut cursor)?;
    ...
    ///
    /// Draft-11 adds none. Every variant of its [`ConnectionError`] is one of the
    /// ten every draft carries, and [`AnyConnectionError`] classifies those
    /// itself — so `None` here is this draft's answer rather than a stub, and it
    /// stays right for exactly as long as that list does.
    ///
    ...
    /// error, beside the doc comment that quotes the sentence it enforces,
    /// rather than as a silent [`ErrorCause::Unclassified`] in the facade.
    ///
    ...
    /// because the peer broke a rule draft-11 answers with a close.
    ///
    ...
    ///     Exist, Section 9.1.1.2. That one arrives on a data stream or a
    ...
    /// and states no consequence for exceeding it, so an oversized message is
    /// refused by the decoder and stops there.
    ///
    ...
    /// widening the match is not - the extension-header rule above is in this
    /// table because it has a variant of its own.
    pub fn codec_session_error_code(
        err: &CodecError,
    ) -> Option<moqtap_codec::draft11::error_codes::SessionErrorCode> {
        use moqtap_codec::draft11::error_codes::SessionErrorCode;
        ...
        match err {
            ...
            // Section 8.8: "Any other value is a protocol error and
            ...
            // session with a Protocol Violation". Draft-11 is where the field
            // arrives, and it carries only these two sites; PUBLISH and
            // PUBLISH_OK add two more from draft-12.
            ...
            //
            // This draft states the general rule alone. The Authorization Token
            // structure it defines gets its own sentence only from draft-12 on,
            // and a token that cannot be decoded is answered here either way.
            //
            ...
            // A Fetch Type outside the set this draft assigns. Section 8.13
            ...
            // The object payload rule, Section 9.1.1.1: "Any object with a status
    ...
    /// Close the session on the wire when a decode failure is one draft-11
...
mod tests {
    ...
    fn client_config_supported_versions_default() {
        ...
        assert_eq!(versions[0].into_inner(), 0xff000000 + 11);
    }
    ...
    fn client_config_alpn_quic() {
        ...
        assert_eq!(config.alpn(), vec![DraftVersion::Draft11.quic_alpn().to_vec()]);
    }
