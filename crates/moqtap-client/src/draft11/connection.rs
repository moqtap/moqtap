...
impl FramedSendStream {
    ...
    /// Draft-11 subgroup objects may carry extension headers but the writer
    /// path defers to the codec `ObjectHeader::encode` helper.
...
impl FramedRecvStream {
    ...
    pub async fn read_subgroup_header(&mut self) -> Result<AnySubgroupHeader, ConnectionError> {
        ...
        loop {
            ...
            match AnySubgroupHeader::decode(DraftVersion::Draft11, &mut cursor) {
                Ok(header) => {
    ...
    pub async fn read_fetch_header(&mut self) -> Result<AnyFetchHeader, ConnectionError> {
        ...
        loop {
            let mut cursor = &self.buf[..];
            match AnyFetchHeader::decode(DraftVersion::Draft11, &mut cursor) {
                Ok(header) => {
                    let consumed = self.buf.len() - cursor.remaining();
                    self.buf.advance(consumed);
                    return Ok(header);
    ...
    /// Read the next draft-11 subgroup object (header + payload). The codec
