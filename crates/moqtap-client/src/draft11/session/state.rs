...
impl SessionStateMachine {
    ...
    ///
    /// draft-11 Section 3.4: "The Transport Session can be terminated at any point."
    /// Section 8.3.1 obliges an endpoint to do so during the Setup exchange itself:
    ...
    /// peer MUST close the session with Version Negotiation Failed."
    /// That failure is reachable only from SetupExchange, so a session that cannot close
    /// there cannot record the one outcome the draft requires of it.
    ///
