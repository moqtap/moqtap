...
///
/// Draft-11 Section 8.1: "The client's Request ID starts at 0 and are even and
/// the server's Request ID starts at 1 and are odd. The Request ID increments
/// by 2 with ANNOUNCE, FETCH, SUBSCRIBE, SUBSCRIBE_ANNOUNCES or TRACK_STATUS
...
/// and closes the session with 'Too Many Requests' on a request ID "equal or
/// larger than this". Section 8.3.2.2 gives the setup parameter of the same
/// name a default of 0 and reads that as "the peer MUST NOT send requests",
/// which only holds if a ceiling of 0 forbids the ID 0 as well.
