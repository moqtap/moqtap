...
/// Allocates and validates request IDs per the MoQT draft-11 spec.
///
/// Draft-11 renames the subscribe-id space to the request-id space but keeps
/// the same monotonic allocation rules. The subscriber allocates IDs
/// monotonically starting at 0; FETCH and SUBSCRIBE share the same namespace
/// since each carries a `request_id` field.
