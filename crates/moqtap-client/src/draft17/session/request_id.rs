...
impl RequestIdAllocator {
    ...
    pub fn new(role: Role) -> Self {
        ...
        // Draft-17 removed MAX_REQUEST_ID; allocator is never blocked.
    ...
    ///
    /// Draft-17 Section 9.1: "The client generates even numbered Request IDs,
    ...
    ///   MAX_REQUEST_ID. Draft-17 removed MAX_REQUEST_ID and
    ///   [`RequestIdAllocator::new`] sets the ceiling to `u64::MAX`, so on
    ///   this draft the check cannot fire for any id a varint can carry.
...
mod tests {
    ...
    /// The error names the peer, not the endpoint that caught it. Draft-17
    /// Section 9.1 makes this a session close, so the reason phrase a peer
    ...
    /// Draft-17 removed MAX_REQUEST_ID, so the ceiling check in
    /// `validate_peer_id` is unreachable on this draft: the largest id a MoQT
    /// varint can carry is still accepted.
    #[test]
