...
pub enum ConnectionError {
    ...
    /// A control message this build decoded for draft-14 and then could not
    /// narrow to draft-14's own message type.
    ///
    ...
    #[error(
        "a control message decoded for draft-14 did not narrow to draft-14: a defect in this          build, and evidence about nothing the peer did"
    )]
...
impl FramedSendStream {
    ...
    /// forbids one, or a status the draft leaves unassigned.
            ...
            match AnySubgroupHeader::decode(self.draft, &mut cursor) {
                Ok(header) => {
                    ...
                    match header {
                        ...
                        // Only this draft's header seeds the object reader. With draft 14 the only
...
impl Connection {
    ...
    ///
    /// Draft-14 adds none. Every variant of its [`ConnectionError`] is one of the
