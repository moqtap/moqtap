...
use crate::draft15::fetch::{FetchError, FetchState, FetchStateMachine};
...
use crate::draft15::subscription::{
    SubscriptionError, SubscriptionState, SubscriptionStateMachine,
};
use crate::draft15::track_status::{TrackStatusError, TrackStatusState, TrackStatusStateMachine};
...
use moqtap_codec::draft15::error_codes::{RequestErrorCode, SessionErrorCode};
...
pub enum EndpointError {
    ...
    ///
    /// Draft-15 states it twice, once per message. Section 9.10: "The same
    ...
    /// preferences within a single track (see Section 2.4.2)."
    ///
    ...
    /// differently. Section 2.4.2 lists "An Object is received with a
    /// different Forwarding Preference than previously observed from the same
    /// Track" among the conditions that make a track malformed, and says of
    /// all of them: "When a subscriber detects a Malformed Track, it MUST
    /// UNSUBSCRIBE any subscription and FETCH_CANCEL any fetch for that Track
    ...
    /// for the same sentence: Section 2.4.2 answers its whole list at
    ...
    /// request under.
    ///
    /// Section 9.2.1.7: "If the length of the Subscription Filter does not match
    /// the parameter length, the publisher MUST close the session with
    /// PROTOCOL_VIOLATION."
    ///
    ...
    /// MUST return a REQUEST_ERROR with error code INVALID_JOINING_REQUEST_ID".
    ///
    ...
    /// announced.
    ///
    /// Section 9.22 says what a cancellation is for: the subscriber "will stop
    ...
    ///
    /// Section 9.23: "A subscriber cannot make overlapping namespace
...
impl EndpointError {
    /// The code to close the session with, when draft-15 answers this error
...
pub struct Endpoint {
    ...
    ///
    /// The lock is what lets the data plane end one. Section 2.4.2's answer to a
...
impl Endpoint {
    ...
    /// report Section 2.4.2's Malformed Track when it arrived after the
    ...
    pub fn subscribe_update(
        ...
        self.require_active_or_err()?;
        let id = subscription_request_id.into_inner();
    ...
    pub fn receive_subscribe_update(&mut self, msg: &SubscribeUpdate) -> Result<(), EndpointError> {
        ...
        // Section 9.11 states the close: "This MUST match an existing Request
        // ID", and the publisher must end the session with PROTOCOL_VIOLATION
        // "if the subscriber specifies an invalid Subscription Request ID". The
        // two halves are quoted apart because the rendering carries a stray
        // backtick in front of the code name, so the sentence it holds is not
        // one anybody wrote. Existence is the only test it states, which is
        // wider than *no subscription the peer opened*: every request this
        // session has carried has existed. Reading the field name as the test
        // instead - that the identifier must be a SUBSCRIBE's - is contradicted
        // in the same section, which leaves a parameter "previously set on the
        // subscription in SUBSCRIBE, PUBLISH_OK or SUBSCRIBE_UPDATE" unchanged
        // when an update omits it: a subscription established by PUBLISH is
        // updatable and its identifier is a PUBLISH's. Only a subscription the
        // peer opened has a transition for an update, so for the rest the
        // message is accepted and the state left alone. Section 9.19 takes the
        // update away from this request kind outright: "the subscriber cannot
        // send SUBSCRIBE_UPDATE or UNSUBSCRIBE". A track status the peer asked
        // for and one this endpoint asked for are the same kind of request and
        // neither is a subscription, so both are refused. The check comes
        // before the set below because an identifier naming a track status has
        // existed, and being told so is what would let it through.
    ...
    /// made, a track status it asked for and a namespace it subscribed to all
    /// end here.
    ///
    ...
    pub fn send_request_error(
        ...
        } else if let Some(sub) = self.inbound_subscribe_namespaces.get_mut(&id) {
    ...
    pub fn fetch(
        ...
        // Before the two of them are moved into the message, which is where
        // they used to go and stay: a fetch that has left no track behind is
        // one no withdrawal can find.
    ...
    /// subscription" is the one a Joining FETCH uses. The drafts before this
    /// one say "an existing Subscribe" and reach only what a SUBSCRIBE opened.
    ...
    ///
    /// Section 9.23 addresses the first half of the overlap rule to this end
    ...
    ///
    /// Section 9.23: "The subscriber sends the SUBSCRIBE_NAMESPACE control
    ...
    pub fn receive_subscribe_namespace(
        ...
        self.require_active_or_err()?;
        let overlaps = self.peer_prefix_overlap(&msg.namespace_prefix);
    ...
    fn peer_prefix_overlap(&self, prefix: &TrackNamespace) -> Option<u64> {
        self.inbound_subscribe_namespaces
            ...
            .filter_map(|(&id, s)| {
                prefixes_overlap(&s.message.namespace_prefix.0, &prefix.0).then_some(id)
            })
            .min()
    }
    ...
    /// SUBSCRIBE_NAMESPACE."
    ///
    /// Section 9.24 names what it carries: "Request ID: The Request ID of the
    /// SUBSCRIBE_NAMESPACE", so the record it ends is found by identifier and
    /// not by prefix.
    ///
    /// # Errors
    ///
    ...
    /// tracks within the provided Track Namespace." and Section 9.22 says what
    ...
    /// used", which is why accepting one binds nothing. Section 6.1 gives a
    /// namespace subscription the same answer: "A publisher MUST send exactly
    /// one REQUEST_OK or REQUEST_ERROR in response to a SUBSCRIBE_NAMESPACE."
    ///
    ...
    /// previously responded REQUEST_OK to". Section 9.22 says what it does: the
    ...
    pub fn send_publish_error(
        ...
        Ok(ControlMessage::RequestError(RequestError { request_id, error_code, reason_phrase }))
    }
