...
impl SubscribeNamespaceStateMachine {
    ...
    /// REQUEST_OK).
    ///
    /// Section 6.1: "A publisher MUST send exactly one REQUEST_OK or
    /// REQUEST_ERROR in response to a SUBSCRIBE_NAMESPACE."
    ///
    ...
    /// reaches by being answered REQUEST_OK and no other way, so a withdrawal
    /// of one never answered is refused here.
