...
/// carries the objects. Section 9.17: "A publisher MAY send Objects in response
/// to a FETCH before the FETCH_OK message is sent, but the FETCH_OK MUST NOT be
/// sent until the End Location is known."
///
/// Section 9.16.3 states the other direction of the same freedom: "The FETCH_OK
/// or REQUEST_ERROR can come at any time relative to object delivery." So the
...
impl FetchStateMachine {
    ...
    /// early leave one behind. Section 9.18: the publisher of a cancelled fetch
    /// "SHOULD promptly close the unidirectional stream, even if it is in the
    /// middle of delivering an object". Section 9.16.3: a relay whose upstream
    /// FETCH failed "sends a REQUEST_ERROR and can reset the unidirectional
...
impl FetchStateMachine {
    ...
    /// Pending | Unanswered -> Done (REQUEST_ERROR sent).
