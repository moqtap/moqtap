...
use crate::draft12::fetch::{FetchError, FetchStateMachine};
use crate::draft12::namespace::{
    AnnounceStateMachine, NamespaceError, SubscribeAnnouncesStateMachine,
};
use crate::draft12::session::request_id::{RequestIdAllocator, RequestIdError};
...
use crate::draft12::subscription::{SubscriptionError, SubscriptionStateMachine};
use crate::draft12::track_status::{TrackStatusError, TrackStatusStateMachine};
...
impl Endpoint {
    /// Create a new draft-12 endpoint.
    pub fn new() -> Self {
        Self {
    ...
    /// varint filter-type discriminant (see the draft-12 codec for
    /// definitions). `LargestObject` (2) is a reasonable default.
    ///
    /// Draft-12 note: the track alias is no longer chosen by the subscriber
