...
use crate::draft12::endpoint::{Endpoint, EndpointError};
use crate::draft12::event::{ClientEvent, Direction, FetchObject, StreamKind, SubgroupObject};
use crate::draft12::observer::ConnectionObserver;
use crate::draft12::session::request_id::Role;
use crate::draft12::session::setup;
...
use moqtap_codec::draft12::message::ControlMessage;
...
/// Errors from the draft-12 connection layer.
...
pub enum ConnectionError {
    ...
    /// A control message this build decoded for draft-12 and then could not
    /// narrow to draft-12's own message type.
    ///
    ...
    #[error(
        "a control message decoded for draft-12 did not narrow to draft-12: a defect in this          build, and evidence about nothing the peer did"
    )]
...
/// Configuration for a draft-12 MoQT client connection.
pub struct ClientConfig {
    /// Additional draft versions to offer in CLIENT_SETUP (draft-12 is always
    /// offered first).
...
impl ClientConfig {
    ...
    pub fn supported_versions(&self) -> Vec<VarInt> {
        let mut versions = vec![DraftVersion::Draft12.version_varint()];
    ...
    pub fn alpn(&self) -> Vec<Vec<u8>> {
        match &self.transport {
            TransportType::Quic => vec![DraftVersion::Draft12.quic_alpn().to_vec()],
...
impl FramedSendStream {
    ...
    /// Append a draft-12 subgroup object (header + payload) to the stream.
    ///
    ...
    /// Append a draft-12 fetch object (header + payload) to the stream.
...
impl FramedRecvStream {
    ...
    pub async fn read_control(
        ...
        // Now decode the whole message using the draft-12 dispatcher
        let mut frame = &self.buf[..total];
        let msg = AnyControlMessage::decode(DraftVersion::Draft12, &mut frame)?;
    ...
    pub async fn read_subgroup_header(&mut self) -> Result<AnySubgroupHeader, ConnectionError> {
        ...
        loop {
            ...
            match AnySubgroupHeader::decode_stream(DraftVersion::Draft12, &mut cursor) {
                Ok(header) => {
                    ...
                    match header {
                        AnySubgroupHeader::Draft12(ref d) => {
    ...
    pub async fn read_fetch_header(&mut self) -> Result<AnyFetchHeader, ConnectionError> {
        ...
        loop {
            let mut cursor = &self.buf[..];
            match AnyFetchHeader::decode_stream(DraftVersion::Draft12, &mut cursor) {
                Ok(header) => {
                    let consumed = self.buf.len() - cursor.remaining();
                    self.buf.advance(consumed);
                    return Ok(header);
...
impl Connection {
    ...
    pub async fn adopt_offering(
        ...
        let any_setup = AnyControlMessage::Draft12(setup_msg);
        ...
        match &server_setup {
            AnyControlMessage::Draft12(ControlMessage::ServerSetup(ref ss)) => {
    ...
    pub async fn send_control(&self, msg: &ControlMessage) -> Result<(), ConnectionError> {
        let any = AnyControlMessage::Draft12(msg.clone());
        let mut send =
    ...
    pub async fn recv_control(&mut self) -> Result<ControlMessage, ConnectionError> {
        ...
        match any {
            AnyControlMessage::Draft12(msg) => Ok(msg),
            // `AnyControlMessage` carries one variant per enabled draft feature. With draft 12 the
    ...
    pub async fn subscribe_announces(
        ...
        let (req_id, msg) =
            self.endpoint.subscribe_announces(track_namespace_prefix, parameters)?;
        self.send_control(&msg).await?;
        Ok(req_id)
    }
    ...
    pub async fn track_status_request(
        ...
        let (req_id, msg) =
            self.endpoint.track_status_request(track_namespace, track_name, parameters)?;
        self.send_control(&msg).await?;
        Ok(req_id)
    }
    ...
    pub async fn recv_datagram(&self) -> Result<(AnyDatagramHeader, Bytes), ConnectionError> {
        ...
        let mut cursor = &data[..];
        let header = AnyDatagramHeader::decode(DraftVersion::Draft12, &mut cursor)?;
    ...
    ///
    /// Draft-12 adds none. Every variant of its [`ConnectionError`] is one of the
    ...
    /// because the peer broke a rule draft-12 answers with a close.
    ///
    ...
    /// widening the match is not - the extension-header rule above is
    /// answerable here because it has a variant of its own rather than being
    /// one more reading of `InvalidField`.
    pub fn codec_session_error_code(
        err: &CodecError,
    ) -> Option<moqtap_codec::draft12::error_codes::SessionErrorCode> {
        use moqtap_codec::draft12::error_codes::SessionErrorCode;
    ...
    /// Close the session on the wire when a decode failure is one draft-12
...
mod tests {
    ...
    fn client_config_supported_versions_default() {
        ...
        assert_eq!(versions[0].into_inner(), 0xff000000 + 12);
    }
    ...
    fn client_config_alpn_quic() {
        ...
        assert_eq!(config.alpn(), vec![DraftVersion::Draft12.quic_alpn().to_vec()]);
    }
