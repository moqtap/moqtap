//! Location filters and fill fetch streams, the two things draft-20 asks a
...
//! places draft-20 is least explicit, and getting either wrong desynchronises
...
//! entire Group". Draft-20 Sections 5.1.2 and 10.13 both say the Location
...
///
/// Spelled here rather than imported because the codec's draft-20 message
...
pub enum FillError {
    ...
    ///
    /// Section 10.2.15: "An endpoint that receives a parameter inside
...
///
/// Section 5.1.4 lets the Range Filters "appear multiple times", and Section
/// 10.2 forbids a repeat of anything else. A repeat is encoded as a `Type
...
///
/// `SUBSCRIBER_PRIORITY` (0x20, Section 10.2.7) and `GROUP_ORDER` (0x22,
/// Section 10.2.8) are the two uint8s that may be nested.
...
///
/// Each constructor names one row of the Section 5.1.2 table, so a caller
...
impl LocationFilter {
    ...
    ///
    /// Section 5.1.2 gives this one job beyond meaning "unfiltered": in a
    ...
    ///
    /// Section 5.1.3 names this filter as half of the recipe for
...
/// both in the message and inside FILL_PARAMETERS." Section 10.2 defines `Type
...
impl FillParameters {
    ...
    /// in.
    ///
    /// Section 10.2.8: "When it appears inside FILL_PARAMETERS, it governs the
    ...
    /// Section 11.4.4.1 makes a Group ID Delta count upward under Ascending and
    ...
    pub fn parameter(&self) -> Result<KeyValuePair, FillError> {
        ...
        // Section 10.2: "Parameters MUST be serialized in ascending order by
...
fn read_group_order(parameters: &[KeyValuePair]) -> Result<Option<GroupOrder>, FillError> {
    ...
    let KvpValue::Varint(value) = &parameter.value else {
        return Err(FillError::MalformedValue {
            key: GROUP_ORDER,
            detail: "bytes where Section 10.2.8 defines a uint8",
        });
    };
    match value.into_inner() {
        ...
        // pair a caller built in memory. Section 10.2.8: "If an endpoint
        ...
        _ => Err(FillError::MalformedValue {
            key: GROUP_ORDER,
            detail: "Section 10.2.8 allows only Ascending (0x1) and Descending (0x2)",
        }),
    }
}
...
///
/// A fill fetch stream is "delivered as a FETCH response" (Section 5.1.3), and
/// a FETCH response's Group ID Deltas are read against a Group Order that is
/// nowhere on the data stream — Section 11.4.4.1 makes a delta count upward
...
/// [`accept_fill_stream`](crate::draft20::connection::Connection::accept_fill_stream)
/// already does.
///
/// Three steps, in the order the two sections put them:
///
/// 1. A `GROUP_ORDER` **inside** `FILL_PARAMETERS`. Section 10.2.8: "When it
...
/// 2. Otherwise the `GROUP_ORDER` on the request itself. Section 5.1.3: "The
...
///
/// Section 10.2.8 gives two different defaults and a fill sits between them:
...
pub fn group_order(parameters: &[KeyValuePair]) -> Result<GroupOrder, FillError> {
    if let Some(fill) = parameters.iter().find(|p| p.key.into_inner() == FILL_PARAMETERS) {
        let KvpValue::Bytes(value) = &fill.value else {
            return Err(FillError::MalformedValue {
                key: FILL_PARAMETERS,
                detail: "a bare varint where Section 10.2.15 defines a parameter block",
            });
        };
...
mod tests {
    ...
    /// Every shape Section 5.1.2 defines, and no others. The field count is
    ...
    /// Section 5.1.2 answers the overflow with a session close, so it is
    ...
    /// worth having on the record: Section 10.2.21 makes it govern the **Track
    /// Properties** in an OK message, and a fill fetch stream has no OK message
    /// at all (Section 5.1.3.1 — no FETCH_OK, no REQUEST_ERROR). It is absent
    ...
    /// 0x20 (Section 11.4.4.1) and by nothing else. `INCLUDE_PROPERTIES` does
    ...
    /// 5.1.3 means by "parameters carried inside FILL_PARAMETERS override them
    ...
    /// With no `GROUP_ORDER` in either scope, Ascending. Section 10.2.8 gives a
