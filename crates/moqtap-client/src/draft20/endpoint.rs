...
//!   Fetch structures (Section 10.13). Track Namespace and Track Name are
...
//! * **Fill fetch streams replace the joining fetch** (Sections 5.1.3 and
//!   5.1.3.1). A SUBSCRIBE or REQUEST_UPDATE carrying `FILL_PARAMETERS` (0x23)
...
//! * **PUBLISH_STATE_NOTIFY (0x22) is new** (Section 10.10): unilateral, from
//!   the publisher, on a subscription's stream, answered with nothing and
//!   explicitly outside the `MAX_REQUEST_UPDATES` ceiling.
//! * **Ranges are inclusive at both ends** (Sections 5.1.2, 10.13, 10.14), so
...
use crate::draft20::fetch::{FetchError, FetchState, FetchStateMachine};
use crate::draft20::fill::{self, FillError, LocationFilter};
use crate::draft20::namespace::{
    NamespaceError, PublishNamespaceState, PublishNamespaceStateMachine, SubscribeNamespaceState,
    SubscribeNamespaceStateMachine,
};
use crate::draft20::publish::{
    PublishError as PublishFlowError, PublishState, PublishStateMachine,
};
use crate::draft20::session::request_id::{RequestIdAllocator, RequestIdError, Role};
use crate::draft20::session::setup::{self, SetupError};

use moqtap_codec::draft20::data_stream::GroupOrder;
...
///
/// A Range Filter whose value is empty is Section 5.1.4's removal form — "In
...
use crate::draft20::session::state::{SessionError, SessionState, SessionStateMachine};
use crate::draft20::subscription::{
    SubscriptionError, SubscriptionState, SubscriptionStateMachine,
};
use crate::draft20::track_status::{TrackStatusError, TrackStatusState, TrackStatusStateMachine};
...
use moqtap_codec::draft20::error_codes::{
...
pub enum EndpointError {
    ...
    /// to build is one draft-20 does not define. Nothing was written.
    ...
    UnknownRequest(u64),
    #[error(
        "response message received on control stream; draft-20 responses belong on bidi request streams"
    )]
    ResponseOnControlStream,
    /// A REQUEST_UPDATE arrived on the control stream.
    ///
    /// Draft-20 Table 5 gives REQUEST_UPDATE the Stream value "Request", and
    ...
    /// stream.
    ///
    /// Draft-20 Table 5 gives all three the Stream value "Request": NAMESPACE
    /// (0x8, Section 10.17) and NAMESPACE_DONE (0xE, Section 10.18) belong on
    /// the SUBSCRIBE_NAMESPACE request stream whose namespace they report, and
    /// PUBLISH_SKIPPED (0xF, Section 10.21) on the SUBSCRIBE_TRACKS stream
    ...
    #[error("{0} received on the control stream; draft-20 Table 5 places it on a request stream")]
    RequestMessageOnControlStream(&'static str),
    /// A REQUEST_UPDATE named a request that cannot be updated, or none.
    ///
    /// Draft-20 Section 10.9: "An endpoint that receives a REQUEST_UPDATE
    /// other than in the two cases above MUST close the session with a
    /// PROTOCOL_VIOLATION." TRACK_STATUS is called out in Section 10.15 as one
    ...
    /// TRACK_STATUS.
    ///
    /// Draft-20 Section 10.5: they "are empty in PUBLISH_OK,
    ...
    ///
    /// Draft-20 Section 10.6.1: "If a server receives a Redirect with a
    ...
    ///
    /// Draft-20 Section 10.6.1: "Track Name is not meaningful for
    ...
    ///
    /// Draft-20 Section 10.4: "If a server receives a GOAWAY with a non-zero
    ...
    ///
    /// Draft-20 Sections 10.19 and 10.20, of SUBSCRIBE_NAMESPACE and
    ...
    ///
    /// Draft-20 Section 10.3.1.7: "If an endpoint receives a REQUEST_UPDATE on
    ...
    /// Range Filter rule in Section 5.1.4 is answered with a REQUEST_ERROR, and
    ...
    ///
    /// Draft-20 Section 11.1: "The same Track Alias MUST NOT be used by a
    ...
    /// subscription.
    ///
    /// Section 10.10: "PUBLISH_STATE_NOTIFY applies only to subscriptions, and
    ...
    /// prevents is handing a conforming peer a message Section 10.10 tells it to
    ...
    ///
    /// Section 5.1.3 makes `FILL_PARAMETERS` the whole of the request: "Its
    ...
    ///
    /// Not a session close. Draft-20 states no rule for it — a fill fetch
    ...
    /// `STOP_SENDING` on that stream alone, which Section 5.1.3.1 gives the
    ...
    ///
    /// Section 10.20: "Within a session, if a publisher receives a
...
///
/// Section 10.2.20: "The TRACK_NAMESPACE_PREFIX parameter (Parameter Type
...
impl EndpointError {
    ...
    /// The code to close the session with, when draft-20 says this error is
    ...
    pub fn session_error_code(&self) -> Option<SessionErrorCode> {
        match self {
            ...
            // Section 10.10 names the code in the sentence that states the
...
pub struct Endpoint {
    ...
    /// fill fetch stream's FETCH_HEADER carries — Section 5.1.3: "the SUBSCRIBE
    ...
    /// stream under that request's Request ID (Section 10.9). So the two ids
    ...
    /// 10.2.15 keeps `FILL_PARAMETERS` off the sticky-parameter rules, but a
    ...
    ///
    /// Draft-20 Section 10.1 makes a duplicate Request ID a session close, and
    ...
    /// which this type has no path for: draft-20 gives the endpoint no
    ...
    ///
    /// Section 10.3.1.6 makes it a ceiling on the peer's total number of Ranges
    ...
    /// requests.
    ///
    /// Section 5.1.4 makes the ceiling a property of the request rather than of
    ...
    /// Draft-20 Section 10.13 deleted the joining fetch; a fill fetch stream
...
/// FETCH_OK and no REQUEST_ERROR (Section 5.1.3.1), so it has none of the
...
struct FillStreams {
    ...
    /// not.
    ///
    /// Section 10.12 makes the count a PUBLISH_DONE field: Stream Count is the
    ...
    /// Group ID Deltas count in (Section 11.4.4.1) and the answer is in a
    ...
    /// [`begin_fetch_objects`](crate::draft20::connection::FramedRecvStream::begin_fetch_objects).
...
impl Endpoint {
    ...
    /// been answered yet.
    ///
    /// Draft-20 Section 10.9: "A subscriber can also send REQUEST_UPDATE to
    ...
    ///
    /// Draft-20 Section 10.9.1 gives a refused REQUEST_UPDATE three different
    ...
    pub fn receive_goaway(&mut self, msg: &GoAway) -> Result<(), EndpointError> {
        // Draft-20 Section 10.4: "If a server receives a GOAWAY with a
        ...
        // Draft-20 Section 10.4: "The endpoint MUST close the session with a
    ...
    pub fn subscribe(
        ...
        // Section 10.2.15: the presence of FILL_PARAMETERS "is what requests a
    ...
    /// Process an incoming SUBSCRIBE_OK. Draft-20: no request_id on wire; the
    ...
    ///
    /// Draft-20 Section 10.9: "The sender of a request (SUBSCRIBE, PUBLISH,
    ...
    /// of all, since Section 10.15 says outright that "the subscriber cannot
    /// send REQUEST_UPDATE".
    ///
    ...
    pub fn receive_request_update(
        ...
        *self.unanswered_peer_updates.entry(id).or_insert(0) += 1;
        ...
        // Section 5.1.3.1: "A publisher opens a fill fetch stream when it
        ...
        // (Section 10.2.20) in a REQUEST_UPDATE."
        //
    ...
    /// request stream, new in draft-20 (Section 10.10).
    ///
    ...
    /// subscriber. Section 10.10 answers both with a session close, and both
    ...
    pub fn receive_publish_state_notify(
        ...
        let id = request_id.into_inner();
        // The two ways a subscription exists, from Section 5.1: this endpoint
    ...
    ///
    /// Draft-20 Section 10.13 rebuilt the message. The `Fetch Type` field, the
    ...
    /// (Section 5.1.2) rather than in the message. So this takes the same three
    ...
    /// worth a method because Section 10.2 makes that order a wire rule and the
    /// codec's encoder refuses a descending pair rather than emitting one.
    ///
    /// **The range is inclusive at both ends** (Sections 5.1.2 and 10.13), and
    ...
    // -- Fill fetch streams -----------------------------------------
    //
    // Draft-20 Sections 5.1.3 and 5.1.3.1, new in this draft, and what replaced
    ...
    /// Section 5.1.3 puts the *subscription's* Request ID on a fill fetch
    ...
    /// ended or not.
    ///
    /// Section 10.12 makes PUBLISH_DONE's Stream Count the total the publisher
    ...
    ///
    /// What [`accept_fill_stream`](crate::draft20::connection::Connection::accept_fill_stream)
    ...
    ///
    /// Section 5.1.3: "a subscription can have multiple fill fetch streams open
    ...
    ///
    /// One entry point for both endings on purpose. Section 5.1.3.1 gives them
    ...
    /// `request_id`. Draft-20: PUBLISH_OK is a REQUEST_OK alias, so this
    /// handler also resolves outstanding PUBLISH requests.
    ///
    /// # Track Properties are refused except on TRACK_STATUS_OK
    ///
    /// Draft-20 Section 10.5: Track Properties "are populated in
    ...
    /// been written yet.
    ///
    /// [`Connection::cancel_request_stream`]: crate::draft20::connection::Connection::cancel_request_stream
    ...
    ///
    /// Draft-20 Table 5 gives every message a Stream value, and only two of
    ...
    /// (Sections 10.17 and 10.18), and PUBLISH_SKIPPED names a track that will
    /// not be published on the SUBSCRIBE_TRACKS stream that asked for it
    /// (Section 10.21). All four route through
    ...
    pub fn receive_message(&mut self, msg: ControlMessage) -> Result<(), EndpointError> {
        match msg {
            ...
            // Section 10.10 puts PUBLISH_STATE_NOTIFY on "a subscription's
            ...
            //
            // Not the close Section 10.10 names, and the difference is worth
    ...
    ///
    /// Sections 10.19 and 10.20 state it once each, for SUBSCRIBE_NAMESPACE
    ...
    /// by `request_id`.
    ///
    /// Beyond the five responses this also takes the messages draft-20 Table 5
    ...
    /// (Section 10.4); draft-19 removed GOAWAY's Request ID and left one wire
    /// form for both places, and draft-20 keeps it that way.
    ///
    ...
    /// one message here that is neither a response nor a request: Section 10.10
    ...
    pub fn receive_response_on_stream(
        ...
        match msg {
            ...
            // neither a response nor a request: Section 10.10 puts it on "a
    ...
    /// control stream.
    ///
    /// Draft-20 Section 10.4: "A GOAWAY MAY also be sent on a request stream
    ...
    pub fn receive_request_on_stream(
        ...
        match msg {
            ControlMessage::Subscribe(m) => {
                // Section 10.2.15: the presence of FILL_PARAMETERS "is what
            ...
            // a REQUEST_ERROR this endpoint had to owe; draft-20 Section 10.13
            ...
            // it one: Section 10.20 makes SUBSCRIBE_TRACKS and
            ...
            ControlMessage::SubscribeTracks(m) => {
                ...
                // Section 10.2.15 lists SUBSCRIBE and REQUEST_UPDATE and not
    ...
    /// 10.2.20 weighs against "another active subscription of the same type"
    ...
    /// the traffic today, since draft-20 is the first draft with these
    /// parameters.
    ...
    fn answer_an_update(&mut self, id: u64, msg: &ControlMessage) -> Result<(), EndpointError> {
        ...
        // the sentence names are known. Section 10.2.20: "If the new prefix
    ...
    ///
    /// Beyond the four responses this also takes the three messages draft-20
    ...
    /// (Sections 10.17 and 10.18) and PUBLISH_SKIPPED on a SUBSCRIBE_TRACKS
    /// stream (Section 10.21). All three require the request to have been
    ...
    pub fn send_response_on_stream(
        ...
        match msg {
            ...
            // Section 10.10, on the way out. Nothing moves: the message is
            ...
            ControlMessage::RequestError(_) => {
                if let Some(sm) = self.subscriptions.get_mut(&id) {
                    ...
                    // live subscription. Draft-20 Section 10.13 deleted the
                    // joining mechanism and Section 15.11.2 unassigned the code,
    ...
    ///
    /// REQUEST_UPDATE, because draft-20 Section 10.9 puts it on the request's
    ...
    /// TRACK_STATUS, by Section 10.15 — closes the session.
    ///
    ...
    ///
    /// PUBLISH_STATE_NOTIFY, new in draft-20 (Section 10.10), for the same
    ...
    pub fn receive_on_peer_request_stream(
        ...
        match msg {
            ...
            // from the end Section 10.10 permits. On a stream the peer opened
...
impl PublishStateMachine {
    ...
    ///
    /// Draft-18 folded PUBLISH_OK into REQUEST_OK and draft-20 keeps it that
...
impl SubscribeNamespaceStateMachine {
    ...
    /// SUBSCRIBE_NAMESPACE request", and Section 10.19 has the publisher send
    ...
    ///
    /// Section 10.18: "All NAMESPACE_DONE messages are in response to a
...
mod responder_tests {
    use super::*;
    use crate::draft20::namespace::SubscribeNamespaceState;
    use crate::draft20::publish::PublishState;
    use crate::draft20::subscription::SubscriptionState;
    use moqtap_codec::kvp::KvpValue;
    ...
    /// Draft-20 Section 10.1: a Request ID whose least significant bit is
    ...
    /// Draft-20 Section 3.3 gives a different code for a different rule: a
    ...
    /// draft-20 Section 10.9 puts it on its request's own stream, and the
    ...
    ///
    /// Section 10.20 keeps its overlap space independent of
    ...
    fn a_peers_subscribe_tracks_is_a_request_of_its_own() {
        ...
        // Section 10.21 puts PUBLISH_SKIPPED on this stream, and only on this
    ...
    /// Draft-20 Table 5 gives NAMESPACE and NAMESPACE_DONE the Stream value
    /// "Request", and Section 10.19 has the publisher send them only once the
    ...
    /// Draft-20 Section 10.5 answers Track Properties on a REQUEST_OK that is
...
mod tests {
    ...
    /// which is the behaviour draft-20 Section 10.9 forbids.
    ...
    /// The other answer, and the one this file gives to a rule draft-20
    ...
    /// bidirectional stream may *begin* with — and no sentence in draft-20
    ...
    fn assert_refused_without_closing(ep: &mut Endpoint, err: &EndpointError) {
        assert_eq!(
            err.session_error_code(),
            None,
            "no sentence in draft-20 answers {err} with a close"
        );
        assert_eq!(ep.session_state(), SessionState::Active);
        ep.subscribe(ns("a"), b"b".to_vec(), vec![])
    ...
    /// The three messages draft-20 Table 5 places on a request stream that are
    ...
    fn namespace_and_publish_skipped_belong_on_a_request_stream() {
        ...
        for (name, build) in cases {
            ...
            // Sections 10.19 and 10.20 make REQUEST_OK or REQUEST_ERROR the
    ...
    /// Draft-20 Section 10.9: "An endpoint that receives a REQUEST_UPDATE
    /// other than in the two cases above MUST close the session with a
    /// PROTOCOL_VIOLATION." Section 10.15 names TRACK_STATUS as one such case:
    ...
    /// Draft-20 Section 10.9's first case reaches every request kind: "The
    ...
    /// Draft-20 Section 10.13 deleted the `Fetch Type` field, both variant
    ...
    fn a_fetch_carries_its_range_in_an_inclusive_location_filter() {
        ...
        // The last object is 5, not 6: Sections 5.1.2 and 10.13 make the range
        // inclusive, and draft-19's "the last Object, plus 1" is gone.
        assert_eq!(bytes, &vec![10, 0, 0, 5]);
    }

    /// Draft-20 Section 5.1.3, and what replaced the joining fetch: a
    ...
    fn a_subscription_that_asks_for_a_fill_can_account_for_its_stream() {
        ...
        ep.on_fill_stream_opened(id).unwrap();
        // Section 5.1.3: "a subscription can have multiple fill fetch streams
        ...
        // Section 10.12 counts every stream the publisher opened, ended or not.
        assert_eq!(ep.fill_streams_opened(id), 2);

        // Section 5.1.3.1: neither ending touches the subscription.
        ep.on_fill_stream_ended(id);
        assert_eq!(ep.active_subscription_count(), 1);
    }

    /// A subscription that asked for no fill has no fill fetch stream to
    /// attribute, and Section 10.2.15 is plain that the parameter is the whole
    ...
    fn a_stream_for_a_subscription_that_asked_for_no_fill_is_refused() {
        ...
        // Not a session close: draft-20 names no code for it, and Section
        // 5.1.3.1 makes cancelling one stream something that "does not affect
        // the subscription".
        assert_eq!(err.session_error_code(), None);
        assert_eq!(ep.session_state(), SessionState::Active);
    }

    /// Draft-20 Section 10.10: PUBLISH_STATE_NOTIFY "applies only to
