...
use crate::draft08::fetch::{FetchError, FetchState, FetchStateMachine};
use crate::draft08::namespace::{
    AnnounceState, AnnounceStateMachine, NamespaceError, SubscribeAnnouncesState,
    SubscribeAnnouncesStateMachine,
};
use crate::draft08::session::setup::{self, SetupError};
use crate::draft08::session::state::{SessionError, SessionState, SessionStateMachine};
use crate::draft08::session::subscribe_id::{SubscribeIdAllocator, SubscribeIdError};
use crate::draft08::subscription::{
    SubscriptionError, SubscriptionState, SubscriptionStateMachine,
};
use crate::draft08::track_status::{TrackStatusError, TrackStatusState, TrackStatusStateMachine};
...
use moqtap_codec::draft08::error_codes::{SessionErrorCode, SubscribeErrorCode};
...
/// that turns on the answer is the one in Section 7.3 about which side may
...
pub enum EndpointError {
    ...
    ///
    /// Section 7.3: "If a server receives a GOAWAY with a non-zero New
    ...
    ///
    /// The send-side mirror of the rule a peer breaks by sending one — Section 7.20:
    ...
    /// about.
    ///
    /// Section 7.12 makes the request the subscriber's: "A potential subscriber
    ...
    ///
    /// Section 7.4, on the Track Alias the subscriber chooses in SUBSCRIBE:
    ...
    /// Section 7.16 states the other end of the same rule, on the alias a
    ...
    /// Section 7.4 says what the receiving publisher does about it, so a
    ...
    ///
    /// Section 8: "Every Track has a single 'Object Forwarding Preference' and
    ...
    ///
    /// Section 8.1.1.1 describes Object Status 0x4, end of Track and
    ...
    /// has ever been given.
    ///
    /// Section 7.5: "A publisher SHOULD close the Session as a 'Protocol
    ...
    ///
    /// Section 7.7: "If a publisher receives a Joining Fetch with a Subscribe ID
    ...
    /// announced.
    ///
    /// Section 7.11 says what a cancellation is for: the subscriber "will stop
    ...
    ///
    /// Section 7.13: "A subscriber cannot make overlapping namespace
...
impl EndpointError {
    ...
    /// The code to close the session with, when draft-08 answers this error
    ...
    pub fn session_error_code(&self) -> Option<SessionErrorCode> {
        match self {
            // Section 7.20 answers a ceiling that does not increase with a
            ...
            // Section 7.3 answers a GOAWAY that repeats one already
            ...
            // Section 7.4 answers a SUBSCRIBE whose Track Alias already
            // names a different track with a session close, and Section 7.16
            ...
            // Section 8 answers a track whose objects mix forwarding
            ...
            // Section 8.1.1.1 answers an end-of-track object in the wrong place
...
/// Unified draft-08 MoQT endpoint wrapping session lifecycle, subscribe ID
...
pub struct Endpoint {
    ...
    /// a case Section 4.1 forbids rather than one this map decides.
    ...
    /// `SUBSCRIBES_BLOCKED` message (draft-08 only).
...
impl Endpoint {
    /// Create a new draft-08 endpoint for the given role.
    ...
    ///
    /// Section 7.4 says "already being used", and a subscription that has
    ...
    /// report Section 8.1.1.1's protocol error when it says the track ended
    ...
    /// Record how a track's object was framed, and report Section 8's "MUST NOT
    ...
    ///
    /// The form that can answer with a MAX_SUBSCRIBE_ID. Section 7.2.2.2
    ...
    pub fn receive_client_setup_and_respond_with(
        ...
        // Section 7.2.2.2 puts no role restriction on MAX_SUBSCRIBE_ID, so a
    ...
    /// 7.2.2.2 gives it a default of 0 - so it is not put through the
    ...
    /// Hold a Subscribe ID the peer chose to the rules Section 7.4
    /// states about it.
    ///
    ...
    /// session's Maximum Subscribe ID", and Section 7.7 repeats the
    ...
    pub fn receive_subscribe(&mut self, msg: &Subscribe) -> Result<(), EndpointError> {
        ...
        // Section 7.4: "If the Track Alias is already being used for a
    ...
    /// Section 7.20: "The Maximum Subscribe Id MUST only increase within a
    ...
    pub fn receive_goaway(&mut self, msg: &GoAway) -> Result<(), EndpointError> {
        ...
        // Section 7.3: "The endpoint MUST terminate the session with a
        // Protocol Violation (Section 3.5) if it receives multiple GOAWAY messages."
    ...
    pub fn receive_subscribe_error(&mut self, msg: &SubscribeError) -> Result<(), EndpointError> {
        let id = msg.subscribe_id.into_inner();
        // Section 7.16 gives SUBSCRIBE_ERROR a Track Alias field with one
    ...
    ///
    /// Section 7.5: "A subscriber issues a SUBSCRIBE_UPDATE to a publisher to
    ...
    ///
    /// The Track Alias goes back out with the refusal because Section 7.16
    ...
    /// draws from: Section 7.7 gives it the same "unique and monotonically
    /// increasing within a session" requirement.
    ///
    /// A Joining Fetch is recorded like any other. Section 7.7 answers one
    ...
    ///
    /// Section 7.8: the subscriber sends it to stop a fetch it no longer
    ...
    ///
    /// Section 7.13: "The subscriber sends the SUBSCRIBE_ANNOUNCES control
    ...
    ///
    /// Section 7.14: "A subscriber issues a UNSUBSCRIBE_ANNOUNCES message to
    ...
    ///
    /// Section 7.22: "The publisher sends the ANNOUNCE control message to
    ...
    ///
    /// Section 4.2: "A subscriber MUST send exactly one ANNOUNCE_OK or
    ...
    ///
    /// The same sentence in Section 4.2 answers both ways: one message back and
    ...
    ///
    /// Section 7.23: "The publisher sends the UNANNOUNCE control message to
    ...
    ///
    /// Section 6.2 names what a cancellation revokes: a namespace "it
    /// previously responded ANNOUNCE_OK to". Section 7.11 says what it does:
    ...
    /// about.
    ///
    /// Section 7.12: "A potential subscriber sends a 'TRACK_STATUS_REQUEST'
    ...
    ///
    /// Section 7.24 says which track the answer is about, and this draft has
    ...
    // ── Subscribes blocked (draft-08 new) ──────────────────────

    /// Process an incoming SUBSCRIBES_BLOCKED.
    ///
    /// Draft-08 adds this message so the peer can explicitly report that a
