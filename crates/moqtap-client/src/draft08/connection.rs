...
use crate::draft08::endpoint::{Endpoint, EndpointError, Role};
...
use crate::draft08::session::setup;
...
pub enum ConnectionError {
    ...
    /// A control message this build decoded for draft-08 and then could not
    /// narrow to draft-08's own message type.
    ///
    ...
    #[error(
        "a control message decoded for draft-08 did not narrow to draft-08: a defect in this          build, and evidence about nothing the peer did"
    )]
...
impl FramedRecvStream {
    ...
    pub async fn read_subgroup_header(&mut self) -> Result<AnySubgroupHeader, ConnectionError> {
        ...
        loop {
            ...
            match AnySubgroupHeader::decode_stream(DraftVersion::Draft08, &mut cursor) {
                Ok(header) => {
    ...
    pub async fn read_fetch_header(&mut self) -> Result<AnyFetchHeader, ConnectionError> {
        ...
        loop {
            let mut cursor = &self.buf[..];
            match AnyFetchHeader::decode_stream(DraftVersion::Draft08, &mut cursor) {
                Ok(header) => {
                    let consumed = self.buf.len() - cursor.remaining();
                    self.buf.advance(consumed);
                    return Ok(header);
...
impl Connection {
    ...
    /// pass. See
    /// [`Endpoint::next_free_track_alias`](crate::draft08::endpoint::Endpoint::next_free_track_alias)
    ...
    ///
    /// Draft-08 adds none. Every variant of its [`ConnectionError`] is one of the
    ...
    /// because the peer broke a rule draft-08 answers with a close.
    ///
    ...
    pub fn codec_session_error_code(
        err: &CodecError,
    ) -> Option<moqtap_codec::draft08::error_codes::SessionErrorCode> {
        use moqtap_codec::draft08::error_codes::SessionErrorCode;
        ...
        match err {
            ...
            // carry one.
            // Not `InvalidForward`: draft-08 has no Forward field. Not
    ...
    /// Close the session on the wire when a decode failure is one draft-08
