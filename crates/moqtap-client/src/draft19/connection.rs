...
impl FramedSendStream {
    ...
    pub async fn write_subgroup_header(
        ...
        self.inner.write_all(&buf).await?;
        match header {
            AnySubgroupHeader::Draft19(ref d17) => {
                self.subgroup_io = Some(SubgroupObjectReader::new(d17));
            }
            // Only this draft's header seeds the object reader. With draft 19
...
impl Connection {
    ...
    pub async fn recv_control(&mut self) -> Result<ControlMessage, ConnectionError> {
        ...
        match any {
            ...
            // When draft 19 is the only one enabled the arm above is exhaustive
    ...
    pub async fn recv_on_request_stream(
        ...
        let msg = match any {
            ...
            // feature. When draft 19 is the only one enabled the arm above is
    ...
    pub fn close_for_data_stream(&self, err: &ConnectionError) -> bool {
        ...
        let protocol_violation = u32::try_from(
            moqtap_codec::draft19::error_codes::SessionErrorCode::ProtocolViolation.as_u64(),
        )
        .unwrap_or(u32::MAX);
        match err {
...
mod accept_on_the_wire {
    ...
    /// owed.
    ///
    /// Ending a publication without asking what the last refusal left owed,
    /// which is what the route a PUBLISH sender ends on used to do:
    ///
    /// ```text
