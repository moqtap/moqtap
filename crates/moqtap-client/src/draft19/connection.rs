...
use crate::draft19::endpoint::{Endpoint, EndpointError};
use crate::draft19::event::{ClientEvent, Direction, StreamKind};
use crate::draft19::observer::ConnectionObserver;
use crate::draft19::session::request_id::Role;
use crate::draft19::session::setup;
...
use moqtap_codec::draft19::error_codes::StreamResetErrorCode;
...
/// The ALPN identifier draft-19 uses on raw QUIC, `moqt-19`.
///
...
///
/// Draft-17 removed UNSUBSCRIBE and FETCH_CANCEL and draft-19 has not brought
/// them back. Cancelling a request is resetting the bidirectional stream it
/// was made on, and `CANCELLED` is the code draft-19 assigns for "the stream
...
/// Draft-19 Section 3.3.3 grants a responder a cancel — "Receivers cancel
...
pub enum ConnectionError {
    ...
    /// A control message this build decoded for draft-19 and then could not
    /// narrow to draft-19's own message type.
    ///
    ...
    #[error(
        "a control message decoded for draft-19 did not narrow to draft-19: a defect in this          build, and evidence about nothing the peer did"
    )]
    ...
    /// A message draft-19 places on a request stream was handed to
    /// [`Connection::send_control`]. Nothing was written.
    ///
    ...
    /// header.
    /// Draft-19 Section 11.2.1.1: "An Object MUST have an empty payload unless
    ...
    PayloadOnStatusDatagram {
        ...
        status: Option<moqtap_codec::draft19::types::ObjectStatus>,
    },
    ...
    PropertiesOnNonNormalStatus {
        ...
        /// Spelled out in full because the glob import of `moqtap_codec::types`
        /// brings a different `ObjectStatus` into this module.
        status: moqtap_codec::draft19::types::ObjectStatus,
    },
    ...
    ///
    /// Draft-19 Section 3.3.2: "An endpoint MUST NOT send a FIN on a direction
    ...
    /// before sending a FIN." The section is new in draft-19; drafts 17 and 18
    /// have no such rule and no such guard.
    #[error(
        "request {0} has not been answered; draft-19 Section 3.3.2 forbids finishing a request stream before its response"
    )]
    ...
    ///
    /// The second half of the same sentence in draft-19 Section 3.3.2: "the
    ...
    #[error(
        "request {0} is an established subscription; draft-19 Section 3.3.2 requires PUBLISH_DONE before a FIN"
    )]
...
impl FramedSendStream {
    ...
    pub async fn write_subgroup_header(
        ...
        match header {
            AnySubgroupHeader::Draft19(ref header) => {
    ...
    /// Append a draft-19 subgroup object to the stream using the
...
impl FramedRecvStream {
    ...
    pub async fn read_control(
        ...
        // Draft-19: 16-bit BE payload length
            ...
            match AnySubgroupHeader::decode(self.draft, &mut cursor) {
                Ok(header) => {
                    ...
                    match header {
                        ...
                        // Only this draft's header seeds the object reader. With draft 19 the only
    ...
    ///
    /// Draft-19 has to be told, as draft-18 does. Section 11.4.4.1 makes a
    ...
    /// for — Section 10.12.3: "The publisher responding to a FETCH is
...
/// Which of the seven message types draft-19 Section 3.3 lets a bidirectional
/// stream begin with opened a request stream.
///
/// Draft-19 Section 3.3: "A request stream begins with one of these seven
...
/// these, and draft-18 — whose set draft-19 keeps — both added
/// SUBSCRIBE_TRACKS and renumbered SUBSCRIBE_NAMESPACE from 0x11 to 0x50. A
/// seven-variant enum here is what makes it impossible to label a draft-19
...
impl RequestKind {
    ...
    /// peer opened. `None` is the PROTOCOL_VIOLATION case of draft-19
    /// Section 3.3.
    ///
...
///
/// Draft-19 Section 3.3 gives every request a bidirectional stream, and either
...
/// Whether draft-19 Table 5 places this message on a request stream that is
/// already open.
///
...
/// stream that asked for them (Sections 10.16 and 10.17), and PUBLISH_SKIPPED
...
/// Whether `ty` is one of the seven message types draft-19 Section 3.3 lets a
...
/// draft-19, 0x50 is SUBSCRIBE_NAMESPACE as a control message type and also a
...
pub const fn starts_a_request_stream(ty: MessageType) -> bool {
    match ty {
        ...
        // request that already has a stream, draft-19 Section 10.9 putting it
        ...
        // present only on the control stream, and draft-19 removed the field
        // outright, leaving one wire form for both places. It may follow a
        ...
        // folded PUBLISH_OK into REQUEST_OK and draft-19 keeps it that way.
        MessageType::SubscribeOk
        | MessageType::RequestOk
        | MessageType::RequestError
        | MessageType::FetchOk
...
///
/// Draft-19 Section 3.3 keeps requests off the control plane: each request is
...
impl RequestStream {
    ...
    /// not enough to find the response: draft-19 responses carry no request
    ...
    ///
    /// Draft-19 Section 3.3.2 settles what a FIN means, which earlier drafts
...
impl Drop for RequestStream {
    ...
    /// and on draft-19 a FIN is a stronger claim than on earlier drafts, since
...
impl Connection {
    ...
    pub async fn adopt(
        ...
        // Unified SETUP in draft-19: server responds with the same message type.
        match &server_setup {
            AnyControlMessage::Draft19(ControlMessage::Setup(ref s)) => {
        ...
        let pending_events = vec![
            ...
            ClientEvent::SetupComplete { negotiated_version: 0xff000000 + 19 },
        ];
    ...
    ///
    /// [`Endpoint::receive_message`]: crate::draft19::endpoint::Endpoint::receive_message
    ///
    ...
    ///
    /// Draft-19 Section 3.3 keeps requests off the control plane: "In addition
    ...
    pub async fn send_control(&mut self, msg: &ControlMessage) -> Result<(), ConnectionError> {
        ...
        let any = AnyControlMessage::Draft19(msg.clone());
    ...
    pub async fn recv_control(&mut self) -> Result<ControlMessage, ConnectionError> {
        ...
        // Unwrap to draft-19 for the endpoint
        match any {
            AnyControlMessage::Draft19(msg) => Ok(msg),
    ...
    async fn begin_request(
        ...
        Ok(RequestStream {
            ...
            responded: false,
            fetch_data: None,
            // The sender of a PUBLISH is the one requester draft-19 Section
    ...
    ///
    /// On draft-19 a response carries no request id; the stream is the
    ...
    pub async fn recv_on_request_stream(
        ...
        let msg = match any {
            AnyControlMessage::Draft19(msg) => Ok::<_, ConnectionError>(msg),
            // `AnyControlMessage` carries one variant per enabled draft feature. With draft 19 the
            // only one enabled the arm above is exhaustive and this rejection arm unreachable.
            // Compiled in every configuration with the lint allowed, rather than gated on a `cfg`
            // naming the other drafts: such a list has to be edited in every draft module
    ...
    ///
    /// Draft-19 Section 3.3, on a stream that begins with the wrong type:
    ...
    pub async fn accept_request_stream(
        ...
        let msg = match any {
            AnyControlMessage::Draft19(msg) => msg,
            // `AnyControlMessage` carries one variant per enabled draft feature. With draft 19 the
            // only one enabled the arm above is exhaustive and this rejection arm unreachable.
            // Compiled in every configuration with the lint allowed, rather than gated on a `cfg`
            // naming the other drafts: such a list has to be edited in every draft module
    ...
    /// on the control stream: draft-19 responses carry no Request ID, so the
    ...
    ///
    /// The FIN is part of the act, not a convenience: draft-19 Section 3.3.3
    ...
    pub async fn publish_done_on(
        ...
        let msg = ControlMessage::PublishDone(moqtap_codec::draft19::message::PublishDone {
            status_code,
            stream_count,
            reason_phrase,
        });
    ...
    ///
    /// Section 10.18: "The publisher MUST NOT send NAMESPACE_DONE for a
    ...
    // Draft-19 inherits draft-17's removal of UNSUBSCRIBE. Subscribers end a
    ...
    // Draft-19 inherits draft-17's removal of FETCH_CANCEL. Fetchers abort
    ...
    /// one that makes draft-19's set differ from draft-17's.
    ...
    pub async fn recv_datagram(&self) -> Result<(AnyDatagramHeader, Bytes), ConnectionError> {
        ...
        if let AnyDatagramHeader::Draft19(h) = &header {
    ...
    /// [`PeerViolation`] is a peer having done something draft-19 forbids, and
    /// carries the session error code draft-19's own text answers it with.
    ///
    ...
    /// decoded because the peer broke a rule draft-19 answers with a close.
    ///
    ...
    pub fn codec_session_error_code(
        err: &CodecError,
    ) -> Option<moqtap_codec::draft19::error_codes::SessionErrorCode> {
        use moqtap_codec::draft19::error_codes::SessionErrorCode;
        ...
        match err {
            ...
            // allows: FORWARD in Section 10.2.17 and GROUP_ORDER in Section 10.2.8.
            ...
            // The object payload rule, Section 11.2.1.1, which draft-19 states
    ...
    /// Close the session on the wire when a decode failure is one draft-19
    ...
    /// implementations SHOULD cancel this way; draft-19 states it as what
    ...
    /// one draft-19 answers with a close. Reports whether it closed.
    ///
    ...
    ///
    /// ```no_run
    /// # async fn f(conn: &moqtap_client::draft19::connection::Connection,
    /// #            framed: &mut moqtap_client::draft19::connection::FramedRecvStream)
...
mod tests {
    ...
    /// Draft-19 uses MoQT's variable-length integer, whose length is the
    ...
    #[test]
    fn client_config_alpn_quic_draft19() {
        ...
        assert_eq!(config.alpn(), vec![b"moqt-19".to_vec()]);
    }
    ...
    fn moqt_alpn_is_the_one_a_client_offers() {
        ...
        assert_eq!(MOQT_ALPN, b"moqt-19");
    }

    /// Draft-19 Section 3.3 names seven message types a bidirectional stream
    ...
    ///
    /// ```text
    /// assertion `left == right` failed: the types that open a request stream are [3, 6, 13, 22, 29, 80]; draft-19 Section 3.3 names [3, 6, 13, 22, 29, 80, 81]
    ...
    fn only_seven_message_types_open_a_request_stream() {
        ...
        assert_eq!(
            ...
            "the types that open a request stream are {opens:?}; \
             draft-19 Section 3.3 names {expected:?}"
        );
    }
    ...
    fn each_request_kind_labels_the_message_its_helper_writes() {
        use crate::draft19::endpoint::Endpoint;
        ...
        for (kind, msg) in built {
            ...
            let mut cursor = &wire[..];
            let on_the_wire =
                DraftVersion::Draft19.decode_varint(&mut cursor).unwrap().into_inner();
            assert_eq!(
...
mod accept_on_the_wire {
    ...
    /// Session termination codes, draft-19 Section 15.11.1.
    ...
    fn request_update(id: u64) -> ControlMessage {
        ControlMessage::RequestUpdate(moqtap_codec::draft19::message::RequestUpdate {
            request_id: v(id),
            parameters: vec![],
        })
    }
    ...
    fn peer_fetch(id: u64) -> ControlMessage {
        ControlMessage::Fetch(moqtap_codec::draft19::message::Fetch {
            request_id: v(id),
    ...
    fn server_endpoint() -> (quinn::Endpoint, SocketAddr) {
        ...
        let mut server_crypto = rustls::ServerConfig::builder()
            ...
            .expect("server cert");
        server_crypto.alpn_protocols = vec![DraftVersion::Draft19.quic_alpn().to_vec()];
        let server_crypto =
    ...
    fn framed(recv: quinn::RecvStream) -> FramedRecvStream {
        FramedRecvStream::new(RecvStream::Quic(recv), DraftVersion::Draft19)
    }
    ...
    async fn next_control(recv: &mut FramedRecvStream) -> ControlMessage {
        ...
        match any {
            AnyControlMessage::Draft19(msg) => msg,
            #[allow(unreachable_patterns)]
            other => panic!("expected a draft-19 message, got {other:?}"),
        }
    }
    ...
    /// and cannot be finished until each message draft-19 Section 3.3.2
    ...
    /// since Section 3.3.2 is new in draft-19 — fails with:
    ///
    /// ```text
    ...
    /// Draft-19 Section 3.3.3: "When an endpoint rejects a request without
    ...
    /// Draft-19 Section 3.3: a bidirectional stream that begins with a message
    ...
    async fn a_refused_fetch_update_resets_the_fetch_data_stream() {
        ...
        lb.conn
            .open_fetch_stream_on(
                &mut stream,
                &AnyFetchHeader::Draft19(FetchHeader { request_id: v(1) }),
            )
            .await
    ...
    async fn a_fetch_data_stream_can_be_opened_through_the_connection() {
        ...
        let mut send = lb
            .conn
            .open_fetch_stream(&AnyFetchHeader::Draft19(FetchHeader { request_id: v(4) }))
            .await
        ...
        assert!(
            matches!(&header, AnyFetchHeader::Draft19(h) if h.request_id.into_inner() == 4),
    ...
    /// Draft-19 Table 5 gives NAMESPACE the Stream value "Request", and
    /// Section 10.16 puts it "on the response stream of a SUBSCRIBE_NAMESPACE
    ...
    /// Draft-19 Section 3.3.2 excludes the sender of a PUBLISH from the
