...
use crate::draft21::endpoint::{Endpoint, EndpointError};
use crate::draft21::event::{ClientEvent, Direction, StreamKind};
use crate::draft21::fill::LocationFilter;
use crate::draft21::observer::ConnectionObserver;
use crate::draft21::session::request_id::Role;
use crate::draft21::session::setup;
...
/// The ALPN identifier draft-21 uses on raw QUIC, `moqt-21`.
///
...
///
/// This is [`DraftVersion::Draft21`]'s own
/// [`quic_alpn`](DraftVersion::quic_alpn), which is what
/// [`ClientConfig::alpn`] offers; the test below holds the two together.
pub const MOQT_ALPN: &[u8] = b"moqt-21";

/// The unidirectional stream type that marks one direction of the control
/// plane, and the SETUP message type. On draft-21 they are one number,
/// 0x2F00: Section 6.4.1 (Unidirectional Stream Types) lists it as the type of
/// a SETUP stream, and Section 9.1 gives it as the SETUP message's own type
/// field.
///
...
///
/// Draft-17 removed UNSUBSCRIBE and FETCH_CANCEL and draft-21 has not brought
/// them back. Cancelling a request is resetting the bidirectional stream it
/// was made on, and `CANCELLED` is the code draft-21 assigns for "the stream
...
/// Draft-21 Section 6.4.2.3 grants a responder a cancel — "Receivers cancel
...
/// and Section 6.4.2.2 says when it has not: "the publisher of an Established
...
pub enum ConnectionError {
    ...
    /// A control message this build decoded for draft-21 and then could not
    /// narrow to draft-21's own message type.
    ///
    ...
    #[error(
        "a control message decoded for draft-21 did not narrow to draft-21: a defect in this          build, and evidence about nothing the peer did"
    )]
    ...
    /// header.
    /// Draft-21 Section 11.1.2: "An Object MUST have an empty payload unless
    /// its Object Status value is registered as permitting a payload in the
    /// Object Status registry" Section 11.2.1 says the same thing about the
    ...
    PayloadOnStatusDatagram {
        ...
        status: Option<moqtap_codec::draft21::types::ObjectStatus>,
    },
    ...
    PropertiesOnNonNormalStatus {
        ...
        /// Spelled out in full because the glob import of `moqtap_codec::types`
        /// brings a different `ObjectStatus` into this module.
        status: moqtap_codec::draft21::types::ObjectStatus,
    },
    ...
    ///
    /// Draft-21 Section 6.4.2.2: "An endpoint MUST NOT send a FIN on a direction
    ...
    /// before sending a FIN." The section arrived in draft-19 and draft-21
    /// keeps it verbatim; drafts 17 and 18 have no such rule and no such
    /// guard.
    #[error(
        "request {0} has not been answered; draft-21 Section 6.4.2.2 forbids finishing a request stream before its response"
    )]
    ...
    #[error(
        "request {0} is an established subscription; draft-21 Section 6.4.2.2 requires PUBLISH_DONE before a FIN"
    )]
...
impl FramedSendStream {
    ...
    pub async fn write_subgroup_header(
        ...
        match header {
            AnySubgroupHeader::Draft21(ref header) => {
                self.subgroup_io = Some(SubgroupObjectReader::new(header));
            }
            // Only this draft's header seeds the object reader. With draft 21 the only enabled
    ...
    /// Append a draft-21 subgroup object to the stream using the
...
impl FramedRecvStream {
    ...
    ///
    /// Every unidirectional MoQT stream on draft-21 opens with a varint
    /// naming what it is (Section 6.4.1): 0x05 for FETCH_HEADER, 0x10-0x1D for
    ...
    pub async fn read_control(
        ...
        // Draft-21: 16-bit BE payload length
    ...
    /// Read the next draft-21 subgroup object from this stream using
    ...
    /// draft-21 Section 11.1.3 answers with a session close. The object is
    ...
    /// Read the next draft-21 fetch header from this stream.
    ...
    ///
    /// Draft-21 has to be told, as draft-18 does. Section 11.4.1.1 makes a
    ...
    /// for — Section 9.11: "The publisher responding to a FETCH is
    /// responsible for delivering all available Objects in the requested range
    /// in the requested order (see Section 9.20.9)" — carried by the
    /// GROUP_ORDER parameter on the FETCH, or by its absence, which Section
    /// 9.20.9 reads as Ascending. Either way it is a control message this
    ...
    /// Read the next draft-21 fetch object and its payload.
    ///
    /// The mirror of [`FramedSendStream::write_fetch_object`]. What comes back
    /// is a [`FetchObject`] rather than a header: draft-21's reader resolves the
    ...
    /// every rule Section 11.4.1.1 states — a first Object that inherits, a
...
/// Which of the seven message types draft-21 Section 6.3 lets a bidirectional
/// stream begin with opened a request stream.
///
/// Draft-21 Section 6.3: "A request stream begins with one of these seven
...
/// these, and draft-18 — whose set draft-21 keeps — both added
/// SUBSCRIBE_TRACKS and renumbered SUBSCRIBE_NAMESPACE from 0x11 to 0x50. A
/// seven-variant enum here is what makes it impossible to label a draft-21
...
impl RequestKind {
    ...
    /// peer opened. `None` is the PROTOCOL_VIOLATION case of draft-21
    /// Section 6.3.
    ///
    ...
    pub const fn from_message_type(ty: MessageType) -> Option<RequestKind> {
        match ty {
            ...
            // New in draft-20. Section 9.10 puts it on "a subscription's
...
///
/// Draft-21 Section 6.3 gives every request a bidirectional stream, and either
...
/// Whether draft-21 Table 5 places this message on a request stream that is
/// already open.
///
/// All five of the messages Table 5 marks "Request" without their beginning one:
/// REQUEST_UPDATE modifies the request its stream carries (Section 9.5),
/// NAMESPACE and NAMESPACE_DONE report namespaces on the SUBSCRIBE_NAMESPACE
/// stream that asked for them (Sections 9.16 and 9.17), PUBLISH_SKIPPED
/// names a track on the SUBSCRIBE_TRACKS stream that asked for it (Section
/// 9.18), and PUBLISH_STATE_NOTIFY — new in draft-20 — reports a change on the
/// subscription's own stream (Section 9.10).
///
...
/// Whether `ty` is one of the seven message types draft-21 Section 6.3 lets a
...
/// draft-21, 0x50 is SUBSCRIBE_NAMESPACE as a control message type and also a
...
pub const fn starts_a_request_stream(ty: MessageType) -> bool {
    match ty {
        ...
        // request that already has a stream, draft-21 Section 9.5 putting it
        ...
        // outright and draft-21 keeps it gone, leaving one wire form for both
        ...
        // folded PUBLISH_OK into REQUEST_OK and draft-21 keeps it that way.
        MessageType::SubscribeOk
        | MessageType::RequestOk
        | MessageType::RequestError
        | MessageType::FetchOk
        | MessageType::PublishDone => false,
        // Not a response — Section 9.10 makes it unilateral, answered with
...
///
/// Draft-21 Section 6.3 keeps requests off the control plane: each request is
...
/// Draft-21 Section 6.4.2.3 is explicit that a cancel is available to both:
...
/// merely different: Section 6.4.2.2 forbids a FIN before the messages that
...
pub struct RequestStream {
    ...
    /// out. It is what draft-21 Section 6.4.2.2's second clause is checked
...
impl RequestStream {
    ...
    /// not enough to find the response: draft-21 responses carry no request
    ...
    /// answered with SUBSCRIBE_OK — the two ways draft-21 Section 6.4.2.2's
    ...
    ///
    /// Draft-17 removed UNSUBSCRIBE and FETCH_CANCEL and draft-21 has not
    ...
    ///
    /// [`REQUEST_CANCELLED`] is the ordinary choice. Draft-21 Section 12.5
    ...
    ///
    /// Draft-21 Section 6.4.2.2 settles what a FIN means, which earlier drafts
    ...
    ///
    /// Drafts 17 and 18 have no Section 6.4.2.2 and no guard: their `finish` is
    ...
    /// Section 6.4.2.2 contrasts with a FIN as the abrupt close, so a request
...
impl Drop for RequestStream {
    ...
    /// and on draft-21 a FIN is a stronger claim than on earlier drafts, since
    /// Section 6.4.2.2 has the receiver of one *treat the request as failed*
    ...
    /// Section 6.4.2.3 describes. A stream the peer opened is reset with
...
impl Connection {
    ...
    ///
    /// Draft-21 Section 6.3: "MOQT uses a pair of unidirectional streams for
    ...
    ///
    /// They are kept, not dropped. Section 6.3 expects them: "Unidirectional
    ...
    pub async fn adopt(
        ...
        // Unified SETUP in draft-21: server responds with the same message type.
        match &server_setup {
            AnyControlMessage::Draft21(ControlMessage::Setup(ref s)) => {
    ...
    ///
    /// Wraps the draft-21 message in `AnyControlMessage::Draft21` for framing.
    /// Of the messages draft-21 Table 5 does not place on a request stream,
    ...
    /// it modifies (Section 9.5); the other three report on the namespace or
    ...
    ///
    /// [`Endpoint::receive_message`]: crate::draft21::endpoint::Endpoint::receive_message
    ///
    ...
    ///
    /// Draft-21 Section 6.3 keeps requests off the control plane: "In addition
    ...
    pub async fn send_control(&mut self, msg: &ControlMessage) -> Result<(), ConnectionError> {
        ...
        let any = AnyControlMessage::Draft21(msg.clone());
    ...
    ///
    /// Returns the `AnyControlMessage` and also extracts the draft-21
    ...
    pub async fn recv_control(&mut self) -> Result<ControlMessage, ConnectionError> {
        ...
        // Unwrap to draft-21 for the endpoint
        match any {
            AnyControlMessage::Draft21(msg) => Ok(msg),
            // `AnyControlMessage` carries one variant per enabled draft feature. With draft 21 the
    ...
    ///
    /// Responses never arrive here. Draft-21 responses carry no request id
    ...
    async fn begin_request(
        ...
        Ok(RequestStream {
            ...
            responded: false,
            fetch_data: None,
            // The sender of a PUBLISH is the one requester draft-21 Section
            // 6.4.2.2 excludes from "MAY FIN immediately after sending a
    ...
    ///
    /// On draft-21 a response carries no request id; the stream is the
    ...
    pub async fn recv_on_request_stream(
        ...
        let msg = match any {
            AnyControlMessage::Draft21(msg) => Ok::<_, ConnectionError>(msg),
            // `AnyControlMessage` carries one variant per enabled draft feature. With draft 21 the
            // only one enabled the arm above is exhaustive and this rejection arm unreachable.
            // Compiled in every configuration with the lint allowed, rather than gated on a `cfg`
            // naming the other drafts: such a list has to be edited in every draft module
        ...
        // (Section 9.10) and reporting it as a state change would be a lie.
    ...
    /// Section 6.4.2.3 puts the cancel at the stream — "Implementations cancel a
    ...
    /// a request, this takes one the peer opened and reads one. Draft-21
    /// Section 6.3 puts requests in both directions on bidirectional streams,
    ...
    ///
    /// Draft-21 Section 6.3, on a stream that begins with the wrong type:
    ...
    /// PROTOCOL_VIOLATION." Section 6.4.2.1, on the Request ID: "If an endpoint
    ...
    pub async fn accept_request_stream(
        ...
        let msg = match any {
            AnyControlMessage::Draft21(msg) => msg,
            // `AnyControlMessage` carries one variant per enabled draft feature. With draft 21 the
            // only one enabled the arm above is exhaustive and this rejection arm unreachable.
            // Compiled in every configuration with the lint allowed, rather than gated on a `cfg`
            // naming the other drafts: such a list has to be edited in every draft module
    ...
    /// on the control stream: draft-21 responses carry no Request ID, so the
    ...
    async fn respond(
        ...
        // subscription is live and Section 9.5 now asks for a PUBLISH_DONE
        ...
        // Section 9.5.1: "When a REQUEST_UPDATE fails for a FETCH, the
    ...
    /// kind REQUEST_OK answers is pending on it — including the Section 9.3
    ...
    /// [`RequestStream::finish`] refuses, per draft-21 Section 6.4.2.2.
    ...
    ///
    /// The FIN is part of the act, not a convenience: draft-21 Section 6.4.2.3
    ...
    /// success path owes the peer something more, which is what Section 6.4.2.2
    ...
    /// [`publish_done`](Self::publish_done) gives: Section 6.4.2.2 names
    ...
    pub async fn publish_done_on(
        ...
        let msg = ControlMessage::PublishDone(moqtap_codec::draft21::message::PublishDone {
            status_code,
            stream_count,
            reason_phrase,
        });
    ...
    /// 9.17 has the announcements follow the acceptance, so this refuses until
    ...
    ///
    /// Section 9.15: "The publisher MUST NOT send NAMESPACE_DONE for a
    ...
    // Draft-21 inherits draft-17's removal of UNSUBSCRIBE. Subscribers end a
    ...
    /// "the last Object, plus 1; or 0 to indicate the entire Group"; draft-21
    /// Sections 3.3.1 and 9.11 both call the filter's range inclusive and
    ...
    // the stream that answers it, and `crate::draft21::fill` for the parameter.

    // Draft-21 inherits draft-17's removal of FETCH_CANCEL. Fetchers abort
    ...
    /// draft-21 keeps the split. This call sends the renumbered
    ...
    /// one that makes draft-21's set differ from draft-17's.
    ...
    ///
    /// REQUEST_OK — which is what draft-21 answers a PUBLISH with, PUBLISH_OK
    ...
    /// another's stream.
    ///
    /// The send half is finished once the message is out. Draft-21 Section
    /// 6.4.2.2 names PUBLISH_DONE as the last required message on a publisher's
    ...
    /// [`Endpoint::fill_group_order`](crate::draft21::endpoint::Endpoint::fill_group_order)
    /// and [`fill::group_order`](crate::draft21::fill::group_order) for the
    /// three-step resolution Sections 3.4 and 9.20.9 define.
    ///
    ...
    ///
    /// Section 3.4.1: a FIN means the whole fill range was delivered, and a
    ...
    /// with [`Endpoint::on_fill_stream_ended`](crate::draft21::endpoint::Endpoint::on_fill_stream_ended);
    /// a subscriber that wants to stop one early sends `STOP_SENDING` with
    /// [`FramedRecvStream::stop`], which Section 3.4.1 gives it outright.
    ///
    /// # Errors
    ///
    /// [`ConnectionError::Endpoint`] carrying
    /// [`EndpointError::UnrequestedFillStream`] when the header names a request
    /// that asked for no fill. Not fatal to the session — draft-21 states no
    ...
    pub async fn accept_fill_stream(
        ...
        // Section 9.20.9 default `fill::group_order` applies.
        let group_order =
    ...
    /// draft-21 datagram carrying properties on a status other than Normal.
    /// Draft-21 Section 11.2.1 builds the datagram's Properties field out of
    /// the structure defined in Section 11.1.3, and that section answers the
    ...
    pub async fn recv_datagram(&self) -> Result<(AnyDatagramHeader, Bytes), ConnectionError> {
        ...
        if let AnyDatagramHeader::Draft21(h) = &header {
    ...
    /// errors draft-21 tells the receiver to close the session over, and the
    ...
    /// [`PeerViolation`] is a peer having done something draft-21 forbids, and
    /// carries the session error code draft-21's own text answers it with.
    ///
    ...
    pub fn draft_specific_cause(
        ...
        use moqtap_codec::draft21::error_codes::SessionErrorCode;
        ...
        match err {
            ...
            // Section 11.1.3 states the rule and names the code in the same
            ...
            // Section 11.1.2 states this one as a property of a conforming
            ...
            // Section 6.3: "Bidirectional streams MUST NOT begin with any
            ...
            // Section 6.4.2.2 forbids a FIN before the response a request type
    ...
    /// decoded because the peer broke a rule draft-21 answers with a close.
    ///
    ...
    /// 8.5 and 9.2), the KVP value maximum and the delta-encoded
    /// type overflow (Section 8.3), the duplicate-parameter rule (Section
    /// 9.20), the Track Namespace field, count and length rules
    /// (Section 8.7), and the Object ID delta wrap (Section 11.3.1). Each of
    ...
    pub fn codec_session_error_code(
        err: &CodecError,
    ) -> Option<moqtap_codec::draft21::error_codes::SessionErrorCode> {
        use moqtap_codec::draft21::error_codes::SessionErrorCode;
        ...
        match err {
            ...
            // **Draft-21 has no Filter Type.** Drafts 15 through 19 state that
            // rule in a Section 5.1.2 that defines a Filter Type field;
            // draft-21's Section 9.20.10 defines the LOCATION_FILTER parameter
            ...
            // the whole draft is prose in Section 3.3.3 about combining
            // filters.
            //
            // So this arm cannot fire: draft-21's message reader raises the
            ...
            // past the end of the number space, Section 9.20.10: "If StartGroup +
            // EndGroupDelta exceeds 2^64 - 1, the endpoint MUST close the
            // session with a PROTOCOL_VIOLATION."
            //
            // Draft-21's own wording. Drafts 18 and 19 state the rule as two
            ...
            // in two: Section 6.4.1 for streams, Section 11 for datagrams, both
            ...
            // allows: FORWARD in Section 9.20.19 and GROUP_ORDER in Section 9.20.9.
            ...
            // range its type allows: DEFAULT_PUBLISHER_GROUP_ORDER in Section 10.5
            // and DYNAMIC_GROUPS in Section 10.6.
            // Each states that a receiver "MUST close the session with
            // PROTOCOL_VIOLATION".
            //
            ...
            // invalid: Section 11.3.1 for a subgroup header whose SUBGROUP_ID_MODE
            // is the reserved 0b11, Section 11.2.1 for a datagram asking to be both
            ...
            // Type defines, Section 8.3: "If a receiver understands a Type,
            ...
            // 9.20: "All Message Parameters MUST be defined in the negotiated
            ...
            // name, Section 9.20.1: "Each Message Parameter definition indicates
            ...
            // The object payload rule, Section 11.1.2, which draft-21 states
            ...
            // permitting a payload in the Object Status registry (Section 16.9).
    ...
    /// Close the session on the wire when a decode failure is one draft-21
    ...
    ///
    /// Section 12.1 answers its whole list of conditions at once: "it MUST
    ...
    /// operation rather than a message — Section 6.4.2.3: "Implementations cancel a request by
    ...
    /// implementations SHOULD cancel this way; draft-21 states it as what
    ...
    /// one draft-21 answers with a close. Reports whether it closed.
    ///
    ...
    ///
    /// ```no_run
    /// # async fn f(conn: &moqtap_client::draft21::connection::Connection,
    /// #            framed: &mut moqtap_client::draft21::connection::FramedRecvStream)
    ...
    /// carried it. The Object ID delta wrap of Section 11.3.1 is the entry that
...
mod tests {
    ...
    /// Draft-21 uses MoQT's variable-length integer, whose length is the
    ...
    fn varint_len_follows_the_moqt_encoding() {
        let draft = DraftVersion::Draft21;
    ...
    #[test]
    fn client_config_alpn_quic_draft21() {
        ...
        assert_eq!(config.alpn(), vec![b"moqt-21".to_vec()]);
    }
    ...
    ///
    /// ```text
    /// assertion `left == right` failed: MOQT_ALPN is "moq-00"; a draft-21 client offers ["moqt-21"]
    /// ```
    #[test]
    fn moqt_alpn_is_the_one_a_client_offers() {
        ...
        assert_eq!(MOQT_ALPN, b"moqt-21");
    }

    /// Draft-21 Section 6.3 names seven message types a bidirectional stream
    ...
    ///
    /// ```text
    /// assertion `left == right` failed: the types that open a request stream are [3, 6, 13, 22, 29, 80]; draft-21 Section 6.3 names [3, 6, 13, 22, 29, 80, 81]
    ...
    fn only_seven_message_types_open_a_request_stream() {
        ...
        assert_eq!(
            ...
            "the types that open a request stream are {opens:?}; \
             draft-21 Section 6.3 names {expected:?}"
        );
    }
    ...
    fn each_request_kind_labels_the_message_its_helper_writes() {
        use crate::draft21::endpoint::Endpoint;
        use moqtap_codec::draft21::message::Setup;
        ...
        for (kind, msg) in built {
            ...
            let mut cursor = &wire[..];
            let on_the_wire =
                DraftVersion::Draft21.decode_varint(&mut cursor).unwrap().into_inner();
            assert_eq!(
...
mod accept_on_the_wire {
    ...
    /// Session termination codes, draft-21 Section 16.11.1.
    ...
    fn encode(msg: ControlMessage) -> Vec<u8> {
        let mut buf = Vec::new();
        AnyControlMessage::Draft21(msg).encode(&mut buf).expect("encode");
        buf
    }
    ...
    fn request_update(id: u64) -> ControlMessage {
        ControlMessage::RequestUpdate(moqtap_codec::draft21::message::RequestUpdate {
            request_id: v(id),
            parameters: vec![],
        })
    }
    ...
    fn peer_fetch(id: u64) -> ControlMessage {
        ControlMessage::Fetch(moqtap_codec::draft21::message::Fetch {
            request_id: v(id),
            track_namespace: ns(),
            track_name: b"video".to_vec(),
            parameters: vec![],
    ...
    fn server_endpoint() -> (quinn::Endpoint, SocketAddr) {
        ...
        let mut server_crypto = rustls::ServerConfig::builder()
            ...
            .expect("server cert");
        server_crypto.alpn_protocols = vec![DraftVersion::Draft21.quic_alpn().to_vec()];
        let server_crypto =
    ...
    fn framed(recv: quinn::RecvStream) -> FramedRecvStream {
        FramedRecvStream::new(RecvStream::Quic(recv), DraftVersion::Draft21)
    }
    ...
    async fn next_control(recv: &mut FramedRecvStream) -> ControlMessage {
        ...
        match any {
            AnyControlMessage::Draft21(msg) => msg,
            #[allow(unreachable_patterns)]
            other => panic!("expected a draft-21 message, got {other:?}"),
        }
    }
    ...
    /// and cannot be finished until each message draft-21 Section 6.4.2.2
    ...
    /// since Section 6.4.2.2 arrived in draft-19 — fails with:
    ///
    /// ```text
    ...
    async fn a_peers_subscribe_is_answered_on_the_stream_it_arrived_on() {
        ...
        // Section 6.4.2.2: "an endpoint sending a response to a request MUST send
    ...
    ///
    /// Section 9.5.1: "When a REQUEST_UPDATE is unsuccessful, the publisher
    ...
    ///
    /// Section 9.5.1 sorts a refused REQUEST_UPDATE by what was being
    ...
    /// the responder MUST close the bidi stream (see Section 6.4.2.2)." There is
    ...
    /// Draft-21 Section 6.4.2.3: "When an endpoint rejects a request without
    ...
    /// Draft-21 Section 6.3: a bidirectional stream that begins with a message
    ...
    /// `session_error_code`, which conflates Section 6.3's rule with Section
    /// 6.4.2.1's, fails with:
    ///
    /// ```text
    ...
    ///
    /// Section 9.5.1: "When a REQUEST_UPDATE fails for a FETCH, the publisher
    ...
    async fn a_refused_fetch_update_resets_the_fetch_data_stream() {
        ...
        lb.conn
            .open_fetch_stream_on(
                &mut stream,
                &AnyFetchHeader::Draft21(FetchHeader { request_id: v(1) }),
            )
            .await
    ...
    async fn a_fetch_data_stream_can_be_opened_through_the_connection() {
        ...
        let mut send = lb
            .conn
            .open_fetch_stream(&AnyFetchHeader::Draft21(FetchHeader { request_id: v(4) }))
            .await
        ...
        assert!(
            matches!(&header, AnyFetchHeader::Draft21(h) if h.request_id.into_inner() == 4),
    ...
    /// Draft-21 Table 5 gives NAMESPACE the Stream value "Request", and
    /// Section 9.16 puts it "on the response stream of a SUBSCRIBE_NAMESPACE
    ...
    async fn a_namespace_is_announced_on_the_request_stream_that_asked_for_it() {
        ...
        // finish it — Section 6.4.2.2's guard covers the response and a
    ...
    ///
    /// Section 9.5 names the one case where a requester answers rather than
    ...
    /// Draft-21 Section 6.4.2.2 excludes the sender of a PUBLISH from the
