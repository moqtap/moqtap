...
/// The PATH setup option. It is the one setup option whose sender is
...
pub enum SetupError {
    /// A required setup option is missing.
    #[error("missing required parameter: {0}")]
    MissingParameter(
        /// Name of the missing option.
        &'static str,
    ),
    /// A setup option was sent by the endpoint that may not send it.
    ...
    /// A PATH option was offered on a session that is not native QUIC.
...
///
/// Draft-21 merges the two setup messages into one and uses ALPN for version
...
/// 9.1.2 says the PATH option "MUST NOT be used by the server", and with
...
/// Refuse a PATH option on a session that is not native QUIC.
///
/// The same sentence in Section 9.1.2 forbids PATH "when WebTransport is
