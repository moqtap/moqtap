...
use crate::draft13::fetch::{FetchError, FetchState, FetchStateMachine};
use crate::draft13::namespace::{
    AnnounceState, AnnounceStateMachine, NamespaceError, SubscribeNamespaceState,
    SubscribeNamespaceStateMachine,
};
use crate::draft13::publish::{
    PublishError as PublishFlowError, PublishState, PublishStateMachine,
};
use crate::draft13::session::request_id::{RequestIdAllocator, RequestIdError, Role};
...
use crate::draft13::subscription::{
    SubscriptionError, SubscriptionState, SubscriptionStateMachine,
};
use crate::draft13::track_status::{TrackStatusError, TrackStatusState, TrackStatusStateMachine};
...
pub enum EndpointError {
    ...
    ///
    /// Section 8.20 treats a TRACK_STATUS as a SUBSCRIBE "except it does not
    ...
    ///
    /// Draft-13 states it twice, once per message. Section 8.8: "The same
    ...
    /// announced.
    ///
    /// Section 8.27 says what a cancellation is for: the subscriber "will stop
    ...
    /// not subscribed to.
    ///
    /// Section 5.1: "An UNSUBSCRIBE_NAMESPACE withdraws a previous SUBSCRIBE_NAMESPACE."
    ///
    ...
    /// MUST respond with SUBSCRIBE_NAMESPACE_ERROR, with error code Namespace
    /// Prefix Overlap."
    ///
...
/// respond with SUBSCRIBE_NAMESPACE_ERROR, with error code Namespace Prefix
/// Overlap."
///
...
impl EndpointError {
    ...
    /// The code to close the session with, when draft-13 answers this error
...
///
/// Section 8.20 treats the request as a SUBSCRIBE "except it does not create
...
impl Endpoint {
    /// Create a new draft-13 endpoint for the given role.
    ...
    pub fn subscribe_range(
        ...
        self.subscribe_inner(
            ...
            group_order,
            filter_type,
            Some((start_location.group, start_location.object)),
            end_group,
            parameters,
        )
    }
    ...
    fn subscribe_inner(
        ...
        let (start_group, start_object) = match start {
            Some((group, object)) => (Some(group), Some(object)),
            None => (None, None),
        };
    ...
    pub fn receive_subscribe_update(&mut self, msg: &SubscribeUpdate) -> Result<(), EndpointError> {
        ...
        // update may change.
        // Section 8.20 takes the update away from this request kind outright:
    ...
    pub fn receive_unsubscribe(&mut self, msg: &Unsubscribe) -> Result<(), EndpointError> {
        let id = msg.request_id.into_inner();
        // The other half of the same sentence. Section 8.20: "the subscriber
    ...
    ///
    /// Section 8.28 addresses the first half of the overlap rule to this end
    ...
    ///
    /// Section 8.28: "The subscriber sends the SUBSCRIBE_NAMESPACE control
    ...
    /// `prefix`.
    ///
    /// Section 8.31 names a Track Namespace Prefix where the
    ...
    ///
    /// Section 8.23: "The publisher sends the ANNOUNCE control message to
    ...
    ///
    /// Section 8.26: "The publisher sends the UNANNOUNCE control message to
    /// indicate its intent to stop serving new subscriptions for tracks within
    /// the provided Track Namespace." and Section 8.27 says what a cancellation
    ...
    /// previously responded ANNOUNCE_OK to". Section 8.27 says what it does:
    ...
    /// Send a TRACK_STATUS message asking after a track, and record it.
    ///
    /// Named for the message this draft carries. TRACK_STATUS_REQUEST was the
    /// earlier drafts' name for the request and is not a message here, so a
    /// method carrying that name would say the wrong thing about which of the
    /// two shapes goes on the wire.
    ///
    ...
    pub fn track_status(
        ...
        // TRACK_STATUS is the SUBSCRIBE message on draft-13, filter-dependent
        // fields included, and this call carries none of them.
        if matches!(filter_type, FilterType::AbsoluteStart | FilterType::AbsoluteRange) {
            return Err(EndpointError::FilterNeedsRange);
        }
        self.require_active_or_err()?;
    ...
    /// Process an incoming TRACK_STATUS_OK reply.
    ///
    /// Section 8.21 gives this draft a pair of answers where the earlier ones
    /// had a single TRACK_STATUS, so the acceptance and the refusal walk
    /// separate edges: a machine told only that an answer arrived cannot say
    ...
    /// Process an incoming TRACK_STATUS_ERROR reply.
    ///
    /// Section 8.22: "The publisher sends a TRACK_STATUS_ERROR control message
    ...
    ///
    /// Section 8.20: the receiver of one "treats it identically as if it had
    ...
    ///
    /// Section 8.21: "The publisher sends a TRACK_STATUS_OK control message in
    ...
    pub fn send_track_status_error(
        ...
        Ok(ControlMessage::TrackStatusError(TrackStatusErrorMsg {
            request_id,
            error_code,
            reason_phrase,
        }))
