...
impl FramedSendStream {
    ...
    /// Draft-13 subgroup objects may carry extension headers but the writer
...
impl FramedRecvStream {
    ...
    pub async fn read_subgroup_header(&mut self) -> Result<AnySubgroupHeader, ConnectionError> {
        ...
        loop {
            ...
            match AnySubgroupHeader::decode(DraftVersion::Draft13, &mut cursor) {
                Ok(header) => {
    ...
    pub async fn read_fetch_header(&mut self) -> Result<AnyFetchHeader, ConnectionError> {
        ...
        loop {
            let mut cursor = &self.buf[..];
            match AnyFetchHeader::decode(DraftVersion::Draft13, &mut cursor) {
                Ok(header) => {
                    let consumed = self.buf.len() - cursor.remaining();
                    self.buf.advance(consumed);
                    return Ok(header);
    ...
    /// Read the next draft-13 subgroup object (header + payload). The codec
...
impl Connection {
    ...
    pub async fn subscribe_namespace(
        ...
        let (req_id, msg) = self.endpoint.subscribe_namespace(track_namespace_prefix)?;
        self.send_control(&msg).await?;
        Ok(req_id)
    }
    ...
    pub async fn track_status_request(
        ...
        let (req_id, msg) = self.endpoint.track_status_request(
            track_namespace,
            track_name,
            subscriber_priority,
            group_order,
