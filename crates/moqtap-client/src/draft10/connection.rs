...
use crate::draft10::endpoint::{Endpoint, EndpointError, Role};
use crate::draft10::event::{ClientEvent, Direction, FetchObject, StreamKind, SubgroupObject};
use crate::draft10::observer::ConnectionObserver;
use crate::draft10::session::setup;
...
use moqtap_codec::draft10::message::ControlMessage;
...
/// Errors from the draft-10 connection layer.
...
pub enum ConnectionError {
    ...
    /// A control message this build decoded for draft-10 and then could not
    /// narrow to draft-10's own message type.
    ///
    ...
    #[error(
        "a control message decoded for draft-10 did not narrow to draft-10: a defect in this          build, and evidence about nothing the peer did"
    )]
...
/// Configuration for a draft-10 MoQT client connection.
pub struct ClientConfig {
    /// Additional draft versions to offer in CLIENT_SETUP (draft-10 is always
    /// offered first).
...
impl ClientConfig {
    ...
    /// Draft-10 first, then any additional versions.
    pub fn supported_versions(&self) -> Vec<VarInt> {
        let mut versions = vec![DraftVersion::Draft10.version_varint()];
    ...
    pub fn alpn(&self) -> Vec<Vec<u8>> {
        match &self.transport {
            TransportType::Quic => vec![DraftVersion::Draft10.quic_alpn().to_vec()],
...
impl FramedSendStream {
    ...
    /// time, and no type byte selects between them. The drafts with a header
    /// type table do need the refusal, and every draft's writer makes the same
    /// checked call, so no draft can be the one that skips it.
    ...
    /// Append a draft-10 subgroup object (header + payload) to the stream.
    ///
    ...
    /// Append a draft-10 fetch object (header + payload) to the stream.
...
impl FramedRecvStream {
    ...
    pub async fn read_control(
        ...
        // Draft-10 uses varint length framing.
        ...
        // Now decode the whole message using the draft-10 dispatcher
        let mut frame = &self.buf[..total];
        let msg = AnyControlMessage::decode(DraftVersion::Draft10, &mut frame)?;
    ...
    pub async fn read_subgroup_header(&mut self) -> Result<AnySubgroupHeader, ConnectionError> {
        ...
        loop {
            ...
            match AnySubgroupHeader::decode_stream(DraftVersion::Draft10, &mut cursor) {
                Ok(header) => {
    ...
    pub async fn read_fetch_header(&mut self) -> Result<AnyFetchHeader, ConnectionError> {
        ...
        loop {
            let mut cursor = &self.buf[..];
            match AnyFetchHeader::decode_stream(DraftVersion::Draft10, &mut cursor) {
                Ok(header) => {
                    let consumed = self.buf.len() - cursor.remaining();
                    self.buf.advance(consumed);
                    return Ok(header);
    ...
    /// Read the next draft-10 subgroup object (header + payload). Since
    /// draft-10 subgroup objects are stateless, this does not require any
    ...
    /// Read the next draft-10 fetch object (header + payload).
...
impl Connection {
    ...
    pub async fn adopt_offering(
        ...
        let any_setup = AnyControlMessage::Draft10(setup_msg);
        ...
        match &server_setup {
            AnyControlMessage::Draft10(ControlMessage::ServerSetup(ref ss)) => {
    ...
    pub async fn send_control(&mut self, msg: &ControlMessage) -> Result<(), ConnectionError> {
        let any = AnyControlMessage::Draft10(msg.clone());
    ...
    pub async fn recv_control(&mut self) -> Result<ControlMessage, ConnectionError> {
        ...
        match any {
            AnyControlMessage::Draft10(msg) => Ok(msg),
            // `AnyControlMessage` carries one variant per enabled draft feature. With draft 10 the
    ...
    /// pass. See
    /// [`Endpoint::next_free_track_alias`](crate::draft10::endpoint::Endpoint::next_free_track_alias)
    ...
    pub async fn recv_datagram(&self) -> Result<(AnyDatagramHeader, Bytes), ConnectionError> {
        ...
        let mut cursor = &data[..];
        let header = AnyDatagramHeader::decode(DraftVersion::Draft10, &mut cursor)?;
    ...
    ///
    /// Draft-10 adds none. Every variant of its [`ConnectionError`] is one of the
    ...
    /// because the peer broke a rule draft-10 answers with a close.
    ///
    ...
    pub fn codec_session_error_code(
        err: &CodecError,
    ) -> Option<moqtap_codec::draft10::error_codes::SessionErrorCode> {
        use moqtap_codec::draft10::error_codes::SessionErrorCode;
        ...
        match err {
            ...
            // Section 8.7: "Any other value is a protocol error and
            ...
            // carry one.
            // Not `InvalidForward`: draft-10 has no Forward field. Not
            ...
            // answers it the other way. Section 8.1.1 Version Specific Parameters: "Each
            ...
            // A Filter Type outside the set this draft assigns. Section 8.6
            ...
            // A Fetch Type outside the set this draft assigns. Section 8.12
    ...
    /// Close the session on the wire when a decode failure is one draft-10
...
mod tests {
    ...
    fn client_config_supported_versions_default() {
        ...
        assert_eq!(versions[0].into_inner(), 0xff000000 + 10);
    }
    ...
    fn client_config_alpn_quic() {
        ...
        assert_eq!(config.alpn(), vec![DraftVersion::Draft10.quic_alpn().to_vec()]);
    }
