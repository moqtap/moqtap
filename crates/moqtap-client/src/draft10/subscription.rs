...
/// Pure state machine for a MoQT subscription (draft-10).
...
impl SubscriptionStateMachine {
    ...
    /// from Active.
    ///
    /// Section 8.9 orders an update against the subscription rather than
    /// against the subscription's answer. All it asks of the identifier is
    /// that it already name something: "This MUST match an existing Subscribe
    /// ID" — and a Subscribe ID exists from the moment the SUBSCRIBE carrying
