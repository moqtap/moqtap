...
impl RequestIdAllocator {
    ...
    pub fn new(role: Role) -> Self {
        ...
        // Draft-17 removed MAX_REQUEST_ID and draft-18 keeps it gone;
    ...
    ///
    /// Draft-18 Section 10.1: "The client generates even numbered Request IDs,
    ...
    ///   MAX_REQUEST_ID. Draft-17 removed MAX_REQUEST_ID and draft-18 keeps it
...
mod tests {
    ...
    /// The error names the peer, not the endpoint that caught it. Draft-18
    /// Section 10.1 makes this a session close, so the reason phrase a peer
    ...
    /// Draft-17 removed MAX_REQUEST_ID and draft-18 did not bring it back, so
    /// the ceiling check in `validate_peer_id` is unreachable on this draft:
    /// the largest id a MoQT varint can carry is still accepted.
    #[test]
