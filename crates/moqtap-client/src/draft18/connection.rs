...
use crate::draft18::endpoint::{Endpoint, EndpointError};
...
pub enum ConnectionError {
    ...
    /// A control message this build decoded for draft-18 and then could not
    /// narrow to draft-18's own message type.
    ///
    ...
    #[error(
        "a control message decoded for draft-18 did not narrow to draft-18: a defect in this          build, and evidence about nothing the peer did"
    )]
...
impl FramedSendStream {
    ...
    pub async fn write_subgroup_header(
        ...
        match header {
            AnySubgroupHeader::Draft18(ref header) => {
                self.subgroup_io = Some(SubgroupObjectReader::new(header));
            }
            // Only this draft's header seeds the object reader. With draft 18 the only enabled
    ...
    /// Append a draft-18 subgroup object to the stream using the
...
impl FramedRecvStream {
            ...
            match AnySubgroupHeader::decode(self.draft, &mut cursor) {
                Ok(header) => {
                    ...
                    match header {
                        ...
                        // Only this draft's header seeds the object reader. With draft 18 the only
    ...
    /// Read the next draft-18 fetch header from this stream.
...
impl Connection {
    ...
    pub async fn adopt(
        ...
        match &server_setup {
            AnyControlMessage::Draft18(ControlMessage::Setup(ref s)) => {
    ...
    pub async fn recv_control(&mut self) -> Result<ControlMessage, ConnectionError> {
        ...
        match any {
            ...
            // `AnyControlMessage` carries one variant per enabled draft feature. With draft 18 the
    ...
    /// [`PeerViolation`] is a peer having done something draft-18 forbids, and
    /// carries the session error code draft-18's own text answers it with.
    ///
...
mod tests {
    ...
    #[test]
    fn client_config_alpn_quic_draft18() {
        ...
        assert_eq!(config.alpn(), vec![b"moqt-18".to_vec()]);
    }
