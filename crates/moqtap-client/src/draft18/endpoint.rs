...
use crate::draft18::fetch::{FetchError, FetchState, FetchStateMachine};
...
use crate::draft18::subscription::{
    SubscriptionError, SubscriptionState, SubscriptionStateMachine,
};
use crate::draft18::track_status::{TrackStatusError, TrackStatusState, TrackStatusStateMachine};
...
pub enum EndpointError {
    ...
    ///
    /// Section 10.14: "the subscriber cannot send REQUEST_UPDATE". A track status is
    /// the one request kind that sentence excludes, and Section 10.9's list of
    ...
    ///
    /// Draft-18 Section 10.9: "The sender of a request ... can later send a
    ...
    /// stream.
    ///
    /// Draft-18 Table 5 gives all three the Stream value "Request". NAMESPACE
    /// (0x8) "is sent on the response stream of a SUBSCRIBE_NAMESPACE request"
    /// (Section 10.16) and NAMESPACE_DONE (0xE) answers the same request — "All
    /// NAMESPACE_DONE messages are in response to a SUBSCRIBE_NAMESPACE"
    /// (Section 10.17). PUBLISH_BLOCKED (0xF) answers the other one: "All
    /// PUBLISH_BLOCKED messages are in response to a SUBSCRIBE_TRACKS"
    /// (Section 10.20). Only SETUP is "Control" alone; GOAWAY is the one
    /// message the table lists as "Control, Request".
    ///
    /// One of these three on the control stream names no request, so there is
    /// nothing it could be reporting on.
    #[error("{0} received on the control stream; draft-18 Table 5 places it on a request stream")]
    RequestMessageOnControlStream(&'static str),
    /// A server received a GOAWAY carrying a New Session URI.
    ///
    /// Draft-18 Section 10.4: "If a server receives a GOAWAY with a non-zero
    ...
    GoAwayUriAtServer,
    /// Track Properties arrived on a REQUEST_OK answering something other than
    /// a TRACK_STATUS.
    ///
    /// Draft-18 Section 10.5: they "are populated in TRACK_STATUS_OK; they are
    /// empty in PUBLISH_OK, REQUEST_UPDATE_OK, SUBSCRIBE_NAMESPACE_OK and
    ...
    ///
    /// Draft-18 Section 10.6.1: "If a server receives a Redirect with a
    ...
    ///
    /// Draft-18 Section 10.6.1: "Track Name is not meaningful for
    /// namespace-scoped requests (SUBSCRIBE_NAMESPACE, PUBLISH_NAMESPACE) and
    /// MUST be empty; an endpoint that receives a non-empty Track Name in a
    /// Redirect for a namespace-scoped request MUST close the session with a
    /// PROTOCOL_VIOLATION."
    ...
    ///
    /// Draft-18 Section 10.1: "If an endpoint receives a Request ID where the
    ...
    ///
    /// Draft-18 Sections 10.18 and 10.19, of SUBSCRIBE_NAMESPACE and
    ...
    ///
    /// Section 10.9 requires "exactly one REQUEST_OK or REQUEST_ERROR
    ...
    ///
    /// Draft-18 Section 11.1: "The same Track Alias MUST NOT be used by a
...
///
/// Section 10.2.14: "The TRACK_NAMESPACE_PREFIX parameter (Parameter Type
...
impl EndpointError {
    ...
    /// The code to close the session with, when draft-18 says this error is
    ...
    /// wrong message type; Section 10.1 gives INVALID_REQUEST_ID for a Request
    ...
    pub fn session_error_code(&self) -> Option<SessionErrorCode> {
        match self {
            // A REQUEST_UPDATE or a request message on the control stream is
            // refused and answers `None`, which is the arm at the foot of this
            // match. Neither is PROTOCOL_VIOLATION, because Section 3.3's
            // opener sentence does not cover them: that sentence is about what
            // a bidirectional stream may *begin* with, and a message on the
            // control stream begins nothing.
            // Draft-18 Section 10.9 describes where a REQUEST_UPDATE travels —
            ...
            // request to modify it." — and attaches no consequence to one that
            // arrives elsewhere. Drafts 19 and 20 add that consequence;
            // draft-18 has not got it, so a close here would be this crate's
            // model rather than the draft's.
...
pub struct Endpoint {
    ...
    ///
    /// Draft-18 Section 10.1 makes a duplicate Request ID a session close, and
    ...
    ///
    /// Section 10.9 gives an update the same two answers a request has, and
...
impl Endpoint {
    ...
    /// stream - Section 3.3.2. Every request lives at the front of a
    ...
    /// been answered yet.
    ///
    /// Draft-18 Section 10.9: "A subscriber can also send REQUEST_UPDATE to
    ...
    ///
    /// Draft-18 Section 10.9.1 gives a refused REQUEST_UPDATE three different
    ...
    /// or PUBLISH_NAMESPACE, the responder MUST close the bidi stream."
    ///
    ...
    pub fn receive_goaway(&mut self, msg: &GoAway) -> Result<(), EndpointError> {
        ...
        // Draft-18 Section 10.4: "The endpoint MUST close the session with a
    ...
    /// initiate migration of that individual request. Upon receiving a GOAWAY on
    /// a request stream, the endpoint SHOULD re-issue that specific request on a
    ...
    ///
    /// The session keeps running — only this one request is being moved — so the
    /// session state machine is not touched and nothing drains. Routing this
    /// form to [`receive_goaway`](Self::receive_goaway) instead would take the
    /// whole session to Draining over something the draft scopes to a single
    /// request.
    ///
    /// The server-side URI rule of the same section still applies here: a server
    /// may not be handed a migration URI on a request stream either.
    ///
    /// Re-issuing the request and closing the old stream are the caller's, since
    /// only the caller knows what the request was; this reports that the GOAWAY
    /// named a request that exists.
    ///
    /// # Errors
    ///
    ...
    ///
    /// The stream names the request being modified, not the message. Draft-18
    /// Section 10.9: "The sender of a request (SUBSCRIBE, PUBLISH, FETCH,
    ...
    /// Section 10.1 lists REQUEST_UPDATE among the messages that consume a
    /// Request ID of their own, so the id in the message body is the update's,
    /// not the target's — reading it as the target means every conforming
    ...
    /// draft names six senders and only one of them is a SUBSCRIBE.
    ...
    /// state move.
    ///
    /// One parameter is read rather than acknowledged. Section 10.2.14 puts
    /// TRACK_NAMESPACE_PREFIX on an update for a namespace subscription, and it
    /// is the only thing an update on any draft can change about a request that
    /// this endpoint has to judge before answering.
    pub fn receive_request_update(
        ...
        let id = request_id.into_inner();
        // Section 10.14 takes the update away from this request kind outright:
        ...
        let updatable = if let Some(sm) = self.subscriptions.get_mut(&id) {
            ...
            self.publishes.contains_key(&id)
                || self.fetches.contains_key(&id)
                || self.subscribe_namespaces.contains_key(&id)
                // Draft-18 added SUBSCRIBE_TRACKS and put it in Section 10.9's
                // list of the requests an update may modify. It gets its own map
                // for the reason the outbound path gives it one, so it needs its
                // own probe here.
                || self.subscribe_tracks.contains_key(&id)
                || self.publish_namespaces.contains_key(&id)
        };
        ...
        *self.unanswered_peer_updates.entry(id).or_insert(0) += 1;
        ...
        // (Section 10.2.14) in a REQUEST_UPDATE."
        //
    ...
    ///
    /// `joining_start` is an offset rather than a group number: draft-18
    ...
    ///
    /// Draft-18 Section 10.12.2.1: "For an Absolute Joining Fetch, the
    ...
    /// one, and Section 10.12.2 leaves the choice with the subscriber: "The
    ...
    pub fn receive_request_ok(
        ...
        let id = request_id.into_inner();
        // Draft-18 Section 10.5: Track Properties "are populated in
    ...
    pub fn receive_request_error(
        ...
        if let Some(redirect) = &msg.redirect {
            ...
            // requests (SUBSCRIBE_NAMESPACE, PUBLISH_NAMESPACE) and MUST be
            // empty; an endpoint that receives a non-empty Track Name in a
            // Redirect for a namespace-scoped request MUST close the session
            // with a PROTOCOL_VIOLATION." Which request this answers is known
            // only here, from the map the stream's id is in. Draft-18 names
            // those two and not SUBSCRIBE_TRACKS, so SUBSCRIBE_TRACKS is not
            // held to the rule on this draft.
    ...
    /// it was made on rather than by sending a message. Section 3.3.2: "Once
    ...
    /// been written yet.
    ///
    /// [`Connection::cancel_request_stream`]: crate::draft18::connection::Connection::cancel_request_stream
    ...
    ///
    /// Draft-18 Table 5 gives every message a Stream value, and only two of
    ...
    /// (Sections 10.16 and 10.17), and PUBLISH_BLOCKED names a track that
    /// cannot be published on the SUBSCRIBE_TRACKS stream that asked for it
    ...
    pub fn receive_message(&mut self, msg: ControlMessage) -> Result<(), EndpointError> {
        match msg {
            ...
            // over — and draft-18 closes over none of them; see
            // `session_error_code`. A control message carries its own
            // length, so the next boundary on the stream is known and a
            // refused message costs the session nothing; that is the answer
            // `ResponseOnControlStream` below has always given.
            ControlMessage::RequestUpdate(_) => Err(EndpointError::RequestUpdateOnControlStream),
            ...
            ControlMessage::PublishBlocked(_) => {
                Err(EndpointError::RequestMessageOnControlStream("PUBLISH_BLOCKED"))
            }
    ...
    /// SUBSCRIBE_TRACKS goes on to carry PUBLISH_BLOCKED, and those are exactly the
    ...
    /// by `request_id`.
    ///
    /// Beyond the five responses this also takes the messages draft-18 Table 5
    ...
    /// report — Section 10.16 says NAMESPACE "is sent on the response stream of
    /// a SUBSCRIBE_NAMESPACE request" — and PUBLISH_BLOCKED (0xF) on the
    /// SUBSCRIBE_TRACKS stream that asked for the track it says cannot be
    /// published: "All PUBLISH_BLOCKED messages are in response to a
    /// SUBSCRIBE_TRACKS" (Section 10.20). Table 5 marks all three "Request", so
    /// this is where they land; the control stream refuses them.
    ///
    ...
    pub fn receive_response_on_stream(
        ...
        match msg {
            ...
            // Section 10.9 puts a REQUEST_UPDATE on the stream of the request it
            // modifies, which is this one.
            ControlMessage::RequestUpdate(ref m) => self.receive_request_update(request_id, m),
            // Table 5 marks GOAWAY (0x10) "Control, Request". Section 10.4's
            // per-request migration form arrives here, and must not be routed
            // to the session-wide handler.
    ...
    /// # Seven kinds, not six
    ///
    /// Draft-18 added SUBSCRIBE_TRACKS (0x51) to the set Section 3.3 lets a
    /// bidirectional stream begin with, and renumbered SUBSCRIBE_NAMESPACE from
    /// 0x11 to 0x50. Both share the namespace-subscription lifecycle, so they
    /// share a state machine type while keeping separate maps: Section 10.19
    /// gives them "independent overlap spaces", so a prefix registered by one
    /// is not registered by the other.
    ///
    ...
    /// 10.2.14 weighs against "another active subscription of the same type"
    ...
    ///
    /// Draft-18's Table 5 marks NAMESPACE (0x8), NAMESPACE_DONE (0xE) and
    /// PUBLISH_BLOCKED (0xF) "Request", and each is written by the *responder*
    /// on the stream of the request that asked for it: Section 10.16 says
    /// NAMESPACE "is sent on the response stream of a SUBSCRIBE_NAMESPACE
    /// request", Section 10.17 says the same of NAMESPACE_DONE, and Section
    /// 10.20 says "All PUBLISH_BLOCKED messages are in response to a
    /// SUBSCRIBE_TRACKS". None of the three ends its request, so each is a
    /// self-transition on an Active namespace subscription; sending one before
    /// the REQUEST_OK that made it Active is refused.
    ///
    /// Draft-18 folded PUBLISH_OK into REQUEST_OK (Section 10.5), so REQUEST_OK
    /// is also what answers a peer's PUBLISH here, where draft-17 had a message
    /// of its own for it.
    ///
    ...
    ///
    /// Section 10.9 gives an update the same two answers a request has: "The
    ...
    /// answering. Draft-17 has a third, PUBLISH, which this draft folded into
    /// REQUEST_OK.
    ///
    ...
    /// error." Draft-17 has no such paragraph and answers one update per
    ...
    fn answer_an_update(&mut self, id: u64, msg: &ControlMessage) -> Result<(), EndpointError> {
        ...
        // the sentence names are known. Section 10.2.14: "If the new prefix
    ...
    /// an overlapping prefix, and each flow's own
    ...
    pub fn send_response_on_stream(
        ...
        match msg {
            ...
            // REQUEST_OK answers the five kinds with no response message of
            // their own, PUBLISH among them on this draft. The probe order does
            // not matter: the maps are keyed by Request ID and one id belongs
            // to one request.
            ControlMessage::RequestOk(_) => {
                if let Some(sm) = self.publishes.get_mut(&id) {
    ...
    /// [`EndpointError::UnexpectedOnPeerRequestStream`]. The same goes for
    /// NAMESPACE, NAMESPACE_DONE and PUBLISH_BLOCKED, which Table 5 puts on a
    /// request stream but which only ever travel from the responder to the
    /// requester — see [`send_response_on_stream`](Self::send_response_on_stream).
    ///
    /// Two messages are expected instead.
    ///
    /// REQUEST_UPDATE, because draft-18 Section 10.9 puts it on the request's
    /// own stream: "The sender of a request (SUBSCRIBE, PUBLISH, FETCH,
    /// PUBLISH_NAMESPACE, SUBSCRIBE_NAMESPACE, SUBSCRIBE_TRACKS) can later send
    /// a REQUEST_UPDATE on the same bidi stream as the request to modify it."
    /// The request it modifies is therefore the stream's, which is why
    /// `request_id` here comes from the stream and the message's own Request ID
    /// field is not consulted — Section 10.1 says REQUEST_UPDATE consumes a
    /// Request ID of its own, so that field does not name the request being
    /// updated. Only a subscription carries a transition for an update; for the
    /// other kinds draft-18 defines no state change, so the message is accepted
    /// and the state left alone.
    ///
    ...
    pub fn receive_on_peer_request_stream(
        ...
        match msg {
            ...
            // peer never opened is in none of the maps that method probes, so
            // this arm needs no check of its own.
            ControlMessage::RequestUpdate(ref m) => self.receive_request_update(request_id, m),
...
impl PublishStateMachine {
    ...
    ///
    /// Draft-18 folded PUBLISH_OK into REQUEST_OK (Section 10.5), so the
    /// message this edge is taken on is a REQUEST_OK here where draft-17 had a
    /// PUBLISH_OK of its own.
...
impl SubscribeNamespaceStateMachine {
    ...
    /// stream).
    ///
    /// Draft-18 Section 10.16 puts NAMESPACE "on the response stream of a
    /// SUBSCRIBE_NAMESPACE request", and Table 5 marks it "Request" rather than
    /// "Control". Reporting a namespace neither completes the subscription nor
    /// ends it, so the edge is a self-loop; what it does rule out is announcing
    /// a namespace on a subscription that has not been accepted yet.
    pub fn on_namespace_sent(&mut self) -> Result<(), NamespaceError> {
        self.stay_active("on_namespace_sent")
    }

    /// Active -> Active (NAMESPACE_DONE written on the peer's
    /// SUBSCRIBE_NAMESPACE stream).
    ///
    /// Section 10.17: NAMESPACE_DONE says the publisher will stop serving new
    /// subscriptions for one namespace, not that the namespace subscription
    /// itself is over — the peer keeps hearing about the others — so this is a
    /// self-loop like [`on_namespace_sent`](Self::on_namespace_sent).
    pub fn on_namespace_done_sent(&mut self) -> Result<(), NamespaceError> {
        self.stay_active("on_namespace_done_sent")
    }

    /// Active -> Active (PUBLISH_BLOCKED written on the peer's SUBSCRIBE_TRACKS
    /// stream).
    ///
    /// Section 10.20: "All PUBLISH_BLOCKED messages are in response to a
    /// SUBSCRIBE_TRACKS", reporting one track the publisher cannot send a
    /// PUBLISH for. The rest of the subscription is unaffected, so this too is
    /// a self-loop.
    pub fn on_publish_blocked_sent(&mut self) -> Result<(), NamespaceError> {
        self.stay_active("on_publish_blocked_sent")
    }

    /// The shared body of the three self-loops above: accepted only in
    /// [`SubscribeNamespaceState::Active`], and moving nothing.
    ///
    /// Written as a state check rather than as a delegation because no existing
    /// transition on this machine walks Active to Active, so there is nothing
    /// to delegate to.
    fn stay_active(&mut self, event: &str) -> Result<(), NamespaceError> {
...
mod responder_tests {
    use super::*;
    use crate::draft18::publish::PublishState;
    use crate::draft18::subscription::SubscriptionState;
    use crate::draft18::track_status::TrackStatusState;
    ...
    fn peer_subscribe_namespace(id: u64) -> ControlMessage {
    ...
    fn peer_subscribe_tracks(id: u64) -> ControlMessage {
    ...
    /// Draft-18 Section 10.1: a Request ID whose least significant bit is wrong
    ...
    /// Draft-18 Section 3.3 gives a different code for a different rule: a
    ...
    fn a_stream_that_opens_no_request_closes_the_session_with_protocol_violation() {
        let mut ep = active_client();
        let not_a_request = ControlMessage::GoAway(GoAway {
    ...
    /// Draft-18 Section 10.5 folded PUBLISH_OK into REQUEST_OK, so a peer's
    /// PUBLISH is accepted with the same message that accepts a TRACK_STATUS.
    /// Draft-17 had PUBLISH_OK as a message of its own here, which is the port
    /// this most easily gets wrong: a REQUEST_OK arm that does not probe
    /// `publishes` leaves a peer's PUBLISH unanswerable.
    ///
    /// Removing the `publishes` probe from the `RequestOk` arm of
    /// `send_response_on_stream` fails with:
    ///
    /// ```text
    /// called `Result::unwrap()` on an `Err` value: UnknownRequest(1)
    /// ```
    #[test]
    fn a_peers_publish_is_accepted_with_request_ok_on_this_draft() {
        let mut ep = active_client();
    ...
    /// SUBSCRIBE_TRACKS (0x51) is the seventh request kind, new in draft-18. It
    /// gets a map of its own rather than sharing SUBSCRIBE_NAMESPACE's, because
    /// Section 10.19 gives the two "independent overlap spaces"; a peer that
    /// sends both must end up with two registrations, not one.
    #[test]
    fn the_seventh_request_kind_is_registered_apart_from_subscribe_namespace() {
        let mut ep = active_client();
        let sn = ep.receive_request_on_stream(&peer_subscribe_namespace(1)).unwrap();
        let st = ep.receive_request_on_stream(&peer_subscribe_tracks(3)).unwrap();
        assert_eq!(ep.active_subscribe_namespace_count(), 1);
        assert_eq!(ep.active_subscribe_tracks_count(), 1);

        ep.send_response_on_stream(sn, &request_ok()).unwrap();
        ep.send_response_on_stream(st, &request_ok()).unwrap();
        assert_eq!(ep.subscribe_namespaces[&1].state(), SubscribeNamespaceState::Active);
        assert_eq!(ep.subscribe_tracks[&3].state(), SubscribeNamespaceState::Active);
    }

    /// The three messages draft-18 Table 5 places on a request stream that are
    /// not responses: NAMESPACE (0x8), NAMESPACE_DONE (0xE) and PUBLISH_BLOCKED
    /// (0xF), on the **receive** side.
    ///
    ...
    /// sending NAMESPACE on the SUBSCRIBE_NAMESPACE stream that asked for it had
    /// its announcement dropped, which is what made SUBSCRIBE_NAMESPACE unusable
    /// end to end on this draft.
    ///
    ...
    fn namespace_and_publish_blocked_arrive_on_a_request_stream() {
        ...
        for (name, build) in cases {
            let mut ep = active_client();
            let id = ep.subscribe_namespace(ns(), vec![]).unwrap().0;
            ...
            // And refused *without* a close. Section 3.3's opener sentence
            // does not reach a message arriving on the control stream — that
            // sentence is about what a bidirectional stream may *begin* with —
            // and no sentence in draft-18 closes a session over a message
            // being in the wrong place. A library that closes a session over
            // its own model of the protocol hands every consumer an accusation
            // the draft will not support.
            assert_eq!(
                err.session_error_code(),
                None,
                "no sentence in draft-18 answers {err} with a close"
            );
    ...
    /// Draft-18's Table 5 marks NAMESPACE (0x8) and NAMESPACE_DONE (0xE)
    /// "Request", and Section 10.16 says NAMESPACE "is sent on the response
    /// stream of a SUBSCRIBE_NAMESPACE request". So a responder writes them on
    /// the peer's SUBSCRIBE_NAMESPACE stream, and only once that subscription
    /// has been accepted — before the REQUEST_OK there is nothing to report on.
    ///
    /// Neither ends the subscription: it is still Active afterwards, which is
    /// what lets a second namespace follow.
    ///
    /// Letting `on_namespace_sent` accept any state fails with:
    ///
    /// ```text
    /// a NAMESPACE was accepted on a namespace subscription that was never answered
    /// ```
    #[test]
    fn namespaces_are_reported_on_the_subscribe_namespace_request_stream() {
        let mut ep = active_client();
        let id = ep.receive_request_on_stream(&peer_subscribe_namespace(1)).unwrap();
        let namespace = ControlMessage::Namespace(message::Namespace { namespace_suffix: ns() });

        assert!(
            ep.send_response_on_stream(id, &namespace).is_err(),
            "a NAMESPACE was accepted on a namespace subscription that was never answered",
        );
        ...
        ep.send_response_on_stream(id, &namespace).unwrap();
        let done = ControlMessage::NamespaceDone(message::NamespaceDone { namespace_suffix: ns() });
        ep.send_response_on_stream(id, &done).unwrap();
        assert_eq!(
            ep.subscribe_namespaces[&1].state(),
            SubscribeNamespaceState::Active,
            "reporting a namespace ended the namespace subscription",
        );
    }

    /// Table 5 marks PUBLISH_BLOCKED (0xF) "Request" too, and Section 10.20
    /// names the stream it belongs on: "All PUBLISH_BLOCKED messages are in
    /// response to a SUBSCRIBE_TRACKS". A SUBSCRIBE_NAMESPACE stream is
    /// therefore the wrong one, and that is checked here rather than assumed —
    /// the two requests are separate maps precisely so this can be told apart.
    #[test]
    fn publish_blocked_belongs_to_a_subscribe_tracks_stream_only() {
        let mut ep = active_client();
        let namespaces = ep.receive_request_on_stream(&peer_subscribe_namespace(1)).unwrap();
        let tracks = ep.receive_request_on_stream(&peer_subscribe_tracks(3)).unwrap();
        ep.send_response_on_stream(namespaces, &request_ok()).unwrap();
        ep.send_response_on_stream(tracks, &request_ok()).unwrap();

        let blocked = ControlMessage::PublishBlocked(PublishBlocked {
            namespace_suffix: ns(),
            track_name: b"t".to_vec(),
        });
        assert!(
            matches!(
                ep.send_response_on_stream(namespaces, &blocked),
                Err(EndpointError::UnknownRequest(1)),
            ),
            "PUBLISH_BLOCKED was accepted on a SUBSCRIBE_NAMESPACE stream",
        );
        ep.send_response_on_stream(tracks, &blocked).unwrap();
        assert_eq!(ep.subscribe_tracks[&3].state(), SubscribeNamespaceState::Active);
    }
    ...
    /// Draft-18 Section 10.9 puts REQUEST_UPDATE on the request's own stream,
    ...
    /// Section 10.1 has REQUEST_UPDATE consume one.
    #[test]
    fn a_peers_request_update_is_correlated_by_the_stream() {
        ...
        // The Request ID field carries 5, an id no request of ours or theirs
        // has opened. The update still lands on the stream's subscription.
        let update =
            ControlMessage::RequestUpdate(RequestUpdate { request_id: v(5), parameters: vec![] });
        ep.receive_on_peer_request_stream(id, update).unwrap();
        assert_eq!(ep.subscriptions[&1].state(), SubscriptionState::Active);
    }

    /// A peer's TRACK_STATUS is answered with REQUEST_OK and is then over —
    /// unlike every other kind, nothing follows it on the stream, which is why
    /// its state machine has no Active node.
    #[test]
    fn a_peers_track_status_is_done_once_it_is_answered() {
        let mut ep = active_client();
        let id = ep
            ...
            .unwrap();
        assert_eq!(ep.track_statuses[&1].state(), TrackStatusState::Pending);
        ep.send_response_on_stream(id, &request_ok()).unwrap();
        assert_eq!(ep.track_statuses[&1].state(), TrackStatusState::Done);
    }
}
