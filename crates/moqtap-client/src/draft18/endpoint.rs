...
impl Endpoint {
    ...
    fn fail_session(&mut self, err: EndpointError) -> EndpointError {
        // `on_close` only accepts Active and Draining. A violation seen in any
        // other state leaves the state machine alone: there is no session to
        // close, and the error itself is still the answer.
