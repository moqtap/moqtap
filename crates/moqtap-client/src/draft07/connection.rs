...
pub enum ConnectionError {
    ...
    /// A control message this build decoded for draft-07 and then could not
    /// narrow to draft-07's own message type.
    ///
    ...
    #[error(
        "a control message decoded for draft-07 did not narrow to draft-07: a defect in this          build, and evidence about nothing the peer did"
    )]
...
impl Connection {
    ...
    /// same config constructor in `transport::quic`, so a certificate is
    /// judged against the same thing on either — the bundled roots plus
    /// `config.ca_certs`, or nothing at all when `skip_cert_verification` is
    /// set — and one relay gets one verdict whichever transport carries it.
    /// Settling trust at this call site instead — from `wtransport`'s own
    /// builder settings, or from a second config of this draft's own — puts the
    /// decision in two places, and a caller's private CA then reaches only the
    /// dials whose call site installed it.
    ...
    /// pass. See
    /// [`Endpoint::next_free_track_alias`](crate::draft07::endpoint::Endpoint::next_free_track_alias)
    ...
    ///
    /// Draft-07 adds none. Every variant of its [`ConnectionError`] is one of the
    ...
    /// 10 do state. Object Status 0x5 is end of Subgroup on this draft —
    /// "Object ID is one greater than the largest normal object ID in the
    /// Subgroup" — and no consequence is named for an Object ID that disagrees.
    /// Draft-08 reassigns 0x5 to end of Track and adds one: an object with that
    /// status and "an Object ID other than zero, is a protocol error, and the
    /// receiver MUST terminate the session."
    ///
    ...
    pub fn codec_session_error_code(
        ...
        match err {
            ...
            // arm would make silently: for every variant at once, and in each
            // draft module that carried one.
            //
