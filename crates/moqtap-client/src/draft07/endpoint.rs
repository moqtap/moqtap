...
use crate::draft07::fetch::{FetchError, FetchState, FetchStateMachine};
use crate::draft07::namespace::{
    AnnounceState, AnnounceStateMachine, NamespaceError, SubscribeAnnouncesState,
    SubscribeAnnouncesStateMachine,
};
use crate::draft07::session::setup::{self, SetupError};
use crate::draft07::session::state::{SessionError, SessionState, SessionStateMachine};
use crate::draft07::session::subscribe_id::{SubscribeIdAllocator, SubscribeIdError};
use crate::draft07::subscription::{
    SubscriptionError, SubscriptionState, SubscriptionStateMachine,
};
use crate::draft07::track_status::{TrackStatusError, TrackStatusState, TrackStatusStateMachine};
use crate::forwarding_preference::{ObjectForwardingPreference, TrackForwardingPreferences};
use moqtap_codec::draft07::error_codes::{SessionErrorCode, SubscribeErrorCode};
use moqtap_codec::draft07::message::{
    self, Announce, AnnounceCancel, AnnounceError, AnnounceOk, ClientSetup, ControlMessage, Fetch,
    FetchCancel, GoAway, MaxSubscribeId, ServerSetup, Subscribe, SubscribeAnnounces,
...
///
/// Two rules on this draft turn on the answer, and both live in the setup and
/// migration messages rather than in any per-request state: the PATH parameter
/// belongs to the client, and a GOAWAY may only travel from the server to the
/// client. Subscribe IDs on this draft are a single session-wide sequence and do
/// not depend on who allocates them, so nothing else reads this.
...
/// Errors that can occur during draft-07 endpoint operations.
#[derive(Debug, thiserror::Error)]
pub enum EndpointError {
    /// A GOAWAY arrived at a server.
    ///
    /// Section 6.3: "The server MUST terminate the session with a Protocol
    /// Violation (Section 3.5) if it receives a GOAWAY message." This draft states the rule
    /// about the message rather than about its contents - a GOAWAY carrying no
    /// URI at all is refused here just the same. Draft-08 replaced the sentence
    /// with a narrower one that turns on a non-zero New Session URI Length, so
    /// this refusal is draft-07's alone and does not carry forward.
    #[error("GOAWAY received at a server")]
    GoAwayAtServer,
    ...
    ///
    /// The send-side mirror of the rule a peer breaks by sending one — Section 6.20:
    ...
    /// about.
    ///
    /// Section 6.12 makes the request the subscriber's: "A potential subscriber
    ...
    /// A SUBSCRIBE arrived for a namespace the peer had cancelled with
    /// ANNOUNCE_CANCEL.
    #[error("subscribe for a namespace the peer cancelled")]
    ...
    ///
    /// Section 6.4, on the Track Alias the subscriber chooses in SUBSCRIBE:
    ...
    /// Section 6.16 states the other end of the same rule, on the alias a
    ...
    /// that chooses the alias. Section 3.5 describes the code as "The
    /// endpoint attempted to use a Track Alias that was already in use", and
    /// Section 6.4 says what the receiving publisher does about it, so a
    ...
    ///
    /// Section 7: "Every Track has a single 'Object Forwarding Preference' and
    ...
    /// has ever been given.
    ///
    /// Section 6.5: "A publisher SHOULD close the Session as a 'Protocol
    /// Violation' if the SUBSCRIBE_UPDATE violates either rule or if the
    /// subscriber specifies a Subscribe ID that does not exist within the Session."
    ///
    ...
    /// announced.
    ///
    /// Section 6.11 says what a cancellation is for: the subscriber "will stop
    ...
    /// not subscribed to.
    ///
    /// Section 6.14: "A subscriber issues a UNSUBSCRIBE_ANNOUNCES message to a publisher indicating it is no longer interested in ANNOUNCE and UNANNOUNCE messages for the specified track namespace prefix."
    ///
    ...
    ///
    /// Section 6.13: "A subscriber cannot make overlapping namespace
...
impl EndpointError {
    ...
    pub fn fault(&self) -> crate::above_codec_rules::EndpointFault {
        ...
        match self {
            ...
            EndpointError::GoAwayAtServer => Fault::Peer(Rule::GoAwayAtServer),
            ...
            EndpointError::SubscribeAfterAnnounceCancel => {
                Fault::Peer(Rule::SubscribeAfterAnnounceCancel)
            }
    ...
    /// The code to close the session with, when draft-07 answers this error
    ...
    pub fn session_error_code(&self) -> Option<SessionErrorCode> {
        match self {
            // Section 6.20 answers a ceiling that does not increase with a
            ...
            // Section 6.11 answers a SUBSCRIBE arriving for a namespace
            // the peer has cancelled, and names this code in the same
            // sentence: "it SHOULD close the session as a 'Protocol
            // Violation'".
            EndpointError::SubscribeAfterAnnounceCancel => {
                Some(SessionErrorCode::ProtocolViolation)
            }
            // Section 6.3 answers a GOAWAY that repeats one already
            // received, and names this code in the same sentence.
            EndpointError::RepeatedGoAway => Some(SessionErrorCode::ProtocolViolation),
            // The same section answers a GOAWAY arriving at a server at all -
            // this draft states the rule about the message rather than about
            // the URI it carries - and names this code for it.
            EndpointError::GoAwayAtServer => Some(SessionErrorCode::ProtocolViolation),
            // Section 6.4 answers a SUBSCRIBE whose Track Alias already
            // names a different track with a session close, and Section 6.16
            // answers the retry alias a SUBSCRIBE_ERROR offers the same way.
            // Section 3.5 names this code for both.
            EndpointError::DuplicateTrackAlias { .. } => {
                Some(SessionErrorCode::DuplicateTrackAlias)
            }
            // Section 7 answers a track whose objects mix forwarding
...
/// Unified draft-07 MoQT endpoint wrapping session lifecycle, subscribe ID
...
pub struct Endpoint {
    ...
    /// a case Section 6.13 forbids rather than one this map decides.
    ...
    /// Namespaces the peer has cancelled with ANNOUNCE_CANCEL.
    ///
    /// Separate from the announcement's own state, which reaches Done by two
    /// routes: this endpoint sending UNANNOUNCE, and the peer sending
    /// ANNOUNCE_CANCEL. Section 6.11 is about the second only, so a Done
    /// announcement is not enough to judge by.
    ///
    /// It is retired rather than accumulated: announcing the same namespace
    /// again replaces the entry below and clears this, because the peer's
    /// cancel was of the announcement that is now over.
    announce_cancelled: HashSet<NamespaceKey>,
...
impl Endpoint {
    /// Create a new draft-07 endpoint for the given role.
    pub fn new(role: Role) -> Self {
        Self {
            ...
            announce_cancelled: HashSet::new(),
    ...
    ///
    /// Section 6.4 says "already being used", and a subscription that has
    ...
    /// Record how a track's object was framed, and report Section 7's "MUST NOT
    ...
    /// from draft-12 onwards: there the sentence is, at draft-12 Section 8.8,
    /// "a different track with an active subscription" and the alias arrives in
    /// the answer, so only an answered request holds one. Here the alias is in
    /// the SUBSCRIBE itself, and the sentence puts no qualifier on "already
    /// being used" - so it is in use from the moment that message is sent or
    ...
    /// The close Section 6.4 requires of an arriving SUBSCRIBE whose Track
    ...
    /// The close Section 6.16 requires of a SUBSCRIBE_ERROR offering a Track
    ...
    ///
    /// The form that can answer with a MAX_SUBSCRIBE_ID. Section 6.2.2.3
    ...
    pub fn receive_client_setup_and_respond_with(
        ...
        // Section 6.2.2.3 puts no role restriction on MAX_SUBSCRIBE_ID, so a
    ...
    /// 6.2.2.3 gives it a default of 0 - so it is not put through the
    ...
    /// Hold a Subscribe ID the peer chose to the rules Section 6.4
    /// states about it.
    ///
    ...
    /// session's Maximum Subscribe ID", and Section 6.7 repeats the
    ...
    ///
    /// Section 6.20: "If a Subscribe ID equal or larger than this is received
    /// in any message, including SUBSCRIBE, the publisher MUST close the
    /// session with an error of 'Too Many Subscribes'." An id that reaches
    /// the ceiling is not a SUBSCRIBE to refuse with a SUBSCRIBE_ERROR: the
    /// session is over, so this moves the endpoint's own state to Closed and
    /// leaves the code to [`EndpointError::session_error_code`].
    ///
    /// # Errors
    ///
    ...
    pub fn receive_subscribe(&mut self, msg: &Subscribe) -> Result<(), EndpointError> {
        ...
        // Section 6.11: "If a publisher receives new subscriptions for that
        // namespace after receiving an ANNOUNCE_CANCEL, it SHOULD close the
        // session as a 'Protocol Violation'." No other draft states it - the
        // sentence is draft-07's alone - and this endpoint is the publisher of
        // a SUBSCRIBE that arrives, so this is where it is answered.
        //
        // A SHOULD, and this endpoint takes it: the draft names both the
        // action and the code, so closing is conforming and leaving the
        // subscription open would serve a namespace the peer has said it no
        // longer wants announced.
        if self.announce_cancelled.contains(&msg.track_namespace.0) {
            return Err(self.fail_session(EndpointError::SubscribeAfterAnnounceCancel));
        }
        // Section 6.4: "If the Track Alias is already being used for a
    ...
    /// smaller Subscribe ID value is a 'Protocol Violation'." Section 3.5 lists
    ...
    /// # Errors
    ///
    /// [`EndpointError::GoAwayAtServer`] if this endpoint is the server.
    /// Section 6.3: "The server MUST terminate the session with a Protocol
    /// Violation (Section 3.5) if it receives a GOAWAY message." Migration is something a
    /// server offers a client, and on this draft the direction is stated about
    /// the message rather than about the URI it carries - so an empty GOAWAY is
    /// refused here too, where on draft-08 and later it would be allowed
    /// through. Refused before the session is moved to Draining, so a server
    /// cannot be talked into draining by a client that had no standing to ask.
    /// The session is over: this endpoint's own state has moved to Closed and
    /// the code the transport should close with is in
    /// [`EndpointError::session_error_code`].
    ///
    ...
    pub fn receive_goaway(&mut self, msg: &GoAway) -> Result<(), EndpointError> {
        if self.role == Role::Server {
            return Err(self.fail_session(EndpointError::GoAwayAtServer));
        }
        // Section 6.3: "The client MUST terminate the session with a Protocol
        // Violation (Section 3.5) if it receives multiple GOAWAY messages." Draining is
        // reached from nowhere else - `on_goaway` is its only entry and this
        // method is that method's only caller - so the session state is the
        // record of the first GOAWAY having arrived. The subject is the client
        // because the sentence beside it makes any GOAWAY at a server a close,
        // so a server never reaches a second one.
    ...
    pub fn subscribe_range(
        ...
        let filter_type = match end_location {
    ...
    fn subscribe_inner(
        ...
        let (end_group, end_object) = match end_location {
            Some(end) => (Some(end.group), Some(end.object)),
            None => (None, None),
        };
    ...
    pub fn receive_subscribe_error(&mut self, msg: &SubscribeError) -> Result<(), EndpointError> {
        let id = msg.subscribe_id.into_inner();
        // Section 6.16 gives SUBSCRIBE_ERROR a Track Alias field with one
    ...
    ///
    /// Section 6.5 gives the message to the subscriber, which is what this
    ...
    ///
    /// Section 6.5: "A subscriber issues a SUBSCRIBE_UPDATE to a publisher to
    /// request a change to a prior subscription." One that arrives is therefore
    /// about a subscription the **peer** opened, which is why it is looked for
    ...
    /// ended. Neither ends the session: Section 6.5 says SHOULD.
    ...
    ///
    /// The Track Alias goes back out with the refusal because Section 6.16
    ...
    /// draws from: Section 6.7 gives it the same "unique and monotonically
    ...
    pub fn receive_fetch(&mut self, msg: &Fetch) -> Result<(), EndpointError> {
        ...
        self.inbound_fetches.insert(id, InboundFetch { message: msg.clone(), state });
        Ok(())
    }
    ...
    /// that identifier, and the fetch flow's own `InvalidTransition` for a
    /// second answer. This draft states no rule about how many answers a
    /// FETCH gets; the flow refuses the second one because a fetch that has
    /// been answered has left the state an answer is sent from.
    ...
    ///
    /// Section 6.8: the subscriber sends it to stop a fetch it no longer
    ...
    ///
    /// Section 6.13: "The subscriber sends the SUBSCRIBE_ANNOUNCES control
    ...
    /// the peer made.
    ///
    /// Section 6.13: "The publisher will respond with SUBSCRIBE_ANNOUNCES_OK
    /// or SUBSCRIBE_ANNOUNCES_ERROR."
    ///
    ...
    ///
    /// Section 6.14: "A subscriber issues a UNSUBSCRIBE_ANNOUNCES message to
    /// a publisher indicating it is no longer interested in ANNOUNCE and
    /// UNANNOUNCE messages for the specified track namespace prefix."
    ///
    ...
    pub fn announce(
        ...
        sm.on_announce_sent()?;
        self.announce_cancelled.remove(&key);
    ...
    pub fn receive_announce_cancel(&mut self, msg: &AnnounceCancel) -> Result<(), EndpointError> {
        ...
        self.announce_cancelled.insert(msg.track_namespace.0.clone());
        Ok(())
    }
    ...
    ///
    /// Section 6.21: "The publisher sends the ANNOUNCE control message to
    ...
    ///
    /// Section 5.2: "The entity receiving the ANNOUNCE MUST send only a single
    /// response to a given ANNOUNCE of either ANNOUNCE_OK or ANNOUNCE_ERROR."
    ///
    ...
    ///
    /// Section 6.22: "The publisher sends the UNANNOUNCE control message to
    ...
    ///
    /// Section 5.2 names what a cancellation revokes: a namespace "it
    /// previously responded ANNOUNCE_OK to". Section 6.11 says what it does:
    ...
    /// about.
    ///
    /// Section 6.12: "A potential subscriber sends a 'TRACK_STATUS_REQUEST'
    ...
    ///
    /// Section 6.12 leaves the answering end no discretion about whether to
    ...
    ///
    /// Section 6.23 says which track the answer is about, and this draft has
