...
//!
//! **`FILL_PARAMETERS` goes out as the block draft-21 defines.** Section
//! 9.20.16 says the value is "a sequence of Parameters that apply to the fill
...
//! count is a choice — made in `draft21::fill`, and made again in the codec's
...
//!
//! **A fill fetch stream is attributable.** Section 3.4: "The FETCH_HEADER on
...
//! what Section 3.4.1 requires.
//!
//! **PUBLISH_STATE_NOTIFY is taken and answered with nothing.** Section 9.10
...
use moqtap_client::draft21::event::{ClientEvent, StreamKind};
use moqtap_client::draft21::fill::{FillParameters, LocationFilter};
use moqtap_client::draft21::observer::ConnectionObserver;
use moqtap_codec::dispatch::{AnyControlMessage, AnyFetchHeader};
use moqtap_codec::draft21::data_stream::FetchHeader;
...
/// `LARGEST_OBJECT`, Parameter Type 0x09 — the parameter Section 9.10 says a
...
/// last object of a range ending at 9; draft-21 Sections 3.3.1 and 9.11 make
...
/// The answer that binds the alias. A draft-21 response carries no Request ID —
...
/// A PUBLISH_STATE_NOTIFY carrying the one parameter Section 9.10 requires.
fn state_notify_bytes() -> Vec<u8> {
    // A Location is two consecutive varints, with no length ahead of them
    // (Section 9.20.18). Both fit one byte, so these are the varints.
...
async fn serve(server: quinn::Endpoint) -> WhatThePeerSaw {
    ...
    // The fill, opened and finished. Section 3.4.1: "The publisher signals
...
async fn a_subscription_asks_for_a_fill_reads_its_stream_and_takes_a_state_notify() {
    ...
    assert_eq!(
        ...
        "Section 3.4 puts the SUBSCRIBE's Request ID on the initial fill's header"
    );
    ...
    assert_eq!(
        ...
        1,
        "Section 3.4.1: ending a fill fetch stream does not affect the subscription"
    );
    ...
    assert!(
        ...
        "Section 9.10 makes the notify unilateral: nothing is owed in reply"
    );
