...
#[cfg(feature = "draft21")]
mod draft21 {
    use moqtap_client::draft21::session::request_id::Role;
    use moqtap_client::draft21::session::setup::{
        validate_client_path_transport, validate_setup, SetupError,
    };
    ...
    /// Draft-21 Section 9.1.2 is draft-19's sentence unchanged, and so is
    /// the one message both directions use: nothing in a SETUP says which end
    /// sent it, so the sender is an argument. A server may not send PATH, a
    /// client may.
    #[test]
