...
//! from inside a single draft: every layer there agrees with itself. Draft-18 is
//! the one where `close_for_data_stream` has the most to get right — its mapping
//! table answers the Object ID wrap as well, which draft-17's does not, so the
//! method here has to route two unrelated data-stream failures to two different
//! close codes without either arm swallowing the other.
//!
//! The section number moved too: the same sentence is draft-17 Section 10.2.1.2
//! and Section 11.2.1.2 here, which is reason enough not to let one
//! draft's gate speak for its neighbour.
//!
...
use moqtap_client::draft18::connection::{
    ClientConfig, Connection, ConnectionError, TransportType,
};
use moqtap_codec::dispatch::AnyControlMessage;
use moqtap_codec::draft18::data_stream::{SubgroupHeader, SubgroupObject, SubgroupObjectReader};
use moqtap_codec::draft18::message::{ControlMessage, Setup};
use moqtap_codec::draft18::types::ObjectStatus;
...
fn subgroup_stream_bytes() -> Vec<u8> {
    ...
    // Properties on End of Group: the combination Section 11.2.1.2 forbids. The
...
///
/// Deleting the `properties_permitted` check from draft-18's
...
async fn properties_on_a_non_normal_status_are_refused_and_close() {
    ...
    let peer = tokio::spawn(async move {
        ...
        // Draft-18 carries control on a pair of unidirectional streams, not on
