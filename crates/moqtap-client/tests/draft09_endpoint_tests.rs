...
fn make_active_client() -> Endpoint {
    ...
    ep.connect().unwrap();
    let versions = vec![varint(0xff000009)];
...
fn endpoint_receive_server_setup_activates_session() {
    ...
    let offered = vec![varint(0xff000009), varint(0xff000003)];
...
fn endpoint_blocked_without_max_subscribe_id() {
    let mut ep = Endpoint::new(Role::Client);
    ep.connect().unwrap();
    let _ = ep.send_client_setup(vec![varint(0xff000009)], vec![]).unwrap();
    let server_setup = ServerSetup { selected_version: varint(0xff000009), parameters: vec![] };
    ep.receive_server_setup(&server_setup).unwrap();
    assert!(ep.is_blocked());
}
