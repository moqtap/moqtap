...
//!
//! `draft20_fill_and_state_notify.rs` takes a fill fetch stream as far as its
//! FETCH_HEADER and finishes it. This file is what happens after the header,
//! which is where draft-20's numbering rules are and where a fill differs from
//! everything the earlier drafts had.
//!
//! # Why a fill stream is the place to test the delta rules
//!
//! A fill fetch stream is "delivered as a FETCH response" (Section 5.1.3), so
//! its Objects are framed by Section 11.4.4.1 — a Serialization Flags byte and
...
//!   11.4.4.1: "When the Group ID Delta field is not present, the Object ID is
...
//!   moves, and Section 11.4.4.1 still says "If Object ID Delta is not present,
...
//!
//! * **A second fill fetch stream on one subscription.** Section 5.1.3: "a
...
//!   the fill range was delivered (Section 5.1.3.1) and is the only end signal
...
//!   implementation: Section 10.2.21 makes the parameter govern the **Track
...
//!   that half.
//! * **The Group Order a fill is read in.** Section 10.2.8: "When it appears
...
use moqtap_codec::draft20::data_stream::{FetchHeader, GroupOrder};
...
/// `INCLUDE_PROPERTIES`, Parameter Type 0x35 (Section 10.2.21).
const INCLUDE_PROPERTIES: u64 = 0x35;

/// `LARGEST_OBJECT`, Parameter Type 0x09 — the parameter Section 10.10 makes a
...
fn first_fill_objects() -> Vec<u8> {
    ...
    // Delta (0x04), Group ID Delta (0x08), Priority (0x10). Section 11.4.4.1
...
/// payload: Section 11.2.1.1 puts Object Status only on subscription-delivered
...
fn descending_fill_objects() -> Vec<u8> {
    ...
    // 0x0D = 0b0000_1101: mode 0b01, both deltas present. Section 11.4.4.1:
...
async fn serve_three_fills(server: quinn::Endpoint) {
    ...
    // Section 5.1.3.1: "the publisher signals a fill failure by resetting the
...
/// timeout.
async fn read_one(
    stream: &mut moqtap_client::draft20::connection::FramedRecvStream,
) -> (moqtap_codec::draft20::data_stream::FetchObject, Vec<u8>) {
...
fn subscribe_parameters(fill: FillParameters) -> Vec<KeyValuePair> {
    vec![
        ...
        // Type that the wire requires. Section 10.2.21 gives the value 0 as
...
async fn objects_on_a_fill_stream_resolve_where_the_deltas_put_them() {
    ...
    assert_eq!(
        ...
        "no GROUP_ORDER in either scope, so Section 10.2.8's FETCH default stands"
    );
    ...
    assert_eq!(
        ...
        "INCLUDE_PROPERTIES = 0 on the SUBSCRIBE governs Track Properties in the OK message \
         (Section 10.2.21); Object Properties on a fill Object answer to flag 0x20 alone"
    );
    assert_eq!(payload, PAYLOAD);

    // Section 5.1.3.1: "The publisher signals that the fill is complete by
    ...
    let (header, mut second_fill) = tokio::time::timeout(PATIENCE, conn.accept_fill_stream())
        ...
        .expect("Section 5.1.3 allows several against one subscription");
    assert_eq!(
        header.request_id(),
    ...
    assert_eq!(
        code,
        Some(FILL_FAILED),
        "Section 5.1.3.1: 'the publisher signals a fill failure by resetting the stream'"
    );
    conn.endpoint_mut().on_fill_stream_ended(request_id);
    ...
    assert_eq!(
        ...
        1,
        "Section 5.1.3.1: resetting a fill fetch stream 'does not affect the subscription'"
    );
