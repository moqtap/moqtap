...
///
/// The proxy forwards data stream bytes verbatim; raw wire bytes for
/// headers are not exposed here because nothing in the forwarding path
/// needs them. If byte-level mutation of data streams is ever required,
/// add a dedicated variant or a capturing parser mode.
#[derive(Debug)]
            ...
            match self.state {
                DataStreamState::AwaitingHeader => match self.try_parse_header() {
                    Some(Ok(header)) => {
                        ...
                        results.push(DataParseResult::Header(header));
                    }
                    Some(Err(e)) => {
                ...
                DataStreamState::InStream => match self.try_parse_object() {
                    Some(Ok(header)) => {
                        results.push(DataParseResult::Object(header));
                    }
                    Some(Err(e)) => {
                        results.push(DataParseResult::Error(e));
                        break;
        ...
        match self.stream_type {
            DataStreamType::Subgroup => match AnySubgroupHeader::decode(self.draft, &mut cursor) {
                Ok(header) => {
                    ...
                    Some(Ok(DataStreamHeaderKind::Subgroup(header)))
                }
                Err(e) => {
            ...
            DataStreamType::Fetch => match AnyFetchHeader::decode(self.draft, &mut cursor) {
                Ok(header) => {
                    let consumed = snapshot.len() - cursor.remaining();
                    self.buf.advance(consumed);
                    Some(Ok(DataStreamHeaderKind::Fetch(header)))
                }
                Err(e) => {
                    if is_incomplete_error(&e) {
                        None
