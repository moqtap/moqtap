...
use moqtap_codec::draft14::data_stream::{
    DatagramHeader, FetchHeader, ObjectHeader, SubgroupHeader,
};
...
fn data_stream_header_kind_fetch() {
    let header = AnyFetchHeader::Draft14(FetchHeader {
        track_alias: VarInt::from_u64(2).unwrap(),
...
fn proxy_event_session_started() {
    ...
    let event = ProxyEvent::SessionStarted { session_id: SessionId(1), client_addr: addr };
...
fn proxy_event_datagram() {
    let header = AnyDatagramHeader::Draft14(DatagramHeader {
        track_alias: VarInt::from_u64(1).unwrap(),
        group: VarInt::from_u64(0).unwrap(),
...
fn proxy_event_object_header() {
    let header = AnyObjectHeader::Draft14(ObjectHeader {
        object_status: ObjectStatus::Normal,
        payload_length: Some(VarInt::from_u64(100).unwrap()),
...
fn noop_observer_does_not_panic() {
    ...
    let addr: SocketAddr = "127.0.0.1:1234".parse().unwrap();
    obs.on_event(&ProxyEvent::SessionStarted { session_id: SessionId(1), client_addr: addr });
...
fn observer_is_object_safe() {
    ...
    obs.on_event(&ProxyEvent::SessionStarted { session_id: SessionId(99), client_addr: addr });
